...
#if IS_ENABLED(CONFIG_BLK_SEC_HCGC)
...
static struct gendisk *get_gendisk(dev_t devno)
{
	struct block_device *bdev;
	struct file *bdev_file;
	struct gendisk *gd = NULL;

	bdev_file = bdev_file_open_by_dev(devno, BLK_OPEN_READ, NULL, NULL);
	if (IS_ERR(bdev_file))
		return NULL;
