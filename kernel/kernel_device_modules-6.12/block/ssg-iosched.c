...
static void init_ssg_rq_info(struct ssg_data *ssg, unsigned int nr_requests)
{
	...
	rq_info = kmalloc_array(nr_requests, sizeof(struct ssg_request_info),
				GFP_KERNEL | __GFP_ZERO);
	if (ZERO_OR_NULL_PTR(rq_info))
		return;

	for (i = 0; i < nr_requests; i++)
		rq_info[i].rq_type = RQ_TYPE_UNKNOWN;
...
static inline int ssg_rq_type(struct ssg_data *ssg, struct request *rq)
{
	struct ssg_request_info *rqi = ssg_rq_info(ssg, rq);

	if (likely(rqi) && rqi->rq_type != RQ_TYPE_UNKNOWN)
		return rqi->rq_type;

	return rq_data_dir(rq);
}

static inline struct rb_root *ssg_rb_root(struct ssg_data *ssg, int rq_type)
{
	return &ssg->sort_list[rq_type];
}
...
static void ssg_add_rq_rb(struct ssg_data *ssg, struct request *rq, int rq_type)
{
	struct rb_root *root = ssg_rb_root(ssg, rq_type);

	elv_rb_add(root, rq);
}

static inline void ssg_del_rq_rb(struct ssg_data *ssg, struct request *rq, int rq_type)
{
	if (ssg->next_rq[rq_type] == rq)
		ssg->next_rq[rq_type] = ssg_latter_request(rq);

	elv_rb_del(ssg_rb_root(ssg, rq_type), rq);
}
...
static void ssg_remove_request(struct request_queue *q, struct request *rq)
{
	...
	if (!RB_EMPTY_NODE(&rq->rb_node))
		ssg_del_rq_rb(ssg, rq, ssg_rq_type(ssg, rq));
...
{
	struct ssg_data *ssg = q->elevator->elevator_data;
	int rq_type = ssg_rq_type(ssg, req);

	/*
	 * if the merge was a front merge, we need to reposition request
	 */
	if (type == ELEVATOR_FRONT_MERGE) {
		elv_rb_del(ssg_rb_root(ssg, rq_type), req);
		ssg_add_rq_rb(ssg, req, rq_type);
	}
}
...
static void ssg_move_request(struct ssg_data *ssg, struct request *rq)
{
	...
	ssg->next_rq[ssg_rq_type(ssg, rq)] = ssg_latter_request(rq);
 ...
 * 1 otherwise. Requires !list_empty(&ssg->fifo_list[rq_type])
 */
static inline int ssg_check_fifo(struct ssg_data *ssg, int rq_type)
{
	struct request *rq = rq_entry_fifo(ssg->fifo_list[rq_type].next);
 ...
 */
static struct request *ssg_fifo_request(struct ssg_data *ssg, int rq_type)
{
	if (list_empty(&ssg->fifo_list[rq_type]))
		return NULL;

	return rq_entry_fifo(ssg->fifo_list[rq_type].next);
}
 ...
 */
static struct request *ssg_next_request(struct ssg_data *ssg, int rq_type)
{
	return ssg->next_rq[rq_type];
}
...
dispatch_find_request:
	/*
	 * we are not running a batch, find best request for selected rq_type
	 */
	next_rq = ssg_next_request(ssg, rq_type);
	if (ssg_check_fifo(ssg, rq_type) || !next_rq) {
		/*
		 * A deadline has expired, the last request was in the other
		 * direction, or we have run out of higher-sectored requests.
		 * Start again from the request with the earliest expiry time.
		 */
		rq = ssg_fifo_request(ssg, rq_type);
	} else {
		/*
...
static int ssg_init_queue(struct request_queue *q, struct elevator_type *e)
{
	...
	init_ssg_rq_info(ssg, q->nr_requests);
			      ...
			      blk_insert_t flags)
{
	struct request_queue *q = hctx->queue;
	struct ssg_data *ssg = q->elevator->elevator_data;
	const int rq_type = ssg_rq_type(ssg, rq);
	...
	if ((flags & BLK_MQ_INSERT_AT_HEAD) || blk_rq_is_passthrough(rq)) {
		 ...
		 */
		rq->fifo_time = jiffies + ssg->fifo_expire[rq_type];
		list_add_tail(&rq->queuelist, &ssg->fifo_list[rq_type]);
	}
}
