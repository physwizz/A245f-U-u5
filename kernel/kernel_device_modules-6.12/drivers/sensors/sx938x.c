...
static int sx938x_parse_dt(struct sx938x_p *data, struct device *dev)
{
	...
	if (data->ic_num == MAIN_GRIP)
		data->ldo_en = of_get_named_gpio(dNode, "sx938x,ldo_en", 0);
#if IS_ENABLED(CONFIG_SENSORS_SX9380_SUB)
	if (data->ic_num == SUB_GRIP)
		data->ldo_en = of_get_named_gpio(dNode, "sx938x_sub,ldo_en", 0);
#endif
#if IS_ENABLED(CONFIG_SENSORS_SX9380_SUB2)
	if (data->ic_num == SUB2_GRIP)
		data->ldo_en = of_get_named_gpio(dNode, "sx938x_sub2,ldo_en", 0);
#endif
#if IS_ENABLED(CONFIG_SENSORS_SX9380_WIFI)
	if (data->ic_num == WIFI_GRIP)
		data->ldo_en = of_get_named_gpio(dNode, "sx938x_wifi,ldo_en", 0);
#endif
...
static int sx938x_probe(struct i2c_client *client)
#else
...
static void sx938x_remove(struct i2c_client *client)
#else
