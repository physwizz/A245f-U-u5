...
#elif !defined(CONFIG_MST_IF_PMIC)
...
static int mst_use_unolim_value;
#endif
#endif
/* function */
/**
...
static void of_mst_hw_onoff(bool on)
{
	...
	if (regulator3_0 == NULL) {
		...
		regulator3_0 = regulator_get(NULL, "sec-mst,mst-pwr-regulator");
		if(regulator3_0 == NULL)
	...
	if (on) {
#if defined(CONFIG_MFC_CHARGER)
		/* Skip MST when wireless charging ON */
		psy_do_property("mfc-charger", get, POWER_SUPPLY_EXT_PROP_MST_WIRELESS_PDRC_STATE, value);
		if(value.intval == 1){
			mst_info("%s: wpc is attached!!!\n", __func__);
			return;
		}
		...
		if (regulator3_0) {
			...
			if (ret < 0) {
				mst_info("%s: Cannot enable regulator\n", __func__);
			}
		...
		if (mst_pwr_use_uno) {
#if !defined(CONFIG_MST_VOUT_SETTING)
			value.intval = mst_use_unolim_value;
			psy_do_property("otg", set,
					POWER_SUPPLY_EXT_PROP_WIRELESS_TX_IOUT, value);
#endif
			value.intval = ON;
...
#if !defined(CONFIG_MST_IF_PMIC) && !defined(CONFIG_MST_REGULATOR)
...
#if defined(CONFIG_MST_REGULATOR)
	regulator3_0 = regulator_get(dev, "sec-mst,mst-pwr-regulator");
	if (regulator3_0 == NULL) {
...
static int mst_ldo_device_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "sec-mst,mst-use-unolim-value",
					&mst_use_unolim_value);
	if (ret < 0) {
		pr_info("%s: fail to read mst-use-unolim-value. default set 1.5A\n", __func__);
		mst_use_unolim_value = 1500;  /* 1.5A */
	}
#endif
