...
static void ufs_sec_set_unique_number(struct ufs_hba *hba, u8 *desc_buf)
{
	...
	if (err) {
		ufs_sec_pr(err, "%s: Failed reading string descriptor. err %d",
				__func__, err);
		goto out;
	}
	...
	memset(get_vdi_member(unique_number), 0, sizeof(get_vdi_member(unique_number)));

	snprintf(get_vdi_member(unique_number), UFS_UN_MAX_DIGITS,
	...
	ufs_sec_pr(debug, "%s: ufs un : %s\n", __func__, get_vdi_member(unique_number));
out:
	kfree(str_desc_buf);
}
...
void ufs_sec_get_health_desc(struct ufs_hba *hba)
{
	...
	if (err) {
		ufs_sec_pr(err, "%s: Failed reading health descriptor. err %d",
				__func__, err);
		goto out;
	}
	...
	ufs_sec_pr(info, "LT: 0x%02x, FLT: %u, ELI: 0x%01x\n",
...
{
	return !!(ufs_sec_features.ext_ufs_feature_sup & mask);
}
...
static inline int ufs_sec_is_host_available(struct ufs_hba *hba)
{
	...
	if (hba->pm_op_in_progress || hba->is_sys_suspended) {
		ufs_sec_pr(err, "%s: pm_op_in_progress or suspended.\n", __func__);
		return -EBUSY;
	}

	if (hba->ufshcd_state != UFSHCD_STATE_OPERATIONAL) {
		ufs_sec_pr(err, "%s: UFS Host state=%d.\n", __func__,
				hba->ufshcd_state);
		return -EBUSY;
	}
...
static int __ufs_sec_wb_ctrl(bool enable)
{
	...
	ufs_sec_pr(info, "%s(%s) is %s, ret=%d.\n", __func__,
...
int ufs_sec_wb_ctrl(bool enable)
{
	...
	if (ret) {
		ufs_sec_pr(err, "%s: it's not available(%d).\n", __func__, ret);
		goto out;
	}

	if (!(enable ^ hba->dev_info.wb_enabled)) {
		ufs_sec_pr(info, "%s: write booster is already %s\n",
...
EXPORT_SYMBOL_GPL(ufs_sec_wb_ctrl);
...
static void ufs_sec_wb_toggle_flush_during_h8(struct ufs_hba *hba, bool set)
{
	...
	ufs_sec_pr(info, "%s: %s WB flush during H8 is %s.\n", __func__,
			set ? "set" : "clear",
			ret ? "failed" : "done");
}
...
EXPORT_SYMBOL_GPL(ufs_sec_wb_register_reset_notify);
...
{
	struct device *dev = ufs_sec_features.dev;
	struct ufs_dev_info *dev_info = &hba->dev_info;
	...
	if (!ufs_sec_check_ext_feature(UFS_DEV_WRITE_BOOSTER_SUP)) {
		ufs_sec_pr(info, "UFS ext feature does not support WB.\n");
		goto wb_disabled;
	}
	...
	if (!dev_info->b_presrv_uspc_en) {
		ufs_sec_pr(err, "%s: Failed preserved user space en value",
				__func__);
		goto wb_disabled;
	}
	...
	if (!wb_buf_alloc) {
		ufs_sec_pr(err, "%s: Failed wb_buf_alloc", __func__);
		goto wb_disabled;
	}

	ufs_wb = devm_kzalloc(dev, sizeof(struct ufs_sec_wb_info),
			GFP_KERNEL);
	if (!ufs_wb) {
		ufs_sec_pr(err, "%s: Failed allocating ufs_wb(%lu)",
				__func__, sizeof(struct ufs_sec_wb_info));
		goto wb_disabled;
	}

	ufs_wb_backup = devm_kzalloc(dev, sizeof(struct ufs_sec_wb_info),
			GFP_KERNEL);
	if (!ufs_wb_backup) {
		ufs_sec_pr(err, "%s: Failed allocating ufs_wb_backup(%lu)",
				__func__, sizeof(struct ufs_sec_wb_info));
		goto wb_disabled;
	}
	...
	ufs_sec_pr(info, "%s: SEC WB is supported. type=%s%d, size=%u.\n",
			__func__,
		...
		char *err_name)
{
	...
	for (i = 0; i < UFS_EVENT_HIST_LENGTH; i++) {
		...
		ufs_sec_pr(err, "%s[%d] = 0x%x at %lld us\n", err_name, p,
			e->val[p], ktime_to_us(e->tstamp[p]));
		found = true;
	}

	if (!found)
		ufs_sec_pr(err, "No record of %s\n", err_name);
	else
		ufs_sec_pr(err, "%s: total cnt=%llu\n", err_name, e->cnt);
}
...
static void ufs_sec_print_evt_hist(struct ufs_hba *hba)
{
	...
	ufs_sec_print_evt(hba, UFS_EVT_SUSPEND_ERR,
			"suspend_fail");
	ufs_sec_print_evt(hba, UFS_EVT_WL_RES_ERR, "wlun resume_fail");
	ufs_sec_print_evt(hba, UFS_EVT_WL_SUSP_ERR,
			 "wlun suspend_fail");
...
static void ufs_sec_print_cmdlog(struct ufs_hba *hba)
{
	...
	i = (ufs_cmd_log->pos + UFS_SEC_CMD_LOGGING_MAX
			- UFS_SEC_TESTMODE_CMD_LOG_MAX);

	ufs_sec_pr(err, "UFS CMD hist\n");
	ufs_sec_pr(err, "%2s: %10s: %2s %3s %4s %9s %6s %16s\n",
	...
	for (idx = 0; idx < UFS_SEC_TESTMODE_CMD_LOG_MAX; idx++, i++) {
		...
		ufs_sec_pr(err, "%2d: %10s: %2d %3d 0x%02x %9u %6d %16llu\n",
				idx,
...
static void ufs_sec_init_error_logging(void)
{
	struct device *dev = ufs_sec_features.dev;
	...
	if (!ufs_err || !ufs_err_backup || !ufs_err_hist) {
		ufs_sec_pr(err, "%s: Failed allocating ufs_err(backup)(%lu)",
 ...
 *
 * Format : U0I0H0L0X0Q0R0W0F0SM0SH0SI0SP0SO0HB0
...
EXPORT_SYMBOL_GPL(ufs_sec_print_err);
		...
		unsigned long event, void *panic_msg)
{
	...
	if (is_FSpanic && ufs_sec_is_err_counted(err_buf))
...
static int ufs_sec_reboot_notify(struct notifier_block *nfb,
...
static void ufs_sec_err_noti_work(struct work_struct *work)
{
	...
	if (ret)
		ufs_sec_pr(err, "%s: Failed to send uevent with err %d\n", __func__, ret);
}

static int ufs_sec_err_uevent(const struct device *dev, struct kobj_uevent_env *env)
{
...
static inline bool ufs_sec_is_uevent_condition(void)
{
...
static void ufs_sec_trigger_err_noti_uevent(struct ufs_hba *hba)
{
	...
	if (!ufs_sec_is_uevent_condition())
		return;
...
static void ufs_sec_init_cmd_logging(void)
{
	struct device *dev = ufs_sec_features.dev;
	...
	ufs_cmd_log = devm_kzalloc(dev, sizeof(struct ufs_sec_cmd_log_info),
			GFP_KERNEL);
	if (!ufs_cmd_log) {
		ufs_sec_pr(err, "%s: Failed allocating ufs_cmd_log(%lu)",
				__func__,
	...
	if (!ufs_cmd_log->entries) {
		ufs_sec_pr(err, "%s: Failed allocating cmd log entry(%lu)",
				__func__,
	...
	ufs_sec_pr(info, "SEC UFS cmd logging is initialized.\n");

	ufs_sec_features.ufs_cmd_log = ufs_cmd_log;
}
/* SEC cmd log : end */

bool ufs_sec_is_flush_needed(void)
{
	return !get_vdi_member(skip_flush);
}
EXPORT_SYMBOL_GPL(ufs_sec_is_flush_needed);

void ufs_sec_set_skip_flush(bool set)
{
	set_vdi_member(skip_flush, set);
}
EXPORT_SYMBOL_GPL(ufs_sec_set_skip_flush);
...
void ufs_sec_check_device_stuck(void)
{
	...
	/* at the 1st device reset, vdi->hba is not ready yet */
	if (!vdi || !vdi->hba)
		return;
...
EXPORT_SYMBOL_GPL(ufs_sec_check_device_stuck);

void ufs_sec_init_logging(void)
{
	ufs_sec_init_error_logging();

	ufs_sec_init_cmd_logging();
}
...
EXPORT_SYMBOL_GPL(ufs_sec_adjust_caps_quirks);
...
EXPORT_SYMBOL_GPL(ufs_sec_config_features);
...
void ufs_sec_set_features(struct ufs_hba *hba)
{
	...
	err = ufshcd_read_desc_param(hba,
			...
			desc_buf, QUERY_DESC_MAX_SIZE);
	if (err) {
		ufs_sec_pr(err, "%s: Failed reading device desc. err %d",
				__func__, err);
		goto out;
	}
...
out:
#if IS_ENABLED(CONFIG_SCSI_UFS_TEST_MODE)
	ufs_sec_pr(info, "UFS test mode enabled\n");
#endif
	kfree(desc_buf);
}
		...
		struct ufs_hba *hba, int tag, int str_t)
{
	...
	if (str_t == UFS_TM_SEND) {
		...
		ufs_sec_pr(err, "%s: undefined ufs tm cmd\n", __func__);
		return;
	}
...
{
	/* Set scsi timeout to 10s */
	sdev->request_queue->rq_timeout = SCSI_UFS_TIMEOUT;

	/* Enable fua write. */
	sdev->broken_fua = 0;

	/* Skip sending mode page 0xa, Command Duration Limit mode page */
