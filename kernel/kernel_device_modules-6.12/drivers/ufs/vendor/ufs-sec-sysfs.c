		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!hba) {
		...
		dev_info(dev, "ufshcd_state: %d, old LT: %01x\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!hba) {
		...
		dev_info(dev, "ufshcd_state : %d, old FLT: %u\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!hba) {
		...
		dev_info(dev, "ufshcd_state: %d, old eli: %01x\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_err(dev, "post_ffu is not allowed if test mode is enabled\n");
	...
	if (ret) {
		...
		/* add delay for entering hibern8 */
		mdelay(2);
...
resume_rpm:
	...
	if (ret) {
		dev_err(dev, "post_ffu error(%d).\n", ret);
		return ret;
	}

	dev_info(dev, "post_ffu finish\n");
...
static int ufs_sec_create_sysfs_dev(struct ufs_hba *hba)
{
	...
	if (IS_ERR_OR_NULL(sec_ufs_node_dev)) {
		dev_err(sec_ufs_node_dev, "Fail to create sysfs dev\n");
		return -ENODEV;
	}
		...
		const struct attribute_group *dev_attr_group, const char *group_name)
{
	...
	if (ret)
		dev_err(dev, "Failed to create %s sysfs group (err=%d)\n",
				group_name, ret);
}

static void ufs_sec_remove_sysfs_group(struct device *dev,
...
void ufs_sec_remove_sysfs_nodes(struct ufs_hba *hba)
{
	...
	if (shost_dev)
		ufs_sec_remove_sysfs_group(shost_dev,
				&sec_ufs_error_attribute_group);

	if (sec_ufs_node_dev) {
		ufs_sec_remove_sysfs_group(sec_ufs_node_dev,
