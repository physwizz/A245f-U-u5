...
static int __kzt_offset_inode_offsets(u32 *offsets)
{
	...
	do {
		...
		offsets[I_ATIME_OF_INODE] = offsetof(struct inode, i_atime_sec);
		if (offsets[I_ATIME_OF_INODE] >= sizeof(struct inode))
			break;

		offsets[I_MTIME_OF_INODE] = offsetof(struct inode, i_mtime_sec);
		if (offsets[I_MTIME_OF_INODE] >= sizeof(struct inode))
			break;

		offsets[I_CTIME_OF_INODE] = offsetof(struct inode, i_ctime_sec);
