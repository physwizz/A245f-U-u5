	...
	char *pctrl_name)
{
	...
	*pstate = pinctrl_lookup_state(pctrl, pctrl_name);
	...
	if (*pstate)
		LOG_INF("sccuess to init (%s) %p\n", pctrl_name, (void *)(*pstate));
...
int cam_ois_pinctrl_get_state_done(struct mcu_info *mcu_info)
{
	if (
		...
		&& (mcu_info->ois_vdd_regulator != NULL)
		) {
		LOG_INF("done before %p\n", (void *)(mcu_info->pinctrl));
		return 0;
	}
	return -1;
}
...
int cam_ois_pinctrl_get_state(struct mcu_info *mcu_info)
{
	...
	mcu_info->ois_vdd_regulator = regulator_get_optional(mcu_info->pdev, CAM_OIS_REGULATOR_OIS);
	if (IS_ERR_OR_NULL(mcu_info->ois_vdd_regulator)) {
		ret = PTR_ERR(mcu_info->ois_vdd_regulator);
		LOG_ERR("fail to get OIS VDD Regulator %d", ret);
		mcu_info->ois_vdd_regulator = NULL;
	}
	mcu_info->ois_vdd_voltage = 3300000; //v3.3
...
int cam_ois_mcu_power_up(struct mcu_info *mcu_info)
{
	...
	if (mcu_info->power_count > 0)
		...
		ret |= cam_ois_regulator_enable(mcu_info->ois_vdd_regulator, mcu_info->ois_vdd_voltage);
...
int cam_ois_mcu_power_down(struct mcu_info *mcu_info)
{
	...
	if (mcu_info->power_count > 1)
		...
		ret |= cam_ois_regulator_disable(mcu_info->ois_vdd_regulator);
		if (ret)
			LOG_ERR("fail to regulator_disable%d\n", ret);
