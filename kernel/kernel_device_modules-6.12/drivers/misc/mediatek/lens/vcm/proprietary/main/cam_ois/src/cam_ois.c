...
struct stAF_DrvList af_drv_list[MAX_NUM_OF_LENS] = {
	...
	{1, AFDRV_DW9818AF, DW9818AF_SetI2Cclient,
	DW9818AF_Ioctl, DW9818AF_Release, DW9818AF_GetFileName, NULL},
};
#else
struct stAF_DrvList af_drv_list[MAX_NUM_OF_LENS] = {
	// {1, AFDRV_AK7377AF_OIS, AK7377AF_OIS_SetI2Cclient, AK7377AF_OIS_Ioctl,
...
int cam_ois_mcu_update(void)
{
	...
	if (mcu_info.pinctrl != NULL && i2c_client != NULL) {
		...
		LOG_ERR("fw_updated %d, pinctrl %p, i2c %p", mcu_info.is_updated, mcu_info.pinctrl, i2c_client);
	}
	return 0;
}
...
{
	LOG_DBG("pinctrl: %p, mcu boot: %p, %p, mcu nRST: %p, %p, mcu VDD: %p, %p, ois VDD: %p, %p",
	...
	long *raw_data_x, long *raw_data_y, long *raw_data_z)
{
	...
	if (detect_comma) {
		...
		if (ret)
    		LOG_ERR("Failed to convert string to long");
...
{
	if (!i2c_client_in) {
		LOG_ERR("cam_ois_set_af_client i2c_client_in is NULL\n");
		return -1;
	}
...
static int cam_ois_af_setup(void)
{
	...
	if (i == retry) {
		LOG_ERR("AFIOC_S_SETOISAUTO fail, ret %d", ret);
#if IS_ENABLED(CONFIG_CAMERA_AAY_V17)
		ret = 0;// temp modify
#endif
	}
