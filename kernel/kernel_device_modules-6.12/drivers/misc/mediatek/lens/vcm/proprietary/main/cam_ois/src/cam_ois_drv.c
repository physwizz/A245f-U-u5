...
static inline int cam_ois_chrdev_register(void)
{
	...
	g_drvClass = class_create(CAM_OIS_CLASS_NAME);
 ...
 * cam_ois_i2c_probe
 **************************************************/
static int cam_ois_i2c_probe(struct i2c_client *client)
{
	int ret = 0;
 ...
 **********************************************/
static void cam_ois_i2c_remove(struct i2c_client *client)
{
 ...
 * cam_ois_i2c_probe2
 ***********************************************/
static int cam_ois_i2c_probe2(struct i2c_client *client)
{
	LOG_INF(" - E");
	/* get sensor i2c client */
	spin_lock(&g_spinLock);
 ...
 ********************************************************/
static void cam_ois_i2c_remove2(struct i2c_client *client)
{
}
...
static int cam_ois_probe(struct platform_device *pdev)
{
	...
	LOG_INF("I2C Client2 0x%p", g_pstI2Cclients[OIS_I2C_DEV_IDX_2]);
	...
	LOG_INF("I2C Client1 0x%p", g_pstI2Cclients[OIS_I2C_DEV_IDX_1]);

	ret = cam_ois_prove_init();
 
	// cam_ois_chrdev_register();
	if (cam_ois_sysfs_init()) {
		LOG_ERR("Deferred Probe! fail to init cam ois sysfs");
		return -EPROBE_DEFER;
	}
 ...
 * cam_ois_remove()
 *******************************************************/
static void cam_ois_remove(struct platform_device *pdev)
{
