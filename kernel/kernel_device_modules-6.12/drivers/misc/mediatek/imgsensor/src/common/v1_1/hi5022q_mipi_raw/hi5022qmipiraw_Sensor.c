...
static void write_sensor_hw_xgc(void)
{
	...
	write_data = (kal_uint16 *)vzalloc(HI5022Q_XGC_HALF_SIZE * sizeof(kal_uint16));
	if (write_data == NULL) {
		LOG_ERR("Failed to allocate memory using vzalloc\n");
		return;
	}
...
static void write_sensor_hw_qbgc(void)
{
	...
	write_data = (kal_uint16 *)vzalloc(HI5022Q_QBGC_SIZE * sizeof(kal_uint16));
	if (write_data == NULL) {
		LOG_ERR("Failed to allocate memory using vzalloc\n");
		return;
	}
 ...
 */
	...
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
		LOG_DBG("SENSOR_FEATURE_GET_MIPI_PIXEL_RATE = %d, MSDK_SCENARIO_ID = %lld\n",
			feature_id, *feature_data);
