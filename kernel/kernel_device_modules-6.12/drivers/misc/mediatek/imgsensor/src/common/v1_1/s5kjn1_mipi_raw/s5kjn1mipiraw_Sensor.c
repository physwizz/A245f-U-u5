...
#define SENSOR_JN1_COARSE_INTEGRATION_TIME_MAX_MARGIN (10)
#define SENSOR_JN1_MAX_COARSE_INTEGRATION_TIME        (65535)
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.frame_time_delay_frame = 1, //changed value by MTK guide(1->2)
	.ihdr_support = 0,
...
#if IS_ENABLED(CONFIG_CAMERA_AAY_V17)
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (imgsensor.frame_length > SENSOR_JN1_MAX_COARSE_INTEGRATION_TIME) {
		int max_coarse_integration_time =
			SENSOR_JN1_MAX_COARSE_INTEGRATION_TIME - SENSOR_JN1_COARSE_INTEGRATION_TIME_MAX_MARGIN;

		cit_lshift_val = (kal_uint32)(imgsensor.frame_length / SENSOR_JN1_MAX_COARSE_INTEGRATION_TIME);
...
static void sensor_HW_GGC_write(void)
{
	...
	write_data_size = data_size * 2; //address + data

	write_data = kmalloc(write_data_size + 4, GFP_KERNEL); //4 is margin
	if (write_data == NULL) {
		LOG_ERR("memory allocation fail");
	} else {
		//data size is always even.
		write_cnt = data_size / 2;

		for (i = 0; i < write_cnt; i++) {
			write_data[2*i] = 0x6F12; //address
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_NIGHTMODE:
		LOG_INF("SENSOR_FEATURE_SET_NIGHTMODE = %d, data=%lld\n", feature_id, *feature_data);
 ...
 */
	...
	case SENSOR_FEATURE_GET_PIXEL_RATE:
		...
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			LOG_INF("MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.cap.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			LOG_INF("MSDK_SCENARIO_ID_VIDEO_PREVIEW = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.normal_video.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			LOG_INF("MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.hs_video.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			LOG_INF("MSDK_SCENARIO_ID_SLIM_VIDEO = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.slim_video.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
		LOG_INF("MSDK_SCENARIO_ID_CUSTOM1 = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.custom1.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM2:
			LOG_INF("MSDK_SCENARIO_ID_CUSTOM2 = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.custom2.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM3:
			LOG_INF("MSDK_SCENARIO_ID_CUSTOM3 = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.custom3.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM4:
			LOG_INF("MSDK_SCENARIO_ID_CUSTOM4 = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.custom4.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CUSTOM5:
			LOG_INF("MSDK_SCENARIO_ID_CUSTOM5 = %lld\n", *feature_data);
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = (imgsensor_info.custom5.pclk /
			...
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			LOG_INF("MSDK_SCENARIO_ID_CAMERA_PREVIEW = %lld\n", *feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		LOG_INF("scenario_id %lld, binning_type %d,\n", *(feature_data + 1), *feature_return_para_32);
		*feature_para_len = 4;
		break;
