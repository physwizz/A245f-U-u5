...
void IMGSENSOR_PROFILE(struct timespec64 *ptv, char *tag)
{
	...
	PK_INFO("[%s]Profile = %llu ns[about %lld ms]",
...
#if IS_ENABLED(CONFIG_KG_DRV)
		if (kg_resource_is_locked(KG_RESOURCE_CAMERA)) {
			PK_PR_ERR("[%s] cam open fail by KG 3.0 Policy\n", __func__);
			kfree(pFeaturePara);
			return -EINVAL;
		}
#endif
...
#if defined(CONFIG_CAMERA_AAW_V24) || defined(CONFIG_CAMERA_AAX_V15) || defined(CONFIG_CAMERA_AAX_V16) || defined(CONFIG_CAMERA_AAY_V17)
			char *XTC_CAL = NULL;
