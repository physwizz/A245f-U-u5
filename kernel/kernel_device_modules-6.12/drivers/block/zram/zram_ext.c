			...
			void *addr, int size, u32 index)
{
	...
	if (strncmp(zram->comps[prio]->ops->name, "lzo-rle", 7))
		return;
...
void zram_decomp_fail(struct zram *zram, u32 prio,
		u32 index, int size, int ret, void *addr)
{
	pr_err("%s Decompression failed! err=%d, index=%u, len=%d, vaddr=0x%px\n",
		zram->comps[prio]->ops->name, ret, index, size, addr);
	print_hex_dump_fmt(addr, size);
	zram_error_count_store(zram, ERR_TYPE1);
}
		...
		struct page *dst_page, unsigned long handle)
{
	...
	ret = zcomp_decompress(zram->comps[prio], zstrm, addr, size, dst);
	zcomp_stream_put(zram->comps[prio]);
	if (unlikely(ret))
		zram_decomp_fail(zram, prio, index, size, ret, addr);
out:
	kunmap_atomic(dst);
	kunmap_atomic(src);
...
static int try_read_from_bdev(struct zram *zram, struct page *page,
		u32 index, struct bio *parent)
{
	...
	if (test_and_set_bit(chunk_idx, zram->read_bitmap)) {
			  ...
			  struct bio *parent)
{
	...
	zram_slot_lock(zram, index);
	while (zram_test_flag(zram, index, ZRAM_WB)) {
		zram_slot_unlock(zram, index);
		ret = try_read_from_bdev(zram, page, index, parent);
		if (!ret)
			return ret;
		zram_slot_lock(zram, index);
	}
	...
	while (!kthread_should_stop()) {
		...
		while (atomic_read(&zram->nr_prefetch)) {
			...
			while (req->first < req->last) {
				...
				try_read_from_bdev(zram, NULL, index, NULL);
				spin_lock(&zram->prefetch_lock);
			}
			spin_unlock(&zram->prefetch_lock);
...
static void zram_count_entry_type(void *data,
		swp_entry_t swpent, int map_count,
...
static void zram_show_entry_type(void *data, struct seq_file *m,
		...
		unsigned long same, unsigned long huge)
{
	seq_printf(m, "SwapShared:     %8lu kB\n", K(swap_shared));
	seq_printf(m, "Writeback:      %8lu kB\n", K(writeback));
	seq_printf(m, "Same:           %8lu kB\n", K(same));
	seq_printf(m, "Huge:           %8lu kB\n", K(huge));
}
...
int init_zram_ext(struct zram *zram, unsigned long nr_pages, unsigned int size)
{
	...
	spin_lock_init(&zram->refcount_lock);
