...
#define NOTI_PER_ERR(val, cnt) ((val) && !((val) % (cnt))) ? true : false

struct mmc_sd_sec_device_info sdi;

static inline bool sd_sec_notify_error(struct sd_sec_status_err_info *status_err)
{
	return (NOTI_PER_ERR(status_err->ge_cnt, 1000) ||
		NOTI_PER_ERR(status_err->ecc_cnt, 1000) ||
		NOTI_PER_ERR(status_err->wp_cnt, 100) ||
		NOTI_PER_ERR(status_err->oor_cnt, 100));
}

static int sd_sec_sdcard_uevent(const struct device *dev,
		struct kobj_uevent_env *env)
{
	...
	retval = add_uevent_var(env, "IOERROR=%s", card_exist ? (
		sd_sec_notify_error(status_err) ?
		"YES" : "NO") : "NoCard");
...
static void sd_sec_sdcard_noti_work(struct work_struct *work)
{
	...
	pr_info("%s: Send notification for SD card IO error. cnt(%d)\n",
			mmc_hostname(op_info->mmc), status_err->noti_cnt);
	status_err->noti_cnt++;
...
static void sd_sec_inc_status_err(struct mmc_card *card, u32 status)
{
	...
	noti = sd_sec_notify_error(status_err);
