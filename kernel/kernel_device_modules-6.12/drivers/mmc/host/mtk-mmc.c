...
static const struct mtk_mmc_compatible mt6858_compat = {
	.clk_div_bits = 12,
	.recheck_sdio_irq = false,
	.hs400_tune = false,
	.pad_tune_reg = MSDC_PAD_TUNE0,
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_info(&pdev->dev, "mmc_regulator_get_supply err\n");
		goto host_free;
	}
	...
	if (host->irq < 0) {
		ret = -EINVAL;
		dev_info(host->dev, "get irq num failed!\n");
		goto host_free;
	}
	...
	if (!host->dma.gpd || !host->dma.bd) {
		ret = -ENOMEM;
		dev_info(host->dev, "host dma bd or gpd alloc failed\n");
		goto release_mem;
	}
...
skip_hwcq:
	...
	if (ret) {
		dev_info(host->dev, "request irq failed!\n");
		goto release;
	}
