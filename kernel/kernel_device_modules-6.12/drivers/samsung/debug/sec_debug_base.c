...
static struct pt_regs __percpu **sec_aarch64_core_reg;
static sec_debug_mmu_reg_t __percpu **sec_aarch64_mmu_reg;

static void secdbg_pcpureg_init(void)
{
	int i;
	struct reserved_mem *rmem;
	size_t vaddr;

	rmem = secdbg_log_get_rmem("samsung,secdbg-pcpureg");
	if (!rmem) {
		pr_info("%s: failed to get rmem for secdbg-pcpureg\n", __func__);
		return;
	}
	
	pr_notice("%s: 0x%llx - 0x%llx (0x%llx)\n",
		"secdbg-pcpureg", (unsigned long long)rmem->base,
		(unsigned long long)rmem->base + (unsigned long long)rmem->size,
		(unsigned long long)rmem->size);

	vaddr = (size_t)secdbg_log_rmem_set_vmap(rmem->base, rmem->size);
	pr_notice("%s: vmap addr = 0x%llx\n", "secdbg-pcpureg", (unsigned long long)vaddr);
	
  	sec_aarch64_mmu_reg = alloc_percpu(sec_debug_mmu_reg_t *);
  	sec_aarch64_core_reg = alloc_percpu(struct pt_regs *);
	
  	for_each_possible_cpu(i) {
  		*per_cpu_ptr(sec_aarch64_mmu_reg, i) = (sec_debug_mmu_reg_t *)(vaddr + i * SZ_512);
  		*per_cpu_ptr(sec_aarch64_core_reg, i) = (struct pt_regs *)(vaddr + i * SZ_512 +	sizeof(sec_debug_mmu_reg_t));
  	}	
}
...
void sec_debug_save_core_reg(void *v_regs)
{
	...
	struct pt_regs *core_reg =
			*per_cpu_ptr(sec_aarch64_core_reg, smp_processor_id());
...
{
	sec_debug_save_mmu_reg(*per_cpu_ptr(sec_aarch64_mmu_reg, raw_smp_processor_id()));
}
...
int sec_save_context(int cpu, void *v_regs)
{
	...
	if (cpu == _THIS_CPU) {
		sec_debug_save_mmu_reg(*per_cpu_ptr(sec_aarch64_mmu_reg, smp_processor_id()));
	} else {
		sec_debug_save_mmu_reg(*per_cpu_ptr(sec_aarch64_mmu_reg, cpu));
	}
...
{
	if (tsk->thread_node.next == &tsk->signal->thread_head)
		return NULL;

	return list_next_entry(tsk, thread_node);
}

#define secdbg_for_each_process(p) \
	for (p = &init_task ; (p = list_next_entry(p, tasks)) != &init_task ; )
...
static void sec_dump_task_info(void)
{
	...
	/* processes */
	secdbg_for_each_process(p) {
		struct task_struct *t;

		sec_dump_one_task_info(p,  true);

		/* thread */
		t = get_next_thread(p);
		while (t) {
			sec_dump_one_task_info(t,  false);
			t = get_next_thread(t);
		}
	}
	pr_info("--------------------------------------------------------"
...
#if IS_ENABLED(CONFIG_SEC_DEBUG_DUMP_SINK)
	if (sec_dump_sink_init())
		pr_emerg("%s: fail to init dump_sink\n", __func__);
#endif
