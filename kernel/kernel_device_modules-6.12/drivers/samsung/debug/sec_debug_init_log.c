...
static DEFINE_SPINLOCK(secdbg_init_log_lock);
/* 
	--------------------------------------------  sec-initlog
	-             INIT_LOG_SIZE 1M             -
	--------------------------------------------  sec-autocomment
...
void secdbg_hook_init_log(const char *str, size_t size)
{
	...
	spin_lock_irqsave(&secdbg_init_log_lock, flags);
	
	current_idx = buf_idx;
	
	if (current_idx + size > buf_size) {
		len = buf_size - current_idx;
		memcpy(buf_ptr + current_idx, str, len);
		memcpy(buf_ptr, str + len, size - len);
		buf_idx = size - len;
	} else {
		memcpy(buf_ptr + current_idx, str, size);
		buf_idx = (current_idx + size) % buf_size;
	}
	
	spin_unlock_irqrestore(&secdbg_init_log_lock, flags);
}
