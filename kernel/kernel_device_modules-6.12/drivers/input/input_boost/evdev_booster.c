...
static void sec_input_boost_get_events(struct input_handle *handle,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0))
	sec_input_boost_get_events(handle, vals, 1);
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0)) /* version 6.12.18 (Miracle) */
					...
					int *cur_idx, const struct input_value *vals, int evt_cnt)
{
	...
	if (evt_cnt > MAX_EVENTS) {
		(*cur_idx)++;
		dev_warn_ratelimited(evbst_dev, "Exceed max event number : %d\n", *cur_idx);
		return ret_val;
	}
