...
static struct attribute *conn_gadget_function_attrs[] = {
	&dev_attr_usb_buffer_size.attr,
	&dev_attr_out_max_packet_size.attr,
	&dev_attr_in_max_packet_size.attr,
	&dev_attr_bInterfaceProtocol.attr,
	NULL
};

ATTRIBUTE_GROUPS(conn_gadget_function);
#ifdef CONFIG_ANDROID_USB_CONFIGFS_UEVENT
extern struct device *android_create_function_device(char *name, void *drvdata,
...
static int conn_gadget_setup(struct conn_gadget_instance *fi_conn_gadget)
{
...
#if IS_ENABLED(CONFIG_F_CONN_GADGET_DEBUGFS)
#ifdef CONFIG_ANDROID_USB_CONFIGFS_UEVENT
	android_dev = android_create_function_device("f_conn_gadget", NULL, conn_gadget_function_groups);
#else
	...
	dev_set_drvdata(android_dev, fi_conn_gadget);
#endif
...
struct usb_function_instance *alloc_inst_conn_gadget(void)
{
	...
	if (err) {
		kfree(fi_conn);
		pr_err("usb: Error setting conn gadget\n");
		return ERR_PTR(err);
	}
