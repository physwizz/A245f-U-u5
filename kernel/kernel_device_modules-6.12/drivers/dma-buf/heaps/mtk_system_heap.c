...
static atomic64_t *get_total_bytes_ptr(struct dma_heap *heap)
{
	...
	else if (!strcmp(dma_heap_get_name(heap), "system-uncached"))
		return &sys_uncached_heap_total_bytes;
	else if (!strcmp(dma_heap_get_name(heap), "mtk_mm"))
		return &mtk_mm_heap_total_bytes;
	else if (!strcmp(dma_heap_get_name(heap), "mtk_mm-uncached"))
