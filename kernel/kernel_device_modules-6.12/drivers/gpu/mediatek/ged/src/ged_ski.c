...
ssize_t gpu_busy_orig_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
...
static KOBJ_ATTR_RO(gpu_busy_orig);

ssize_t gpu_busy_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	unsigned int gpu_loading_avg = 0;

	mtk_get_gpu_loading_avg(&gpu_loading_avg);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	table_num = ged_get_opp_num_real();
...
{
	int temperature = 0;
#if IS_ENABLED(CONFIG_MTK_GPU_MT6789_SUPPORT)
...
GED_ERROR ged_ski_init(void)
{
	...
	ret = ged_sysfs_create_file(g_gpu_kobj, &kobj_attr_gpu_busy_orig);
	if (ret) {
		ret = GED_ERROR_OOM;
		GED_LOGE("SKI: failed to create gpu_busy_orig!\n");
		goto EXIT;
	}
...
void ged_ski_exit(void)
{
	...
	ged_sysfs_remove_file(g_gpu_kobj, &kobj_attr_gpu_busy_orig);
