...
#define DISP_REG_CONFIG_OVLSYS_BYPASS_MUX_SHADOW_MT6993 0x400
...
static inline void mtk_drm_crtc_cmdq_config_reg(
	int value, resource_size_t config_regs_pa, unsigned int addr,
	bool reset_flag, struct cmdq_pkt *cmdq_handle, struct mtk_drm_crtc *mtk_crtc)
{
...
static inline void mtk_drm_crtc_cmdq_config_mutex(unsigned int comp_id,
	bool reset_flag, struct cmdq_pkt *cmdq_handle, struct mtk_drm_crtc *mtk_crtc)
{
	if (!reset_flag) {
		mtk_disp_mutex_add_comp_with_cmdq(mtk_crtc, comp_id,
		...
		mtk_disp_mutex_remove_comp_with_cmdq(mtk_crtc, comp_id,
				cmdq_handle, 0);
	}
}
	...
	struct mtk_plane_state *plane_state)
{
	if (mtk_crtc->path_data->is_exdma_dual_layer)
		if (plane_state->comp_state.dual_comp_id &&
			plane_state->comp_state.dual_blender_comp_id)
			return true;
		else
			return false;
	else
...
static void mtk_drm_crtc_init_bind_comp(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for_each_comp_in_crtc_path_bound(comp, mtk_crtc, i, j, 1) {
		...
		if (next_comp &&
			...
			if (drm_crtc_index(&mtk_crtc->base) == 1 && priv && priv->data->mmsys_id == MMSYS_MT6993) {
				mtk_crtc->first_blender = priv->ddp_comp[DDP_COMPONENT_OVL2_BLENDER0];
				if (comp->id == DDP_COMPONENT_OVL2_EXDMA2)
	...
	struct cmdq_pkt *cmdq_handle, bool reset_flag, bool rpo_flag)
{
	...
	if (comp->id == DDP_COMPONENT_OVL1_EXDMA2)
		sys_idx = 1;
	else if (comp->id == DDP_COMPONENT_OVL2_EXDMA2)
		sys_idx = 2;


	if (priv->data->mmsys_id == MMSYS_MT6991)
		comp_id = (rpo_flag) ? DDP_COMPONENT_OVL0_MDP_RSZ0 : DDP_COMPONENT_OVL0_EXDMA_OUT_CB3;
	else if (priv->data->mmsys_id == MMSYS_MT6993) {
		if (rpo_flag) {
			if (sys_idx == 1)
				comp_id = DDP_COMPONENT_OVL1_MDP_RSZ0;
			else
				comp_id = DDP_COMPONENT_OVL0_MDP_RSZ0;
		}
		else {
			if (crtc_id == 0) {
				if (sys_idx == 1)
					comp_id = DDP_COMPONENT_OVL1_EXDMA_OUT_CB3;
				else
					comp_id = DDP_COMPONENT_OVL0_EXDMA_OUT_CB3;
			} else if (crtc_id == 1)
				comp_id = DDP_COMPONENT_OVL2_EXDMA_OUT_CB3;
		}
	}

	value = priv->ovlsys_data->exdma_mout(comp->id, comp_id, &addr);

	if (comp->id < DDP_COMPONENT_OVL1_EXDMA0)
		config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
	else if (comp->id < DDP_COMPONENT_OVL2_EXDMA0)
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		...
		value_rsz = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL0_RSZ_IN_CB1, next_blender,
			&addr_rsz);
	} else if (priv->data->mmsys_id == MMSYS_MT6993) {
		if (crtc_id == 0) {
			if (sys_idx == 1) {
				value = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL1_MDP_RSZ0,
					next_blender, &addr);
				value_rsz = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL1_RSZ_IN_CB2,
					next_blender, &addr_rsz);

			} else {
				value = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL0_MDP_RSZ0,
					next_blender, &addr);
				value_rsz = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL0_RSZ_IN_CB2,
					next_blender, &addr_rsz);
			}
		} else if (crtc_id == 1) {
			value_rsz = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVL2_EXDMA_OUT_CB3,
				next_blender, &addr_rsz);
		}
	}
	...
	if (crtc_id == 0) {
		...
		if (value_rsz >= 0 && config_regs_pa > 0) {
			if (reset_flag) {
				cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
					config_regs_pa + addr_rsz, 0, ~0);
			} else {
...
void mtk_drm_crtc_async_connect(struct mtk_drm_crtc *mtk_crtc,
	unsigned int cur_id, unsigned int next_id,
	struct cmdq_pkt *cmdq_handle, bool reset_flag)
{
	unsigned int addr = 0;
	struct mtk_drm_private *priv = mtk_crtc->base.dev->dev_private;
	...
	switch (reg_data->dispsys_map[cur_id]) {
	case OVLSYS0:
		config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
		break;
	case OVLSYS1:
	...
	case OVLSYS2:
		...
		break;
	default:
		DDPMSG("%s mtk drm not support mmsys id %d\n",
			__func__, reg_data->dispsys_map[cur_id]);
		break;
	}

	value = priv->ovlsys_data->exdma_mout(cur_id, next_id, &addr);

	mtk_drm_crtc_cmdq_config_reg(value, config_regs_pa, addr,
						reset_flag, cmdq_handle, mtk_crtc);
	mtk_drm_crtc_cmdq_config_mutex(cur_id,
						reset_flag, cmdq_handle, mtk_crtc);
	mtk_drm_crtc_cmdq_config_mutex(next_id,
						reset_flag, cmdq_handle, mtk_crtc);
}
	...
	struct cmdq_pkt *cmdq_handle, bool reset_flag, bool rpo_flag)
{
	...
	/* cross sys control */
	if (reg_data->dispsys_map && (reg_data->dispsys_map[comp->id] !=
		reg_data->dispsys_map[blender_id])) {
		if (priv->data->mmsys_id == MMSYS_MT6993) {
			if (reg_data->dispsys_map[comp->id] == OVLSYS0) {
				if (reg_data->dispsys_map[blender_id] == OVLSYS1) {
					DDPINFO("ovlsys 0 > 1\n");

					next_blender = DDP_COMPONENT_OVLSYS_DLO_ASYNC1;
					mtk_drm_crtc_cmdq_config_mutex(DDP_COMPONENT_OVLSYS_DLO_ASYNC1,
						reset_flag, cmdq_handle, mtk_crtc);

					mtk_drm_crtc_async_connect(mtk_crtc, DDP_COMPONENT_OVLSYS1_DLI_ASYNC2,
						blender_id, cmdq_handle, reset_flag);
				}
			} else if (reg_data->dispsys_map[comp->id] == OVLSYS1) {
				if (reg_data->dispsys_map[blender_id] == OVLSYS0) {
					DDPINFO("ovlsys 1 > 0\n");

					next_blender = DDP_COMPONENT_OVLSYS1_DLO_ASYNC1;
					mtk_drm_crtc_cmdq_config_mutex(DDP_COMPONENT_OVLSYS1_DLO_ASYNC1,
						reset_flag, cmdq_handle, mtk_crtc);

					mtk_drm_crtc_async_connect(mtk_crtc, DDP_COMPONENT_OVLSYS2_DLI_ASYNC2,
						DDP_COMPONENT_OVLSYS2_DLO_ASYNC1, cmdq_handle, reset_flag);
					mtk_drm_crtc_async_connect(mtk_crtc, DDP_COMPONENT_OVLSYS_DLI_ASYNC2,
						blender_id, cmdq_handle, reset_flag);
				}
			}
		}
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		if (comp->id == DDP_COMPONENT_OVL_EXDMA2 || comp->id == DDP_COMPONENT_OVL1_EXDMA2) {
			...
			value = priv->ovlsys_data->exdma_mout(comp->id, next_blender, &addr);
			if (comp->id < DDP_COMPONENT_OVL1_EXDMA0)
				config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
			else
				config_regs_pa = mtk_crtc->ovlsys1_regs_pa;
		...
		if (comp->id == DDP_COMPONENT_OVL_EXDMA2 || comp->id == DDP_COMPONENT_OVL1_EXDMA2 ||
			comp->id == DDP_COMPONENT_OVL2_EXDMA2) {
			mtk_drm_crtc_rsz_exdma_ovl_path(mtk_crtc, comp, next_blender,
					cmdq_handle, reset_flag, rpo_flag);
			goto check_reset;
		} else {
			value = priv->ovlsys_data->exdma_mout(comp->id, next_blender, &addr);
	...
	struct mtk_ddp_comp *comp, struct cmdq_pkt *cmdq_handle, bool reset_flag)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		...
		if (comp->id < DDP_COMPONENT_OVL1_BLENDER0)
			config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
		else if (comp->id < DDP_COMPONENT_OVL2_BLENDER0)
			config_regs_pa = mtk_crtc->ovlsys1_regs_pa;
		else
			config_regs_pa = mtk_crtc->ovlsys2_regs_pa;
	...
	if (reg_data->dispsys_map &&
		reg_data->dispsys_map[comp->id] != reg_data->dispsys_map[out_proc] &&
		mtk_crtc->last_blender->id == comp->id) {
		if (priv->data->mmsys_id == MMSYS_MT6993) {
			DDPINFO("blender ovlsys 1 > 0\n");
			mtk_drm_crtc_cmdq_config_mutex(DDP_COMPONENT_OVLSYS1_DLO_ASYNC8,
				reset_flag, cmdq_handle, mtk_crtc);

		value = priv->ovlsys_data->exdma_mout(comp->id,
			DDP_COMPONENT_OVLSYS1_DLO_ASYNC8, &addr);
		config_regs_pa = mtk_crtc->ovlsys1_regs_pa;

		mtk_drm_crtc_cmdq_config_reg(value, config_regs_pa, addr,
			reset_flag, cmdq_handle, mtk_crtc);
		mtk_drm_crtc_cmdq_config_mutex(DDP_COMPONENT_OVLSYS_DLI_ASYNC9,
			reset_flag, cmdq_handle, mtk_crtc);
		value = priv->ovlsys_data->exdma_mout(DDP_COMPONENT_OVLSYS_DLI_ASYNC9, out_proc, &addr);
		config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
		}
	} else
		value = priv->ovlsys_data->exdma_mout(comp->id, out_proc, &addr);
...
void mtk_drm_crtc_relay_blender_connect(struct mtk_drm_crtc *mtk_crtc,
	struct cmdq_pkt *cmdq_handle, bool reset_flag)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6993 &&
		mtk_crtc->path_data->is_exdma_dual_layer && mtk_crtc->bypass_bld_id) {
		if (mtk_crtc->bypass_bld_id < mtk_crtc->last_blender->id) {
			for (i = mtk_crtc->bypass_bld_id + 1; i < mtk_crtc->last_blender->id; i++){
				if (i == DDP_COMPONENT_OVL0_BLENDER8 || i == DDP_COMPONENT_OVL0_BLENDER9)
					continue;
				r_comp = priv->ddp_comp[i];
				if (r_comp && r_comp->funcs && r_comp->funcs->connect)
					r_comp->funcs->connect(r_comp, cmdq_handle, r_comp->id, r_comp->id);

				DDPINFO("%s, comp:%s\n", __func__, mtk_dump_comp_str_id(i));

				if (!reset_flag)
					mtk_disp_mutex_add_comp_with_cmdq(mtk_crtc, r_comp->id,
						mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base), cmdq_handle, 0);
				else {
					int keep_layer = false;

					mtk_disp_mutex_remove_comp_with_cmdq(mtk_crtc, r_comp->id,
						cmdq_handle, 0);
...
void mtk_drm_crtc_bypass_blender(struct mtk_ddp_comp *comp, struct mtk_drm_crtc *mtk_crtc)
{
	struct mtk_drm_private *priv = mtk_crtc->base.dev->dev_private;

	if (mtk_crtc->pre_bld_id != 0 && mtk_crtc->bypass_bld_id == 0 &&
		comp->id - mtk_crtc->pre_bld_id > 1) {
		if (priv->data->mmsys_id == MMSYS_MT6993) {
			if (mtk_crtc->pre_bld_id != DDP_COMPONENT_OVL0_BLENDER7)
				mtk_crtc->bypass_bld_id = mtk_crtc->pre_bld_id;
		}
	}
	mtk_crtc->pre_bld_id = comp->id;

	if (mtk_crtc->bypass_bld_id)
		DDPINFO("%s, bypass comp:%s\n", __func__,
			mtk_dump_comp_str_id(mtk_crtc->bypass_bld_id));
}
...
retrigger_blank_connect:
	...
	mtk_drm_crtc_bypass_blender(comp, mtk_crtc);

	//right pipe
	if (plane_state && plane_state->comp_state.blender_comp_id != 0) {
		if (mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
			r_comp = priv->ddp_comp[plane_state->comp_state.dual_blender_comp_id];
			if (r_comp && r_comp->funcs && r_comp->funcs->connect) {
			...
			mtk_drm_crtc_bypass_blender(r_comp, mtk_crtc);
		}
	}
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		...
		if (crtc_id == 0) {
			...
			out_proc = DDP_COMPONENT_OVL2_OUTPROC0;
			out_comp = DDP_COMPONENT_OVLSYS2_DLO_ASYNC12;
			first_blender = DDP_COMPONENT_OVL2_BLENDER0;
			/* dp single exdma: if dual exdma or exdma2 path occurs problems */
			// first_blender = DDP_COMPONENT_OVL2_BLENDER2;
		} else if  (crtc_id == 2) {
			out_proc = DDP_COMPONENT_OVL1_OUTPROC3;
			out_comp = DDP_COMPONENT_OVLSYS_WDMA2;
		} else if  (crtc_id == 3) {
	...
	value = priv->ovlsys_data->exdma_mout(last_blender->id, out_proc, &addr);
...
/*
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		value = priv->ovlsys_data->exdma_mout(out_proc, out_comp, &addr);
		if (last_blender->id < DDP_COMPONENT_OVL1_BLENDER0)
			config_regs_pa = mtk_crtc->ovlsys0_regs_pa;
		else
			config_regs_pa = mtk_crtc->ovlsys1_regs_pa;
...
void mtk_drm_crtc_exdma_path_setting_reset_without_cmdq(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (crtc_id == 0) {
		...
		if (priv->data->mmsys_id == MMSYS_MT6991){
			...
			mask = 0x783C0;	//exdma6-9, blender5-8
		} else {
			if (ddp->ovlsys2_regs)
				ovl_mutex_regs = ddp->ovlsys2_regs + offset;
			mask = 0xF3C ; //exdma2-5, blender0-3
		}
	...
	priv->ovlsys_data->exdma_mout_reset(MTK_OVL_EXDMA, &offset, &addr_begin,
		&addr_end, crtc_id);
	...
	if (crtc_id == 0) {
		priv->ovlsys_data->exdma_mout_reset(MTK_DISP_MDP_RSZ, &offset, &addr_begin,
			&addr_end, crtc_id);
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	if (crtc_id == 0) {
		...
		if (priv->data->mmsys_id == MMSYS_MT6991) {
			...
			if (ddp->ovlsys2_regs)
				ovl1_mutex_regs_pa = ddp->ovlsys2_regs_pa;
			mask = 0x3c;
		}
	...
	for (i = addr_begin; i <= addr_end; i = i + offset) {
		if (crtc_id == 0) {
			...
			if (priv->data->mmsys_id == MMSYS_MT6991) {
				...
				cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
							mtk_crtc->ovlsys2_regs_pa + i, 0, ~0);
			}
		} else if (crtc_id == 2) {
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
						mtk_crtc->ovlsys1_regs_pa + i, 0, ~0);
...
void mtk_drm_crtc_mini_dump(struct drm_crtc *crtc)
{
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		if (!priv->pwr_node || (priv->pwr_node && !in_interrupt())) {
	...
	case MMSYS_MT6993:
			...
			if (mtk_crtc->ovlsys2_regs) {
				...
				priv->ovlsys_data->config_dump_reg(mtk_crtc->ovlsys2_regs);
			}


			DDPDUMP("== DISP pipe-0 MMSYS_CONFIG REGS:0x%pa ==\n",
						&mtk_crtc->config_regs_pa);
			priv->dispsys_data->config_dump_reg(mtk_crtc->config_regs);
			if (mtk_crtc->side_config_regs) {
				DDPDUMP("== DISP pipe-1 MMSYS_CONFIG REGS:0x%pa ==\n",
					&mtk_crtc->side_config_regs_pa);
				priv->dispsys_data->config_dump_reg(mtk_crtc->side_config_regs);
			}
			if (mtk_crtc->sys_b_config_regs) {
				DDPDUMP("== DISP pipe-2 MMSYS_CONFIG REGS:0x%pa ==\n",
					&mtk_crtc->sys_b_config_regs_pa);
				priv->dispsys_data->config_dump_reg(mtk_crtc->sys_b_config_regs);
			}
...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
		...
		priv->dispsys_data->mutex_dump_reg(mtk_crtc->mutex[0]);
		priv->ovlsys_data->mutex_dump_reg(mtk_crtc->mutex[0]);
		break;
	case MMSYS_MT6993:
...
void mtk_drm_crtc_mini_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6993:
			DDPDUMP("== DUMP OVLSYS pipe-0 ANALYSIS:0x%pa ==\n",
				&mtk_crtc->ovlsys0_regs_pa);
			priv->ovlsys_data->config_dump_analysis(mtk_crtc->ovlsys0_regs, false);
			if (mtk_crtc->ovlsys1_regs) {
				DDPDUMP("== DUMP OVLSYS pipe-1 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->ovlsys1_regs_pa);
				priv->ovlsys_data->config_dump_analysis(mtk_crtc->ovlsys1_regs, false);
			}
			if (mtk_crtc->ovlsys2_regs) {
				DDPDUMP("== DUMP OVLSYS pipe-2 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->ovlsys2_regs_pa);
				priv->ovlsys_data->config_dump_analysis(mtk_crtc->ovlsys2_regs, false);
			}

			DDPDUMP("== DUMP DISP pipe-0 ANALYSIS:0x%pa ==\n",
				&mtk_crtc->config_regs_pa);
			priv->dispsys_data->config_dump_analysis(mtk_crtc->config_regs, 0);
			if (mtk_crtc->side_config_regs) {
				DDPDUMP("== DUMP DISP pipe-1 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->side_config_regs_pa);
				priv->dispsys_data->config_dump_analysis(mtk_crtc->side_config_regs, 1);
			}
			if (mtk_crtc->sys_b_config_regs) {
				DDPDUMP("== DUMP DISP pipe-2 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->sys_b_config_regs_pa);
				//mmsys_config_dump_analysis_mt6993(mtk_crtc->sys_b_config_regs, 0);
				priv->dispsys_data->config_dump_analysis(mtk_crtc->sys_b_config_regs, 0);
			}
			if (mtk_crtc->sys_b_side_config_regs) {
				DDPDUMP("== DUMP DISP pipe-3 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->sys_b_side_config_regs_pa);
				priv->dispsys_data->config_dump_analysis(mtk_crtc->sys_b_side_config_regs, 1);
			}
			for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
...
void mtk_drm_crtc_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
		...
		priv->dispsys_data->mutex_dump_analysis(mtk_crtc->mutex[0]);
		priv->ovlsys_data->mutex_dump_analysis(mtk_crtc->mutex[0]);
		if (mtk_crtc->is_dual_pipe) {
			DDPDUMP("anlysis dual pipe\n");
			for_each_comp_in_dual_pipe(comp, mtk_crtc, i, j) {
				mtk_dump_analysis(comp);
				       ...
				       struct drm_crtc_state *state)
{
	...
	DDPINFO("destroy/free crtc_state, 0x%llx, crtc->state=0x%llx\n",
				(u64)state, (u64)crtc->state);
	kfree(s);
}
...
static void mtk_crtc_update_ovl_hrt_usage(struct drm_crtc *crtc)
{
	...
	drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
		...
		if (mtk_crtc && mtk_crtc->path_data &&
			mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state) && priv) {
			//comp_idx = dual_pipe_comp_mapping(priv->data->mmsys_id, comp->id);
			comp_idx = plane_state->comp_state.dual_comp_id;
...
static unsigned int dual_comp_map_mt6993(unsigned int comp_id)
{
	unsigned int ret = 0;

	switch (comp_id) {
	case DDP_COMPONENT_OVL2_EXDMA2:
		ret = DDP_COMPONENT_OVL2_EXDMA3;
		break;
	case DDP_COMPONENT_OVL2_EXDMA4:
		ret = DDP_COMPONENT_OVL2_EXDMA5;
		break;
	case DDP_COMPONENT_OVL2_BLENDER0:
		ret = DDP_COMPONENT_OVL2_BLENDER1;
		break;
	case DDP_COMPONENT_OVL2_BLENDER2:
		ret = DDP_COMPONENT_OVL2_BLENDER3;
		break;
...
unsigned int dual_pipe_comp_mapping(unsigned int mmsys_id, unsigned int comp_id)
{
	...
	case MMSYS_MT6991:
		...
		break;
	case MMSYS_MT6993:
		ret = dual_comp_map_mt6993(comp_id);
		break;
	default:
					  ...
					  struct cmdq_pkt *cmdq_handle)
{
	...
	for (i = mtk_crtc->layer_nr - 1; i >= 0; i--) {
		...
		struct mtk_plane_state *plane_state;
		struct mtk_ddp_comp *comp = NULL;
		struct mtk_ddp_comp *d_comp = NULL;
		...
		if (plane_state->base.visible &&
			...
			for_each_comp_in_cur_crtc_path(
				...
				if (mtk_crtc->is_dual_pipe || mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
					struct mtk_drm_private *priv =
						mtk_crtc->base.dev->dev_private;
					unsigned int index = 0;

					if (mtk_crtc->path_data->is_exdma_dual_layer) {
						index = plane_state->comp_state.dual_comp_id;
					} else
						index = dual_pipe_comp_mapping
							(priv->data->mmsys_id, comp_state->comp_id);

					if (index < DDP_COMPONENT_ID_MAX) {
						d_comp = priv->ddp_comp[index];
						mtk_ddp_comp_layer_off(
							d_comp,
							comp_state->lye_id,
							comp_state->ext_lye_id,
	...
	for (i = mtk_crtc->layer_nr - 1; i >= 0; i--) {
		...
		if (plane_state->comp_state.blender_comp_id != 0) {
			...
			if (plane_state->comp_state.ext_lye_id == 0) {
				...
				if (mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
					struct mtk_ddp_comp *d_comp = NULL;
					struct mtk_ddp_comp *d_blender_comp = NULL;

					d_comp = priv->ddp_comp[plane_state->comp_state.dual_comp_id];
					d_blender_comp = priv->ddp_comp[plane_state->comp_state.dual_blender_comp_id];
					d_comp->bind_comp = d_blender_comp;
				}
			}
				...
				if (plane_state->comp_state.ext_lye_id == 0 ||
					...
					if (mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
						mtk_crtc->last_blender =
							priv->ddp_comp[plane_state->comp_state.dual_blender_comp_id];
					}
...
bool mtk_crtc_with_event_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		/* Need use event loop to wait VACT start to change threshold */
			...
			struct mtk_ddp_comp *comp)
{
	...
	}  else if (comp->id == DDP_COMPONENT_DISP_DVO || comp->id == DDP_COMPONENT_SYS_B_DISP_DVO) {
		return mtk_crtc->gce_obj.event[EVENT_VDO_EOF];
...
#endif
{
	...
	DDPINFO("crtc_state:0x%llx, atomic_state:%lu, crtc:%lu, pf:%u, crtc->state:0x%llx\n",
		(u64)crtc_state, (unsigned long)atomic_state,
		(unsigned long)crtc, cb_data->pres_fence_idx, (u64)crtc->state);

	session_id = mtk_get_session_id(crtc);

	// we only support frame submit on primary display, because other
	// crtc may release to other drm user (ex: drmwrapper) which not
	// support frame submit flow.
	use_frame_submit =
	...
	if (use_frame_submit) {
		/* config fence and present fence use different timeline,
		 * and each frame always has these two fences,
		 * so their fence index will be the same.
		 * But when cmdq timeout happened, fence index from
		 * backup_slot(N-1) and cb_data(N) are different.
		 * So if fence index is used for config fence,
		 * we need get it from backup slot to avoid tearing or TF,
		...
		struct mtk_plane_state *plane_state, struct cmdq_pkt *cmdq_handle)
{
	...
	if (mtk_crtc->path_data->is_exdma_dual_layer)
		comp_index = plane_state->comp_state.dual_comp_id;
	else
...
void __mtk_crtc_restore_plane_setting(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	if (priv->data->ovl_exdma_rule)
		mtk_drm_crtc_relay_blender_connect(mtk_crtc, cmdq_handle, false);
}
...
static void _mtk_crtc_1tnp_setting(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6989) {
		...
		base_addr = mtk_crtc->sys_b_config_regs + MT6993_DISPSYS_DLI_RELAY_1TNP;
		for (i = 0; i < MT6993_DISPSYS_DLI_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
		...
		base_addr = mtk_crtc->sys_b_config_regs + MT6993_DISPSYS_DLO_RELAY_1TNP;
		for (i = 0; i < MT6993_DISPSYS_DLO_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
		...
		base_addr = mtk_crtc->sys_b_side_config_regs + MT6993_DISPSYS1_DLI_RELAY_1TNP;
		for (i = 0; i < MT6993_DISPSYS1_DLI_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6983 ||
		...
		if (mtk_crtc->sys_b_config_regs)
			writel(0x3, mtk_crtc->sys_b_config_regs +
					DISP_REG_CONFIG_MMSYS_GCE_EVENT_SEL_MT6993);

		if (mtk_crtc->sys_b_side_config_regs)
			writel(0x3, mtk_crtc->sys_b_side_config_regs +
					DISP_REG_CONFIG_MMSYS_GCE_EVENT_SEL_MT6993);
		...
		if (mtk_crtc->ovlsys2_regs)
			writel(0x3, mtk_crtc->ovlsys2_regs +
					DISP_REG_CONFIG_OVLSYS_GCE_EVENT_SEL_MT6993);
				...
				struct drm_atomic_state *atomic_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_SPHRT) && index < MAX_CRTC) {
		if (priv->usage[index] == DISP_ENABLE || priv->usage[index] == DISP_OPENING) {
			...
			CRTC_MMP_EVENT_END((int) index, resume, mtk_crtc->enabled, 1);
			return;
		}
...
void mtk_crtc_first_enable_ddp_config(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6985 ||
		...
		if (mtk_crtc->ovlsys2_regs) {
			...
			/*Set BYPASS_MUX_SHADOW*/
			writel(0x1, mtk_crtc->ovlsys2_regs +
					DISP_REG_CONFIG_OVLSYS_BYPASS_MUX_SHADOW_MT6993);
			writel(0x1fe0000, mtk_crtc->ovlsys2_regs +
					DISP_REG_CONFIG_OVLSYS_CB_BYPASS_MUX_SHADOW_MT6993);
		}
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	/* reset bypass blender */
	mtk_drm_crtc_relay_blender_connect(mtk_crtc, mtk_crtc_state->cmdq_handle, true);
	mtk_crtc->bypass_bld_id = 0;
	mtk_crtc->pre_bld_id = 0;
	...
	unsigned int w)
{
	...
	/*right path*/
	if (mtk_crtc->path_data->is_exdma_dual_layer) {
		plane_state_r->comp_state.comp_id = plane_state->comp_state.dual_comp_id;
		plane_state_r->comp_state.blender_comp_id = plane_state->comp_state.dual_blender_comp_id;
	} else {
	...
	if (plane_state->pending.enable) {
		...
		if (comp_state->comp_id) {
			if (mtk_crtc->is_dual_pipe || mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
				...
				if (mtk_crtc->path_data->is_exdma_dual_layer)
					comp_id = plane_state->comp_state.dual_comp_id;
				else
			...
			if (!state->crtc && comp) {
				if (mtk_crtc->is_dual_pipe || mtk_drm_crtc_check_dual_exdma(mtk_crtc, plane_state)) {
					...
					if (mtk_crtc->path_data->is_exdma_dual_layer)
						comp_r_id = plane_state->comp_state.dual_comp_id;
					else
		...
		/*if (priv->data->ovl_exdma_rule) {
			...
			value = priv->ovlsys_data->exdma_mout(comp_state->comp_id,
...
static int mtk_drm_pf_release_thread(void *data)
{
	...
	crtc_idx = drm_crtc_index(crtc);
	use_frame_submit =
		mtk_drm_helper_get_opt(private->helper_opt, MTK_DRM_OPT_FRAME_SUBMIT) &&
		crtc_idx == 0;
	sched_setscheduler(current, SCHED_RR, &param);
...
/* This frame done release thread only is aiming for frame trigger mode's CRTC */
static int mtk_drm_frame_done_release_thread(void *data)
{
	struct sched_param param = {.sched_priority = 87};
	struct mtk_drm_private *private;
	struct mtk_drm_crtc *mtk_crtc = (struct mtk_drm_crtc *)data;
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(mtk_crtc->frame_done_fence_wq,
				 atomic_read(&mtk_crtc->frame_done_event));
		atomic_set(&mtk_crtc->frame_done_event, 0);
...
#define CAPS_CHAR_SIZE 25
	...
	mtk_crtc->sys_b_config_regs = priv->sys_b_config_regs;
	mtk_crtc->sys_b_config_regs_pa = priv->sys_b_config_regs_pa;
	mtk_crtc->sys_b_side_config_regs = priv->sys_b_side_config_regs;
	mtk_crtc->sys_b_side_config_regs_pa = priv->sys_b_side_config_regs_pa;
	...
	if (priv->data->mmsys_id == MMSYS_MT6985 ||
		...
		mtk_crtc->ovlsys2_regs = priv->ovlsys2_regs;
		mtk_crtc->ovlsys2_regs_pa = priv->ovlsys2_regs_pa;
	}
	...
	init_waitqueue_head(&mtk_crtc->frame_done_fence_wq);
	atomic_set(&mtk_crtc->frame_done_event, 0);
	mtk_crtc->frame_done_release_thread =
		kthread_create(mtk_drm_frame_done_release_thread,
				mtk_crtc, "fd_release_thread");
	wake_up_process(mtk_crtc->frame_done_release_thread);
	...
	int crtc_index, int session_id, struct mtk_fence_info *config_layer_info)
{
	...
	int present_timeline_id = 0;
	struct mtk_fence_info *present_layer_info = NULL;
	unsigned int curr_present_fence_idx = 0;
	struct fence_data present_fence;

	int frame_done_timeline_id = 0;
	struct mtk_fence_info *frame_done_layer_info = NULL;
	unsigned int curr_frame_done_fence_idx = 0;
	...
	config_fence.value = curr_config_fence_idx + 1;
	atomic_inc(&priv->crtc_config[crtc_index]);
	if (mtk_sync_fence_create(config_layer_info->timeline, &config_fence)) {
		DDPPR_ERR("%s: create fence object for config_fence failed!"
			" (session_id:%d, config_timeline_id:%d)\n",
			__func__, MTK_SESSION_DEV(session_id), config_timeline_id);
		ret |= -EFAULT;
	}
	args->config_fence_fd = config_fence.fence;
	args->config_fence_idx = config_fence.value;
create_config_fence_done:
	...
	if (!present_layer_info) {
		DDPPR_ERR("%s: present_layer_info is null\n", __func__);
		return -EFAULT;
	}
	curr_present_fence_idx = atomic_read(&priv->crtc_present[crtc_index]);
	present_fence.fence = MTK_INVALID_FENCE_FD;
	present_fence.value = curr_present_fence_idx + 1;
	atomic_inc(&priv->crtc_present[crtc_index]);
	if (mtk_sync_fence_create(present_layer_info->timeline, &present_fence)) {
		DDPPR_ERR("%s: create fence object for present_fence failed!"
			" (session_id:%d, present_timeline_id:%d)\n",
			__func__, MTK_SESSION_DEV(session_id), present_timeline_id);
		ret |= -EFAULT;
	}
	args->present_fence_fd = present_fence.fence;
	args->present_fence_idx = present_fence.value;
create_present_fence_done:
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc) &&
		...
		if (!frame_done_layer_info) {
			DDPPR_ERR("%s: frame_done_layer_info is null\n", __func__);
			return -EFAULT;
		}
		curr_frame_done_fence_idx = atomic_read(&priv->crtc_frame_done[crtc_index]);
		frame_done_fence.fence = MTK_INVALID_FENCE_FD;
		frame_done_fence.value = curr_frame_done_fence_idx + 1;
		atomic_inc(&priv->crtc_frame_done[crtc_index]);
		if (mtk_sync_fence_create(frame_done_layer_info->timeline, &frame_done_fence)) {
			DDPPR_ERR("%s: create fence object for frame_done_fence failed!"
				" (session_id:%d, frame_done_timeline_id:%d)\n",
				__func__, MTK_SESSION_DEV(session_id), frame_done_timeline_id);
			ret |= -EFAULT;
		}
		args->frame_done_fence_fd = frame_done_fence.fence;
		args->frame_done_fence_idx = frame_done_fence.value;
create_frame_done_fence_done:
		...
		DDPFENCE("P+/%s%d/cfg,T%d,idx%d,fd%d/pf,T%d,idx%d,fd%d/fdone,T%d,idx%d,fd%d\n",
			mtk_fence_session_mode_spy(session_id), MTK_SESSION_DEV(session_id),
			config_timeline_id, args->config_fence_idx, args->config_fence_fd,
			present_timeline_id, args->present_fence_idx, args->present_fence_fd,
			frame_done_timeline_id, args->frame_done_fence_idx,
			args->frame_done_fence_fd);
	} else {
		/* At vdo mode or no support PU, we don't need to support frame done fence */
		args->frame_done_fence_fd = -1;
		args->frame_done_fence_idx = 0;

		DDPFENCE("P+/%s%d/cfg,T%d,idx%d,fd%d/pf,T%d,idx%d,fd%d\n",
			mtk_fence_session_mode_spy(session_id), MTK_SESSION_DEV(session_id),
			config_timeline_id, args->config_fence_idx, args->config_fence_fd,
			present_timeline_id, args->present_fence_idx, args->present_fence_fd);
	}
...
{
	struct mtk_frame_submit *args = (struct mtk_frame_submit *)data;
	struct mtk_union_fence *union_fence = &args->union_fence;
	unsigned int layer_num = args->layer_num;
	struct drm_mtk_gem_submit *gem_submits = NULL;
	...
	if (layer_num > 0) {
		gem_submits = kcalloc(layer_num, sizeof(struct drm_mtk_gem_submit), GFP_KERNEL);
		if (!gem_submits) {
			DDPPR_ERR("%s: drm_mtk_gem_submit allocate fail\n", __func__);
			return -ENOMEM;
		}
		if (copy_from_user(gem_submits, args->gem_submits,
			layer_num * sizeof(struct drm_mtk_gem_submit))) {
			DDPPR_ERR("%s: drm_mtk_gem_submit copy fail\n", __func__);
			kfree(gem_submits);
			return -EFAULT;
		}
	}
	...
	for (i = 0; i < layer_num; i++) {
		gem_submits[i].fence_fd = MTK_INVALID_FENCE_FD;
