...
#include "platform/mtk_ovl_mapping_table.h"
...
static int mtk_lye_get_exdma_comp_id(int disp_idx, int layer_idx,
	struct mtk_drm_private *priv, struct mtk_drm_crtc *mtk_crtc, int fun_lye, int rsz_lye)
{
	/* TODO: The component ID should be changed by ddp path and platforms */
	...
	if (disp_idx == 0) {
		...
		if (rsz_lye)
			first_comp = l_rule_info->comp_map[CRTC_0_1ST_EXDMA_W_RPO];
		else
			first_comp = l_rule_info->comp_map[CRTC_0_1ST_EXDMA_WO_RPO];

		if (mtk_crtc->path_data->is_exdma_dual_layer) {
			exdma_comp = first_comp + ((layer_idx - fun_lye) * 2);
			if (priv->data->mmsys_id == MMSYS_MT6993 &&
				exdma_comp > l_rule_info->comp_map[SYS_LAST_EXDMA])
				if (rsz_lye)
					exdma_comp += l_rule_info->comp_map[EXDMA_SYS_GAP_W_RPO];
				else
					exdma_comp += l_rule_info->comp_map[EXDMA_SYS_GAP_WO_RPO];
		} else
		...
		if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH)) {
			...
			exdma_comp = l_rule_info->comp_map[CRTC_1_1ST_EXDMA] + ((layer_idx - fun_lye) * 2);
		...
		if (priv->data->mmsys_id == MMSYS_MT6991) {
			...
			exdma_comp = l_rule_info->comp_map[CRTC_2_1ST_EXDMA] + ((layer_idx - fun_lye) * 2);
		...
		if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH)) {
			...
			return (l_rule_info->comp_map[CRTC_3_1ST_EXDMA] + layer_idx - fun_lye);
		else
			return DDP_COMPONENT_OVL2_2L;
	}
...
static int mtk_lye_get_blender_comp_id(int disp_idx, int layer_idx,
	struct mtk_drm_private *priv, struct mtk_drm_crtc *mtk_crtc,
	int blender_lye, int dual_blender, int exdma_comp_id, int *reply_bld_lye,
	int rsz_lye)
{
	/* TODO: The component ID should be changed by ddp path and platforms */
	if (disp_idx == 0) {
		...
		first_bld = l_rule_info->comp_map[CRTC_0_1ST_BLENDER];
		if (priv->data->mmsys_id == MMSYS_MT6991)
			bld_comp = first_bld + layer_idx - blender_lye + dual_blender;
		else if (priv->data->mmsys_id == MMSYS_MT6993) {
			bld_comp = first_bld + layer_idx - blender_lye + dual_blender + *reply_bld_lye;
			if (bld_comp > l_rule_info->comp_map[SYS_LAST_BLENDER])
				bld_comp += l_rule_info->comp_map[BLENDER_SYS_GAP];

			if (exdma_comp_id > l_rule_info->comp_map[SYS_LAST_EXDMA] &&
				bld_comp <= l_rule_info->comp_map[SYS_LAST_BLENDER]) {
				if (exdma_comp_id == l_rule_info->comp_map[RPO_1_EXDMA] && rsz_lye)
					return bld_comp;
				else {
					bld_comp += (l_rule_info->comp_map[CRTC_0_SYS1_1ST_BLENDER] - bld_comp);
					*reply_bld_lye += (l_rule_info->comp_map[CRTC_0_SYS1_1ST_BLENDER] - bld_comp);
				}
			}
		}
		...
		if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH)) {
			...
			bld_comp = l_rule_info->comp_map[CRTC_1_1ST_BLENDER] + ((layer_idx - blender_lye) * 2);
		...
		if (priv->data->mmsys_id == MMSYS_MT6991) {
			...
			bld_comp = l_rule_info->comp_map[CRTC_2_1ST_BLENDER] + ((layer_idx - blender_lye) * 2);
		...
		if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH)) {
			...
			return (l_rule_info->comp_map[CRTC_3_1ST_BLENDER] + layer_idx - blender_lye);
		else
			return DDP_COMPONENT_OVL2_2L;
	}
...
static int mtk_lye_get_dual_exdma_id(int comp_id, struct mtk_drm_crtc *mtk_crtc,
				struct mtk_drm_private *priv, int rsz_lye, int dual_layer)
{
	int ret_comp = 0;
	...
	if (mtk_crtc->path_data->is_exdma_dual_layer) {
		if (priv->data->mmsys_id == MMSYS_MT6991) {
			...
			if (comp_id == l_rule_info->comp_map[SYS_LAST_EXDMA]) {
				if (rsz_lye)
					ret_comp = l_rule_info->comp_map[CRTC_0_SYS1_1ST_EXDMA_W_RPO];
				else
					ret_comp = l_rule_info->comp_map[CRTC_0_SYS1_1ST_EXDMA_WO_RPO];
			} else
				ret_comp = comp_id + 1;
			/* full dual pipe
			 * if (rsz_lye)
			 *	dual_sys_gap = DDP_COMPONENT_OVL1_EXDMA3 - DDP_COMPONENT_OVL_EXDMA8;
			 * else
			 *	dual_sys_gap = DDP_COMPONENT_OVL1_EXDMA2 - DDP_COMPONENT_OVL_EXDMA8;
			 *
			 * if (rsz_lye && comp_id == DDP_COMPONENT_OVL_EXDMA2)
			 *	return DDP_COMPONENT_OVL1_EXDMA2;
			 * else if (comp_id + dual_layer >= DDP_COMPONENT_OVL_EXDMA8)
			 *	return comp_id + dual_layer + dual_sys_gap;
			 * else
			 *	return comp_id + dual_layer;
			 */
		}
		return ret_comp;
	}
...
static int mtk_lye_get_dual_blender_id(int comp_id, struct mtk_drm_crtc *mtk_crtc,
				struct mtk_drm_private *priv, int *reply_bld_lye,
				uint32_t exdma_comp_id, int rsz_lye)
{
	int ret_comp = 0;
	...
	if (mtk_crtc->path_data->is_exdma_dual_layer) {
		if (priv->data->mmsys_id == MMSYS_MT6991) {
			...
			if (comp_id == l_rule_info->comp_map[SYS_LAST_BLENDER])
				ret_comp = l_rule_info->comp_map[CRTC_0_SYS1_1ST_BLENDER];
			else
				ret_comp = comp_id + 1;

			if (exdma_comp_id > l_rule_info->comp_map[SYS_LAST_EXDMA] &&
				ret_comp <= l_rule_info->comp_map[SYS_LAST_BLENDER]) {
				if (exdma_comp_id == l_rule_info->comp_map[RPO_1_EXDMA] && rsz_lye)
					return ret_comp;
				ret_comp += (l_rule_info->comp_map[CRTC_0_SYS1_1ST_BLENDER] - ret_comp);
				*reply_bld_lye += (l_rule_info->comp_map[CRTC_0_SYS1_1ST_BLENDER] - ret_comp);
			}
		}
		return ret_comp;
	}
				  ...
				  struct drm_device *drm_dev, struct mtk_lye_ddp_state *lye_state)
{
	...
	struct mtk_drm_private *priv = drm_dev->dev_private;
	int fun_lye = 0, rsz_lye = 0, mml_lye = 0, total_phy_lyr = 0, dual_con_lyr = 0;
	enum FUN_LYE_TYPE lye_type;
	int blender_lye = 0, dual_blender = 0;
	int mml_decouple2 = 0;
	int reply_bld_lye = 0;
	...
	/* dual pipe node
	 * total_phy_lyr = total phy layer
	 * dual_con_lyr = dual pipe mapping layer count
	 */
	...
	for (i = 0; i < disp_info->layer_num[idx]; i++) {
		...
		if (clear_idx < 0) {
			...
			}  else if (mtk_has_layer_cap(layer_info, MTK_MML_DISP_DIRECT_DECOUPLE_LAYER)
				||mtk_has_layer_cap(layer_info, MTK_MML_DISP_DIRECT_LINK_LAYER)) {
				mml_lye++;
			}
		}
		if (mtk_crtc->path_data->is_exdma_dual_layer) {
	...
	/*TODO: RPO Layer will need to dual */
	if (mtk_crtc->path_data->is_exdma_dual_layer) {
		if (clear_idx > 0) /* Add one layer for clear layer */
			total_phy_lyr++;
		dual_con_lyr = total_phy_lyr - rsz_lye - mml_lye;
	}
	...
	DDPINFO("layer_num = %d total_phy_lyr %d dual_con_lyr %d\n",
			disp_info->layer_num[idx], total_phy_lyr, dual_con_lyr);
	for (i = 0; i < disp_info->layer_num[idx]; i++) {

		layer_info = &disp_info->input_config[idx][i];
		lye_type = LYE_NOR;
		comp_state.dual_comp_id = 0;
		comp_state.dual_blender_comp_id = 0;
		...
		if (l_rule_info->ovl_exdma_rule) {
			...
			if (mtk_has_layer_cap(layer_info, MTK_MML_DISP_DIRECT_DECOUPLE_LAYER)) {
				...
				if (mtk_crtc->path_data->is_exdma_dual_layer)
					comp_state.dual_comp_id =
						mtk_lye_get_dual_exdma_id(comp_state.comp_id,
							mtk_crtc, priv, rsz_lye, dual_con_lyr);
				fun_lye = -1;
				blender_lye = -1;
			} else {
				comp_state.comp_id =
					mtk_lye_get_exdma_comp_id(disp_idx, i, priv, mtk_crtc, fun_lye, rsz_lye);
				if (mtk_crtc->path_data->is_exdma_dual_layer)
					comp_state.dual_comp_id =
						mtk_lye_get_dual_exdma_id(comp_state.comp_id,
							mtk_crtc, priv, rsz_lye, dual_con_lyr);
			}
			DDPMSG("%s disp_idx %d, i %d, fun_lye %d, rsz_lye %d, comp %s dual %s\n", __func__,
				disp_idx, i, fun_lye, rsz_lye, mtk_dump_comp_str_id(comp_state.comp_id),
				mtk_dump_comp_str_id(comp_state.dual_comp_id));
			comp_state.lye_id = 0;
		} else {
			comp_state.comp_id =
		...
		if (is_extended_layer(layer_info)) {
			...
			ext_cnt++;
			if (mtk_crtc->path_data->is_exdma_dual_layer &&
				comp_state.comp_id == prev_comp_id && lye_type == LYE_NOR)
				dual_blender--;
		} else {
		...
		if (priv->data->ovl_exdma_rule) {
			comp_state.blender_comp_id =
				mtk_lye_get_blender_comp_id(disp_idx, i, priv, mtk_crtc, blender_lye,
					dual_blender, comp_state.comp_id, &reply_bld_lye, rsz_lye);
			if (mtk_crtc->path_data->is_exdma_dual_layer && lye_type == LYE_NOR) {
				dual_blender++;
				comp_state.dual_blender_comp_id =
					mtk_lye_get_dual_blender_id(comp_state.blender_comp_id, mtk_crtc, priv,
							&reply_bld_lye, comp_state.dual_comp_id, rsz_lye);
			}
		}

		DDPINFO("dispatch comp %s ,bind %s dual_bind %s, fun_lye %d, relay_lye %d\n",
			mtk_dump_comp_str_id(comp_state.comp_id),
			mtk_dump_comp_str_id(comp_state.blender_comp_id),
			mtk_dump_comp_str_id(comp_state.dual_blender_comp_id), blender_lye, reply_bld_lye);
