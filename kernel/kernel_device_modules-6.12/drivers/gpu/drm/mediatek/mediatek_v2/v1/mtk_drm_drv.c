...
static bool mtk_mcd_panel_lock(struct drm_atomic_state *state)
{
	...
	bool crtc_need_lock = false;

	struct drm_connector *connector;
	struct drm_connector_state *old_connector_state, *new_connector_state;
	struct mtk_connector_state *old_mtk_connector_state, *new_mtk_connector_state;
	bool conn_need_lock = false;
	unsigned index;
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		panel_funcs = mtk_drm_get_lcm_ext_funcs(crtc);
		if (!panel_funcs) {
			pr_err("panel_funcs is null\n");
			continue;
		}

		panel_params = mtk_drm_get_lcm_ext_params(crtc);
		if (!panel_params) {
			pr_err("panel_params is null\n");
			continue;
		}
		...
		/* 3: doze */
			...
			crtc_need_lock = true;
		/* 4: HBM enable */
		} else if (old_state->prop_val[CRTC_PROP_HBM_ENABLE] != new_state->prop_val[CRTC_PROP_HBM_ENABLE]) {
			crtc_need_lock = true;
		}

		if (crtc_need_lock) {
			pr_info("%s: [CRTC:%d:%s:%d] active(%d->%d) changed(%d %d %d)->(%d %d %d) disp_mode(%llu->%llu) doze(%llu->%llu) hbm(%llu->%llu)\n",
				...
				old_state->prop_val[CRTC_PROP_HBM_ENABLE], new_state->prop_val[CRTC_PROP_HBM_ENABLE]);		}
	}

	for_each_oldnew_connector_in_state(state, connector, old_connector_state, new_connector_state, i) {
		crtc = connector->state->crtc;
		if (!crtc) {
			pr_info("%s connector has no crtc\n", __func__);
			continue;
		}
		if (drm_crtc_index(crtc))
			continue;
		...
		old_mtk_connector_state = to_mtk_connector_state(old_connector_state);
		new_mtk_connector_state = to_mtk_connector_state(new_connector_state);
		if (!old_mtk_connector_state || !new_mtk_connector_state) {
			pr_err("connector_state is null\n");
			continue;
		}

		if (old_mtk_connector_state->prop_val[index][CONNECTOR_PROP_CSC_BL]  != new_mtk_connector_state->prop_val[index][CONNECTOR_PROP_CSC_BL])
			conn_need_lock = true;

		if (conn_need_lock)
			pr_info("%s: [conn:%d] br(%llu->%llu)\n", __func__, index,
				old_mtk_connector_state->prop_val[index][CONNECTOR_PROP_CSC_BL],
				new_mtk_connector_state->prop_val[index][CONNECTOR_PROP_CSC_BL]);
	}

	need_lock = (crtc_need_lock ||conn_need_lock);
...
static void mtk_drm_panel_first_enable(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_panel_funcs *panel_funcs = mtk_drm_get_lcm_ext_funcs(crtc);
	...
	if (!panel_funcs->late_register || !panel_params->drm_panel)
		return;

	panel_funcs->late_register(panel_params->drm_panel);
...
static void mtk_drm_first_enable(struct drm_device *drm)
{
 ...
 #if IS_ENABLED(CONFIG_DRM_PANEL_MCD_COMMON)
		mtk_drm_panel_first_enable(crtc);
#endif
	}
}
