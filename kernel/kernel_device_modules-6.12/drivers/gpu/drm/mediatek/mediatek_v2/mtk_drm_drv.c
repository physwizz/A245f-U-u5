...
#define _ATOMIC_MML_FMT \
	...
	for (i = 0; i < MML_MAX_PLANES; ++i)
		mtk_crtc->mml_cfg->buffer.src.fd[i] = -1;
	for (i = 0; i < MML_MAX_OUTPUTS; ++i) {
		mtk_crtc->mml_cfg->buffer.dest[i].fence = -1;
		mtk_crtc->mml_cfg_pq->buffer.dest[i].fence = -1;
	}
...
static const enum mtk_ddp_comp_id mt6993_mtk_ovlsys_main_bringup_dual[] = {
	DDP_COMPONENT_OVL_EXDMA3,
	DDP_COMPONENT_OVL0_BLENDER1,
	DDP_COMPONENT_OVL_EXDMA4,
	DDP_COMPONENT_OVL0_BLENDER2,
	...
	DDP_COMPONENT_OVL1_BLENDER4,
	DDP_COMPONENT_OVLSYS1_DLO_ASYNC8,
	DDP_COMPONENT_OVLSYS_DLI_ASYNC9,
	DDP_COMPONENT_OVL0_OUTPROC0,
	//DDP_COMPONENT_OVL0_OUTPROC_OUT_CB6,
	DDP_COMPONENT_OVLSYS_DLO_ASYNC12,
	DDP_COMPONENT_OVLSYS_DLI_ASYNC2,
	DDP_COMPONENT_OVLSYS_DLO_ASYNC7,
	DDP_COMPONENT_OVLSYS_DLO_ASYNC1,
	DDP_COMPONENT_OVLSYS1_DLI_ASYNC8,
	DDP_COMPONENT_OVLSYS1_DLI_ASYNC2,
	DDP_COMPONENT_OVLSYS1_DLO_ASYNC1,
	DDP_COMPONENT_OVLSYS2_DLI_ASYNC2,
	DDP_COMPONENT_OVLSYS2_DLO_ASYNC1,
	DDP_COMPONENT_OVL_EXDMA0,
};
...
static const struct mtk_crtc_path_data mt6993_mtk_main_dual_path_data = {
	.ovl_path[DDP_MAJOR][0] = mt6993_mtk_ovlsys_main_bringup_dual,
	.ovl_path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt6993_mtk_ovlsys_main_bringup_dual),
	.path[DDP_MAJOR][0] = mt6993_mtk_ddp_main_bringup,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt6993_mtk_ddp_main_bringup),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.addon_data = mt6993_addon_main,
...
static const struct mtk_mmsys_driver_data mt6993_mmsys_driver_data = {
	...
	.main_dual_path_data = &mt6993_mtk_main_dual_path_data,
...
*/
	...
	.update_channel_hrt = mtk_disp_update_channel_hrt_common,
	.update_channel_hrt_write = mtk_disp_update_channel_hrt_write_common,
	.get_channel_idx = mtk_disp_get_channel_idx,
	.pwr_clk_map = pwr_clk_map,
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		if (g_dpc_dev) {
			...
			if (atomic_read(&top_isr_ref) == 1) {
				DRM_MMP_EVENT_START(IRQ, comp->id, 0x11111111);
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		if (g_dpc_dev) {
			...
			if (atomic_read(&top_isr_ref) == 0) {
				mtk_vidle_user_power_release(DISP_VIDLE_USER_TOP_CLK_ISR);
				DRM_MMP_EVENT_END(IRQ, comp->id, 0x22222222);
			}
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	if (of_property_read_bool(private->mmsys_dev->of_node, "enable-main-full-ovl-path"))
		...
		DDPMSG("%s path_type[%u]\n", __func__, crtc_path_type);
	}

	ret = mtk_drm_crtc_create(drm, path_data);

	DDPMSG("%s crtc_path_type[%u]\n", __func__, crtc_path_type);
		...
		struct drm_file *file_priv)
{
	...
	if (target_t > 0) {
		...
		if (current_t < wakeup_t) {
			...
			// sleep_t > 100ms -> get a corrupted target_t
			if (sleep_t > 100000) {
				DDPPR_ERR("%s: fail, time1 corrupted, step_dur:%u, target_t:%u, current_t:%u\n",
					__func__, step_dur, target_t, current_t);
			} else {
	...
	if (target_t > 0) {
		...
		if (current_t < wakeup_t) {
			...
			// sleep_t > 100ms -> get a corrupted target_t
			if (sleep_t > 100000) {
				DDPPR_ERR("%s: fail, time2 corrupted, step_dur:%u, target_t:%u, current_t:%u\n",
					__func__, step_dur, target_t, current_t);
			} else {
