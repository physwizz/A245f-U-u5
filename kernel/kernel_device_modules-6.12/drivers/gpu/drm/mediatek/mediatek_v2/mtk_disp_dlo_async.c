				 ...
				 struct cmdq_pkt *handle)
{
	...
	u32 dlo_in_relay_size, width = 0, height = 0;
	...
	if ((addon_config->addon_async_config.module == DISP_OVLSYS_0_1_EXDMA) ||
		...
		dlo_in_relay_size = dlo->data->regs[OVL_DL_OUT_RELAY1_SIZE];
		...
		DDPINFO("%s addon:%d comp:%s dli:%#x w:%u h:%u\n",
		__func__, addon_config->addon_async_config.module, mtk_dump_comp_str(comp),
		dlo_in_relay_size, width, height);
	}
