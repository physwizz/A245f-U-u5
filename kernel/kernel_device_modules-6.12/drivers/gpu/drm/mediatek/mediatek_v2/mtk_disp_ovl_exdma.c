	...
	struct cmdq_pkt *handle, int keep_first_layer)
{
	...
	if (keep_first_layer) {
		if (comp->id == DDP_COMPONENT_OVL_EXDMA2 ||
			comp->id == DDP_COMPONENT_OVL_EXDMA3 ||
			comp->id == DDP_COMPONENT_OVL_EXDMA4) {
...
static void mtk_ovl_exdma_start(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	...
	*/
	...
	} else if (comp->id == DDP_COMPONENT_OVL1_EXDMA2) /* WA without RPO, need fix */
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa +
			regs[OVL_EXDMA_MOUT], 0x1, 0x3);
	else
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa +
				 ...
				 struct cmdq_pkt *handle)
{
	...
	struct mtk_addon_module_data module_data;
	...
	if (blender_id)
		...
		mtk_drm_crtc_exdma_ovl_path(mtk_crtc, comp, comp->bind_comp->id, handle, false,
			rpo_check_flag);
		blender_id = comp->bind_comp->id;
	}
#endif

	if (reg_data->dispsys_map[comp->id] != reg_data->dispsys_map[blender_id]) {
		if ((reg_data->dispsys_map[comp->id] == OVLSYS0) &&
			(reg_data->dispsys_map[blender_id] == OVLSYS1)) {
			addon_config.addon_async_config.module = DISP_OVLSYS_0_1_EXDMA;
			module_data.module = DISP_OVLSYS_0_1_EXDMA;
		} else if ((reg_data->dispsys_map[comp->id] == OVLSYS1) &&
			(reg_data->dispsys_map[blender_id] == OVLSYS0)) {
			addon_config.addon_async_config.module = DISP_OVLSYS_1_0_EXDMA;
			module_data.module = DISP_OVLSYS_1_0_EXDMA;
		}

		addon_config.addon_async_config.width = pending->width;
		addon_config.addon_async_config.height = pending->height;
		mtk_addon_path_config(crtc, &module_data, &addon_config, handle);
	}
...
void mtk_ovl_exdma_dump_golden_setting(struct mtk_ddp_comp *comp)
{
	...
	const u16 *regs = exdma->data->regs;
	const u32 *reg_fld = exdma->data->reg_fld;
	unsigned int src_con, ext_en[3], ovl_en;
	...
	if (!(ovl_en & 0x1) || (!(src_con & 0x1) && !(ext_en[0] & 0x1) &&
		...
		DDPDUMP("OVL_EN or all Layer not enable\n");
		return;
	}
