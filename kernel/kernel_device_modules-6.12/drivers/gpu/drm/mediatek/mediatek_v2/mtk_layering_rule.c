...
static uint16_t layer_mapping_table_sys_dual_exdma[HRT_TB_NUM] = {
	0x0003, 0x00F8, 0x00F0, 0x0001
};
...
static uint16_t ovl_mapping_table_sys_dual_exdma[HRT_TB_NUM] = {
	0x0002, 0x00F9, 0x00F9, 0x0001
};
	...
	case DISP_HW_OVL_TB:
		if (priv->data->mmsys_id == MMSYS_MT6985 ||
			...
			if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH))
				...
				//	map = ovl_mapping_table_sys_dual_exdma[addon_data->hrt_type];
				//else
	...
	case DISP_HW_LAYER_TB:
		if (param <= MAX_PHY_OVL_CNT && param >= 0) {
			if (priv->data->mmsys_id == MMSYS_MT6985 ||
				...
				//if (priv->data->mmsys_id == MMSYS_MT6993)
				//	tmp_map = layer_mapping_table_sys_dual_exdma[addon_data->hrt_type];
				//else
...
void mtk_layering_rule_init(struct drm_device *dev)
{
	...
	if (private->data->mmsys_id == MMSYS_MT6991)
		l_rule_info.comp_map = ovl_spda_mapping_mt6991;
	else if (private->data->mmsys_id == MMSYS_MT6993)
		l_rule_info.comp_map = ovl_spda_mapping_mt6993;
