...
int mtk_ovl_blender_dump(struct mtk_ddp_comp *comp)
{
	...
	dp_con = readl(regs[OVL_BLD_DATAPATH_CON] + baddr);

	if (!(ovl_en & 0x1) || !dp_con)
		return 0;
...
int mtk_ovl_blender_analysis(struct mtk_ddp_comp *comp)
{
	...
	DDPDUMP("ovl_en=%d, bgclr_in_sel=%d, out_sel=%x, sta=0x%x\n",
		ovl_en & 0x1,
	...
	struct cmdq_pkt *handle, int keep_first_layer)
{
	...
	if (keep_first_layer) {
		if (comp->id == DDP_COMPONENT_OVL0_BLENDER0 ||
			comp->id == DDP_COMPONENT_OVL0_BLENDER1 ||
			comp->id == DDP_COMPONENT_OVL0_BLENDER2) {
			DDPINFO("%s+ %s not off\n", __func__, mtk_dump_comp_str(comp));
