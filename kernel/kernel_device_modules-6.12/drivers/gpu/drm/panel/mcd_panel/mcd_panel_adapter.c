...
static int mcd_drm_parse_dt(void *_ctx, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "MTK,pps_rate_control_m", &ctx->spec.pps_rate_control_m);
	dev_info(ctx->dev, "%s: pps_rate_control_m: %d\n", __func__, ctx->spec.pps_rate_control_m);

	ret = of_property_read_u32(np, "MTK,cont_clk", &ctx->spec.cont_clk);
	dev_info(ctx->dev, "%s: cont_clk: %d\n", __func__, ctx->spec.cont_clk);
...
int mtk_drm_cmdset_add(struct mtk_panel *ctx, struct mipi_dsi_device *dsi, u8 type, const u8 *data, size_t size)
{
	...
	if (size + 4 >= MAX_HW_TX_FIFO_SIZE) {
		dev_err(ctx->dev, "%s fifo sfr size limit over. (size:%d max:%d) (type:0x%02x cmd:0x%02x)\n",
			__func__, (int)size, MAX_HW_TX_FIFO_SIZE, type, data[0]);
		BUG();
	}
	...
	dev_dbg(ctx->dev, "%s index: %d tx_cmd_num: %d flags:%x\n",
			__func__, index, (int)dsi_msg->tx_cmd_num, dsi_msg->flags);
...
int mtk_drm_cmdset_flush(struct mtk_panel *ctx, bool wait_done)
{
	...
	ret = set_lcm_wrapper(ctx->cmdq_handle, &ctx->mtk_dsi_msg, set_lcm_blocking);
	if (ret < 0) {
...
static int mtk_dsi_read(struct mipi_dsi_device *dsi, u8 cmd, u8 *buf, int len)
{
	...
	ret = read_lcm_wrapper(NULL, cmd_msg);
	...
	for (i = 0; i < (u16)cmd_msg->rx_cmd_num; i++) {
		...
		dev_info(&dsi->dev, "read lcm addr:0x%x--dlen:%d--cmd_idx:%d\n",
			*(char *)(cmd_msg->tx_buf[i]), (int)ret_dlen, i);
...
/**************  MTK EXT **************/
static int mtk_panel_ext_late_register(struct drm_panel *panel)
{
	struct mtk_panel *ctx = container_of(panel, struct mtk_panel, panel);

	pr_info("%s: dev_name(%s)\n", __func__, dev_name(ctx->dev));

#if (MCD_PANEL_PROBE_DELAY_MSEC == 0)
...
static int mcd_panel_set_lock(struct mtk_panel *ctx, int lock)
{
	...
	if (lock) {
		...
		if (ret < 0) {
			dev_err(ctx->dev, "%s failed to panel big lock %d\n", __func__, ret);
			return ret;
		}
	} else {
		ret = call_mcd_panel_func(ctx->mcd_panel_dev, unlock);
		if (ret < 0) {
			dev_err(ctx->dev, "%s failed to panel big unlock %d\n", __func__, ret);
			return ret;
		}
	}
...
static int mtk_panel_ext_set_panel_lock_pid(struct drm_panel *panel, int pid)
{
	struct mtk_panel *ctx = container_of(panel, struct mtk_panel, panel);
	int old_pid;

	old_pid = call_mcd_panel_func(ctx->mcd_panel_dev, set_lock_pid, pid);
	if (old_pid < 0) {
		dev_err(ctx->dev, "%s failed to set_lock_pid %d\n", __func__, old_pid);
		return old_pid;
	}
...
static int mtk_panel_brightness_set(struct drm_panel *panel, void *handle, unsigned int level)
{
	int ret = 0;
	struct mtk_panel *ctx;
	...
	//added 6.12
//et = mcd_panel_check_probe(ctx);
//f (ret < 0)
//dev_err(ctx->dev, "%s mcd-panel not probed %d\n", __func__, ret);

	dev_info(ctx->dev, "%s %u ++\n", __func__, level);

	ctx->cmdq_handle = (struct cmdq_pkt *)handle;

	ret = call_mcd_panel_func(ctx->mcd_panel_dev, set_brightness, level);
	if (ret < 0) {
		dev_err(ctx->dev, "%s: mcd_panel set_brightness failed %d", __func__, ret);
		return ret;
	}

	ctx->cmdq_handle = NULL;
...
static struct mtk_panel_funcs ext_funcs = {
	.late_register = mtk_panel_ext_late_register,
	.set_power = mtk_panel_ext_set_power,
#if IS_ENABLED(CONFIG_USDM_PANEL_BIG_LOCK)
	.set_panel_lock = mtk_panel_ext_set_panel_lock,
	.set_panel_lock_pid = mtk_panel_ext_set_panel_lock_pid,
#endif
...
#endif
	.set_lcm_backlight_cmdq = mtk_panel_brightness_set,
};
...
static void mcd_panel_adapter_remove(struct mipi_dsi_device *dsi)
{
