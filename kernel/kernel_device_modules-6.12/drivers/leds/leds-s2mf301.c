...
int volt_direction = VOLTAGE_INCREASE; /* 1-Increase, 0-Decrease */

void s2mf301_set_volt_direction(int value)
{
	volt_direction = value;
}
EXPORT_SYMBOL(s2mf301_set_volt_direction);

int s2mf301_get_volt_direction(void)
{
	return volt_direction;
}
EXPORT_SYMBOL(s2mf301_get_volt_direction);
...
int s2mf301_set_charger_voltage(enum flash_chg_voltage set_voltage)
{
	int ret = 0;
	static enum flash_chg_voltage current_chager_voltage = FLASH_CHG_VOLTAGE_NOT_SET;

	if (set_voltage == FLASH_CHG_VOLTAGE_NORMAL && current_chager_voltage != FLASH_CHG_VOLTAGE_NORMAL) {
		pdo_ctrl_by_flash(1);
		ret = muic_afc_request_voltage(FLED, 5);
		if (ret < 0)
			pr_err("[%s]muic_afc_request_voltage(5V): failed\n", __func__);
		else {
			current_chager_voltage = FLASH_CHG_VOLTAGE_NORMAL;
			pr_info("[%s]set charger voltage: 5V\n", __func__);
		}
	} else if (set_voltage == FLASH_CHG_VOLTAGE_HIGH && current_chager_voltage != FLASH_CHG_VOLTAGE_HIGH) {
		pdo_ctrl_by_flash(0);
		ret = muic_afc_request_voltage(FLED, 9);
		if (ret < 0)
			pr_err("[%s]muic_afc_request_voltage(9V): failed\n", __func__);
		else {
			current_chager_voltage = FLASH_CHG_VOLTAGE_HIGH;
			pr_info("[%s]set charger voltage: 9V\n", __func__);
		}
	}

	return ret;
}
EXPORT_SYMBOL(s2mf301_set_charger_voltage);
...
static int s2mf301_fled_set_mode(struct s2mf301_fled_data *fled, int chan, int mode)
{
	...
	int gpio_torch, gpio_flash;
	if ((fled != NULL) && (fled->pdata != NULL)) {
		gpio_torch = fled->pdata->torch_gpio;
		gpio_flash = fled->pdata->flash_gpio;
	} else {
		pr_err("%s: fled or its pdata is NULL(g_fled_data is not initialized).\n", __func__);
		return -EFAULT;
	}
	...
	pr_info("%s: get gpio status as gpio_torch(%d) = %d, gpio_flash(%d) = %d.\n", __func__, gpio_torch,
		gpio_get_value(gpio_torch), gpio_flash, gpio_get_value(gpio_flash));
	...
	case S2MF301_FLED_MODE_PREFLASH:
		s2mf301_update_reg(fled->i2c, S2MF301_FLED_CTRL0,
				S2MF301_EN_FLED_PRE, S2MF301_EN_FLED_PRE);
	...
	pr_info("%s: set gpio status as gpio_torch(%d) = %d, gpio_flash(%d) = %d.\n", __func__, gpio_torch,
		gpio_get_value(gpio_torch), gpio_flash, gpio_get_value(gpio_flash));
...
int s2mf301_mode_change_cam_to_leds(enum cam_flash_mode cam_mode)
{
	...
	case CAM_FLASH_MODE_OFF:
		...
		break;
	case CAM_FLASH_MODE_FLASH:
		mode = S2MF301_FLED_MODE_FLASH;
		break;
	case CAM_FLASH_MODE_TORCH:
		mode = S2MF301_FLED_MODE_TORCH;
		break;
	case CAM_FLASH_MODE_PREFLASH:
		mode = S2MF301_FLED_MODE_PREFLASH;
		break;
	default:
		mode = S2MF301_FLED_MODE_OFF;
		break;
...
int s2mf301_fled_set_mode_ctrl(int chan, enum cam_flash_mode cam_mode)
{
	...
	if (fled == NULL) {
		pr_err("%s: g_fled_data is not initialized.\n", __func__);
		return -1;
	}
...
EXPORT_SYMBOL(s2mf301_fled_set_mode_ctrl);
...
int s2mf301_fled_set_curr(int chan, enum cam_flash_mode cam_mode, int curr)
{
	...
	case S2MF301_FLED_MODE_TORCH:
		/* Set curr. */
		if (preflash_done == true)
			break;
		if (curr == 50)
			s2mf301_fled_set_torch_curr(fled, chan, curr);   //SAMSUNG_CAMERA_FLASH_CURRENT_JAPAN
		else
			s2mf301_fled_set_torch_curr(fled, chan, fled->movie_current);
		break;
	case S2MF301_FLED_MODE_FLASH:
		/* Set curr. */
		if (preflash_done == true)
...
EXPORT_SYMBOL(s2mf301_fled_set_curr);
...
static int s2mf301_led_dt_parse_pdata(struct device *dev, struct s2mf301_fled_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "torch_max_current", &pdata->torch_max_current);
	if (ret < 0)
		pr_err("%s : could not find torch_max_current\n", __func__);
	ret = of_property_read_u32(np, "flash_max_current", &pdata->flash_max_current);
	if (ret < 0)
		pr_err("%s : could not find flash_max_current\n", __func__);
...
int s2mf301_get_max_current(int mode)
{
	...
	if (mode == CAM_FLASH_MODE_FLASH)
		return g_fled_data->pdata->flash_max_current;
	else if (mode == CAM_FLASH_MODE_TORCH)
		return g_fled_data->pdata->torch_max_current;

	return -1;
}
EXPORT_SYMBOL_GPL(s2mf301_get_max_current);

ssize_t s2mf301_rear_flash_store(struct device *dev,
...
ssize_t s2mf301_rear_flash_show(struct device *dev,
...
int create_flash_sysfs(void)
{
	...
	IMGSENSOR_SYSFS_GET_CAMERA_CLASS(&camera_class);
...
int destroy_flash_sysfs(struct class *class)
{
#if IS_ENABLED(CONFIG_IMGSENSOR_SYSFS)
	if (flash_dev)
		device_remove_file(flash_dev, &dev_attr_rear_flash);
	if (class && flash_dev)
		device_destroy(class, flash_dev->devt);
#endif
	return 0;
}
...
static int s2mf301_led_probe(struct platform_device *pdev)
{
...
#if IS_ENABLED(CONFIG_SEC_FACTORY)
	s2mf301_read_reg(fled_data->chg, S2MF301_CHG_QBAT_OFF, &chg_reg);
	pr_info("%s: chg_reg(0x%x)\n", __func__, chg_reg);

	if (chg_reg & 0x02) {
		s2mf301_fled_operating_mode(fled_data, TA_MODE);
		fled_data->set_on_factory = 1;
		pr_info("%s: TA MODE on boot\n", __func__);
	}
#endif
	...
	create_flash_sysfs();
	pr_info("%s: end, int get flash_gpio(%d) = %d, get torch_gpio(%d) = %d\n", __func__, g_fled_data->flash_gpio,
		gpio_get_value(g_fled_data->flash_gpio), g_fled_data->torch_gpio, gpio_get_value(g_fled_data->torch_gpio));
	return 0;
}

static void s2mf301_led_remove(struct platform_device *pdev)
{
	destroy_flash_sysfs(camera_class);
}
...
static const struct of_device_id s2mf301_fled_match_table[] = {
	{ .compatible = "samsung,s2mf301-fled",},
	{},
};
#endif
...
static struct platform_driver s2mf301_led_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = s2mf301_fled_match_table,
#endif
		},
