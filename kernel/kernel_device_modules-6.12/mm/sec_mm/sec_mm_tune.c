...
static void sec_mm_cma_alloc_retry(void *data, char *name, int *retry)
{
	*retry = 10;
}
...
static void sec_mm_override_batch(void *data, int *batch)
{
	*batch = ((SZ_1M / PAGE_SIZE) / 4) - 1;
}
...
void init_sec_mm_tune(void)
{
	...
	register_trace_android_vh_cma_alloc_retry(
			sec_mm_cma_alloc_retry, NULL);
...
void exit_sec_mm_tune(void)
{
	unregister_trace_android_vh_adjust_alloc_flags(
			sec_mm_adjust_alloc_flags, NULL);
	unregister_trace_android_vh_cma_alloc_retry(
			sec_mm_cma_alloc_retry, NULL);
