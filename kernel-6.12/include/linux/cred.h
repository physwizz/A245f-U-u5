...
#if defined(CONFIG_KDP) && defined(MODULE)
/* Kernel modules do not have access to the implementations in kdp.c.
 * So, we define these variants for modules here
 */
static inline void kdp_usecount_add(int nr, struct cred *cred)
{
	atomic_long_add(nr, ((struct cred_kdp *)cred)->use_cnt);
}
static inline unsigned int kdp_usecount_sub_and_test(int nr, struct cred *cred)
{
...
#ifdef CONFIG_KDP
		if ((atomic_long_read(&cred->usage) & KDP_CRED_MAGIC) == KDP_CRED_MAGIC) {
			if (kdp_usecount_sub_and_test(nr, cred))
				__put_cred(cred);
		} else {
