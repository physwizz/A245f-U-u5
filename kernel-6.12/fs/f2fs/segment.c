...
retry:
	...
	if ((f2fs_sb_has_blkzoned(sbi) || (sbi->pin_guaranteed_blkaddr)) &&
			err == -EAGAIN && gc_required) {
		f2fs_down_write(&sbi->gc_lock);
		if (f2fs_sb_has_blkzoned(sbi))
			err = f2fs_gc_range(sbi, 0, GET_SEGNO(sbi, FDEV(0).end_blk),
					true, ZONED_PIN_SEC_REQUIRED_COUNT);
		else
			err = f2fs_gc_range_for_pinning(sbi, 0,
					GET_SEGNO(sbi, sbi->pin_guaranteed_blkaddr - 1), true, 1);
