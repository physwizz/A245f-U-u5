...
int f2fs_resize_fs(struct file *filp, __u64 block_count)
{
	...
	f2fs_update_sec_ddp_time_stats(sbi, resize_start_time,
			resize_end_time, DDP_SHRINK_ELAPSED_TIME);
...
int f2fs_gc_range_for_pinning(struct f2fs_sb_info *sbi,
		unsigned int start_seg, unsigned int end_seg,
		bool dry_run, unsigned int dry_run_sections)
{
	unsigned long long gc_start_time, gc_end_time = 0;
	struct cp_control cpc = { .reason = CP_SYNC, };
	int ret;

	gc_start_time = local_clock();
	ret = f2fs_gc_range(sbi, start_seg, end_seg, dry_run, dry_run_sections);
	...
	sbi->sec_ddp_stat.ddp_stats[DDP_MIGRATED_SEG_COUNT] += dry_run_sections;
