...
static void f2fs_print_lower_node(struct f2fs_sb_info *sbi, struct page *page)
{
	...
	if (PageUptodate(page)) {
		...
		print_block_data(sbi, page->index, page_address(page), 0, F2FS_BLKSIZE);
	}
}
...
out_err:
	if (PageUptodate(&folio->page)) {
		print_block_data(sbi, nid, page_address(&folio->page), 0, F2FS_BLKSIZE);
		if (!is_zeroed_page(&folio->page, 0, F2FS_BLKSIZE))
			f2fs_print_lower_node(sbi, &folio->page);
		f2fs_bug_on(sbi, 1);
	}
	folio_clear_uptodate(folio);
out_put_err:
			...
			struct page *nat_page, nid_t start_nid)
{
	...
	for (; i < NAT_ENTRY_PER_BLOCK; i++, start_nid++) {
		...
		if (blk_addr == NEW_ADDR) {
			print_block_data(sbi, current_nat_addr(sbi, start_nid),
