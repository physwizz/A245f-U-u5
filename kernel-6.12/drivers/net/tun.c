				 ...
				 struct knox_meta_param *metalocal)
{
	...
	if (knox_shinfo->android_oem_data1[2] >= META_MARK_BASE_LOWER &&
	    knox_shinfo->android_oem_data1[2] <= META_MARK_BASE_UPPER) {
		metalocal->uid = (uid_t)knox_shinfo->android_oem_data1[0];
		metalocal->pid = (pid_t)knox_shinfo->android_oem_data1[1];
	}

	if (knox_shinfo) {
		knox_shinfo->android_oem_data1[0] = 0;
		knox_shinfo->android_oem_data1[1] = 0;
			    ...
			    struct iov_iter *iter)
{
	...
	if (meta_param_get_status == 1) {
		...
		if (tun->flags & TUN_META_HDR) {
			...
			if (copy_to_iter(&metalocal, sizeof(struct knox_meta_param), iter) !=
