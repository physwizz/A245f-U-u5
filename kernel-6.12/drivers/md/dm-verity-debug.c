...
int verity_handle_err_hex_debug(struct dm_verity *v,
				...
				struct dm_verity_io *io, const u8 *data, const u8 *want_digest)
{
	...
	if (!(strcmp(type_str, "metadata"))) {
		...
		print_block_data(0, (unsigned char *)(io->tmp_digest), 0, v->digest_size);
		print_block_data(0, (unsigned char *)(want_digest), 0, v->digest_size);
