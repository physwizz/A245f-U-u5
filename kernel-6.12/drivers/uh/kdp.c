...
void kdp_usecount_add(int nr, struct cred *cred)
{
	if (is_kdp_protect_addr((unsigned long)cred))
		atomic_long_add(nr, ((struct cred_kdp *)cred)->use_cnt);
	else
...
unsigned int kdp_usecount_sub_and_test(int nr, struct cred *cred)
{
	if (is_kdp_protect_addr((unsigned long)cred))
		return (unsigned int)atomic_long_sub_and_test(nr, ((struct cred_kdp *)cred)->use_cnt);
	else
...
struct cred *prepare_ro_creds(struct cred *old, int kdp_cmd, u64 p)
{
	...
	struct cred_kdp_init *use_cnt_ptr = NULL;
	struct ro_rcu_head *rcu_ptr = NULL;
	void *tsec = NULL;
	...
	// get_usecnt_rcu
	rcu_ptr = &(use_cnt_ptr->ro_rcu_head_init);
	rcu_ptr->bp_cred = (void *)new_ro;
	...
	param_data.use_cnt_ptr = (void *)use_cnt_ptr;
	...
	if (kdp_cmd == CMD_COPY_CREDS) {
		if ((new_ro->bp_task != (void *)p) ||
			...
			new_ro->use_cnt != (void *)use_cnt_ptr) {
