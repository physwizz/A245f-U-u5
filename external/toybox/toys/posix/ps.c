...
static int ps_match_process(long long *slot)
{
  ...
  // Filter implicit categories for other display types
    ...
    /* { SRCG:BAIDU_CARLIFE */
    char value[92] = {0};
    int is_car_link_force_on = property_get("persist.sys.adb.config.carlife_force", value, "false");
    if (is_car_link_force_on == strlen("true") && FLAG(a)) {
        return 1;
    }
    /* SRCG:BAIDU_CARLIFE }*/
#endif
...
#ifdef SEC_CARLIFE
  /* { SRCG:BAIDU_CARLIFE */
  char *app_process[9] = {"root", "713", "28671", "4418488", "94264", "do_sys_poll", "0", "S", "app_process"};
  int count = 0;
  char value[92] = {0};
  int is_car_link_force_on = property_get("persist.sys.adb.config.carlife_force", value, "false");
  int is_car_life_start = property_get("persist.sys.adb.config.carlife_start", value, "false");
  /* SRCG:BAIDU_CARLIFE } */
#endif
...
#ifdef SEC_CARLIFE
    /* { SRCG:BAIDU_CARLIFE start*/
    if (is_car_link_force_on == strlen("true")) {
        if (count < 9 && TT.carlifecount == 0) {
            out = app_process[count];
            if (is_car_life_start == strlen("true") && count == 8) {
                out = "app_process u0_a132 5156 442 1923660 374244 SyS_epoll_ 00f731548c S com.baidu.carlife";
            }
        } else {
            return;
        }
...
static char *parse_rest(void *data, char *str, int len)
{
  ...
  char value[92] = {0};
  int is_car_link_force_on = property_get("persist.sys.adb.config.carlife_force", value, "false");
  if (is_car_link_force_on == strlen("true") && strstart(&str, "app_process")) {
    return 0;
  }
  /* SRCG:BAIDU_CARLIFE }*/
#endif
