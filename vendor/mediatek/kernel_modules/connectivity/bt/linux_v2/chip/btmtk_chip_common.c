...
char picus_filter_cmd[PICUS_FILTER_CMD_LEN] = {0x01, 0x5F, 0xFC, 0x2E, 0x50, 0x01, 0x0A, 0x00, 0x00, 0x00,
		0x01, 0x00, 0x00, 0xE0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
		0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00,
		0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
char picus_filter_event[PICUS_FILTER_EVT_LEN] = {0x04, 0x0E, 0x04, 0x01, 0x5F, 0xFC, 0x00};
...
struct bt_cmds_s commands[] = {
	...
	{"PICUS_FILTER_CMD", {{PICUS_FILTER_CMD_LEN, picus_filter_cmd}, {0, NULL}}, DEFAULT},
	{"PICUS_FILTER_EVT", {{PICUS_FILTER_EVT_LEN, picus_filter_event}, {0, NULL}}, DEFAULT}
};
...
int btmtk_common_data_initialize(struct btmtk_dev *bdev)
{
	if (bdev->dongle_index >= 4) {
		BTMTK_ERR_DEV(bdev, "%s: dongle_index must smaller than 4 (%d)", __func__, bdev->dongle_index);
		return -1;
	}
...
void btmtk_free_chip_struct(struct btmtk_dev *bdev, struct data_struct *fw_cfg, int count)
{
	int i = 0;

	for (i = 0; i < count; i++) {
		if (fw_cfg[i].content) {
			BTMTK_INFO_DEV(bdev, "%s:kfree %d", __func__, i);
...
void btmtk_free_chip_data(struct btmtk_dev *bdev)
{
	...
	for (i = 0; i < BTMTK_CMD_NUM; i++) {
		btmtk_free_chip_struct(bdev, &bt_raw_cmds[i].chip_data[FROM_BIN], 1);
		bt_raw_cmds[i].usage = DEFAULT;
	}
}

int btmtk_load_chip_data_item(struct btmtk_dev *bdev, struct bt_cmds_s *raw_cmds,
		int counter, u8 *searchcontent, enum fw_cfg_index_len index_length)
{
	...
	for (i = 0; i < counter; i++) {
		...
		if (search_result) {
			...
			BTMTK_INFO_DEV(bdev, "%s: %s has been found", __func__, search);

			memset(temp, 0, TEMP_LEN);
			search_end = search_result;
			search_result = strstr(search_result, "0x");

			BTMTK_INFO_DEV(bdev, "%s: search is %zu", __func__, strlen(search));
			BTMTK_INFO_DEV(bdev, "%s: len is %d", __func__, (int)(search_result - search_end));
		...
		if (temp_len && temp_len < TEMP_LEN) {
			...
			BTMTK_INFO_RAW(bdev, save_content[i].content, save_content[i].len, "%s", search);
...
int btmtk_load_raw_data(char *data_bin_name, struct btmtk_dev *bdev)
{
	...
	if (err) {
		BTMTK_WARN_DEV(bdev, "btmtk_load_code_from_setting_files failed!!");
		goto LOAD_END;
	} else {
	/* load from file */
		for (i = 0; i < BTMTK_CMD_NUM; i++) {
			err = btmtk_load_chip_data_item(bdev, &bt_raw_cmds[i],
						1, bdev->setting_file, FW_CFG_INX_LEN_NONE);
			if (err)
				BTMTK_WARN_DEV(bdev, "%s: search item %s is invalid!", __func__, bt_raw_cmds[i].name);
		}
	}
LOAD_END:
		...
		enum cmd_index_s type, struct data_struct *default_cmd)
{
	...
	if (bdev == NULL || bdev->bt_raw_data == NULL) {
		BTMTK_ERR_DEV(bdev, "%s: Incorrect bdev", __func__);
		return;
	}

	if (type >= BTMTK_CMD_NUM) {
		BTMTK_ERR_DEV(bdev, "%s: type %d is illegal!!!", __func__, type);
		return;
	}
	...
	if (bt_cmd.usage < 0) {
		BTMTK_ERR_DEV(bdev, "%s: bt_cmd.usage is a negative value", __func__);
		return;
	}
	...
	if (bt_cmd.usage == SEND_NULL) {
		...
		default_cmd->len = 0;
		BTMTK_INFO_DEV(bdev, "%s: type %s no data to send! ", __func__, bt_cmd.name);
	} else {
		BTMTK_DBG_DEV(bdev, "%s: %s load from %d", __func__, bt_cmd.name, bt_cmd.usage);
		default_cmd->content = bt_cmd.chip_data[bt_cmd.usage].content;
		default_cmd->len = bt_cmd.chip_data[bt_cmd.usage].len;
	}
	// BTMTK_DBG_RAW(bdev, default_cmd->content, default_cmd->len, "%s: get(%s) - ", __func__, bt_cmd.name);
		...
		int delay, int retry, int pkt_type, bool flag)
{
	...
	if (cmd.content == NULL || cmd.len <= 0) {
		BTMTK_WARN_DEV(bdev, "%s: do not send cmd %d", __func__, cmd_type);
		return ret;
	}
	...
	if (ret < 0)
		BTMTK_ERR_DEV(bdev, "%s: failed, type is %d", __func__, cmd_type);

	BTMTK_DBG_DEV(bdev, "%s: end. ret = %d", __func__, ret);
	return ret;
}
		...
		const u8 *buf, u32 len, const u8 *dbg_buf, u32 dbg_len)
{
	...
	btmtk_hci_snoop_print(bdev, dbg_buf, dbg_len);
	btmtk_hci_snoop_print(bdev, buf, len);
}
...
int btmtk_cif_rx_packet_handler_common(struct btmtk_dev *bdev,  struct sk_buff *skb)
{
	...
	if (bdev->bt_cfg.reset_stack_after_woble &&
		...
		kfree_skb(skb);
		err = CONTINUE_RET;
		//BTMTK_INFO_DEV(bdev, "%s: skip", __func__);
...
int btmtk_cif_tx_frame_handler_common(struct btmtk_dev *bdev, struct sk_buff *skb, u8 *fw_dump)
{
	int ret = 0;
	/* parsing commands */
	struct data_struct reset_cmd = {0}, fw_coredump_cmd = {0}, picus_cmd = {0};
	...
	BTMTK_GET_CMD_OR_EVENT_DATA(bdev, PICUS_ENABLE_CMD, picus_cmd);
	...
	if (hci_skb_pkt_type(skb) == HCI_COMMAND_PKT) {
		...
		if (bdev->get_hci_reset == 1) {
			...
			if (ret < 0) {
				BTMTK_ERR_DEV(bdev, "%s btmtk_set_audio_setting failed!!", __func__);
				return ret;
			}
		}
#endif
		/* save hci cmd pkt for debug */
		btmtk_hci_snoop_save(bdev, HCI_SNOOP_TYPE_CMD_STACK, skb->data, skb->len);
		if (skb->len == FD5B_ASSERT_CMD_LEN &&
			!memcmp(skb->data, fw_coredump_cmd.content, fw_coredump_cmd.len)) {
			BTMTK_INFO_DEV(bdev, "%s: Dongle FW Assert Triggered by BT Stack!", __func__);
			...
			*fw_dump = 1;
			...
			if (bdev->main_info.hif_hook.trigger_assert) {
				...
				BTMTK_INFO_DEV(bdev, "%s: got command: 0x03 0x0C 0x00 (HCI_RESET)", __func__);

				do {
					ret = bdev->main_info.hif_hook.send_and_recv(bdev, skb, skb->data, 3,
							DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_FROM_HOST, CMD_NEED_FILTER);
				} while (ret == -EAGAIN && drv_own_retry--);

				if (ret < 0)
					BTMTK_ERR_DEV(bdev, "%s host call send_and_recv failed, ret[%d]",
							__func__, ret);
				else
					/* return 1 means data already handled */
					ret = 1;
				return ret;
#else
			BTMTK_INFO_DEV(bdev, "%s: got command: 0x03 0C 00 (HCI_RESET)", __func__);
#endif
		} else if (skb->len == picus_cmd.len &&
			!memcmp(skb->data, picus_cmd.content, picus_cmd.len - 1)) {
			/* set fw log level from host */
			bdev->main_info.fw_log_on = skb->data[picus_cmd.len - 1];
			BTMTK_INFO_DEV(bdev, "%s: set fw log level from host[%d]", __func__, bdev->main_info.fw_log_on);
		}
#if (CFG_SUPPORT_TAS_HOST_CONTROL == 1)
		else if (skb->len == HOST_ENABLE_TASAR_V3_CMD_LEN &&
			skb->data[1] == 0x2D && skb->data[2] == 0xFC && skb->data[4] == 0x0D) {
			/* TASAR 3.0: set countrycode and eci from host
			 *   This cmd is only used between host and driver, for setting country and eci
			 *   format(big endian)
			 *     cmd => 01 2D FC 07 0D XX XX YY YY YY YY
			 *     XX XX => country code, ex: TW:[54 57]
			 *     YY YY YY YY YY => ECI. ex: 5:[00 00 00 05]
			 */

			unsigned short int countryCode = 0;
			unsigned int eci = 0;

			countryCode = skb->data[5] << 8 | skb->data[6];
			eci = (skb->data[7] << 24 | skb->data[8] << 16 | skb->data[9] << 8 | skb->data[10]);

			BTMTK_INFO_RAW(bdev, skb->data, skb->len, "%s: host set TASAR, CC[0x%x], eci[%u] len[%d]:",
					__func__, countryCode, eci, skb->len);
		...
		btmtk_hci_snoop_save(bdev, HCI_SNOOP_TYPE_TX_ACL_STACK, skb->data, skb->len);
	} else if (hci_skb_pkt_type(skb) == HCI_ISO_PKT) {
		btmtk_hci_snoop_save(bdev, HCI_SNOOP_TYPE_TX_ISO_STACK, skb->data, skb->len);
	} else if (hci_skb_pkt_type(skb) == HCI_SCODATA_PKT) {
		btmtk_hci_snoop_save(bdev, HCI_SNOOP_TYPE_TX_SCO_STACK, skb->data, skb->len);
#else
			...
			BTMTK_INFO_RAW(bdev, skb->data, skb->len, "%s: VTS TX before rearrange[%d]:",
						__func__, skb->len);
			...
			BTMTK_INFO_RAW(bdev, skb->data, skb->len, "%s: VTS TX after rearrange[%d]:",
					__func__, skb->len);
#endif
	}
...
int btmtk_cif_check_power_status_common(struct btmtk_dev *bdev, const uint8_t *cmd, int pkt_type)
{
	...
	if (memcmp(cmd, bdev->main_info.wmt_over_hci_header, WMT_OVER_HCI_HEADER_SIZE) &&
			pkt_type == BTMTK_TX_PKT_FROM_HOST &&
			bdev->power_state != BTMTK_DONGLE_STATE_POWER_ON) {
		BTMTK_WARN_DEV(bdev, "%s: chip power isn't on, ignore this command, state is %d",
...
static int btmtk_set_audio_pin_mux_common(struct btmtk_dev *bdev)
{
	...
	if (write_pinmux_cmd.len < MTK_HCI_CMD_HEAD_SIZE || write_pinmux_cmd.len > HCI_MAX_FRAME_SIZE) {
		BTMTK_INFO_RAW(bdev, write_pinmux_cmd.content, write_pinmux_cmd.len,
				"%s: get write pinmux cmd failed:", __func__);
		return -1;
	}

	if(!write_pinmux_cmd.len){
		BTMTK_WARN_DEV(bdev, "%s write_pinmux_cmd is NULL, return", __func__);
		return ret;
	}

	if (audio_pinmux_num->content && audio_pinmux_num->length) {
		BTMTK_INFO_DEV(bdev, "%s load audio pinmux num from bt.cfg", __func__);
		memcpy(write_pinmux_pin_num.content, audio_pinmux_num->content, audio_pinmux_num->length);
	} else {
		BTMTK_INFO_DEV(bdev, "%s load default audio pinmux num", __func__);
	}
	BTMTK_INFO_RAW(bdev, write_pinmux_pin_num.content,
			write_pinmux_pin_num.len, "%s: Pin NUM:", __func__);

	if (audio_pinmux_mode->content && audio_pinmux_mode->length) {
		BTMTK_INFO_DEV(bdev, "%s load audio pinmux mode from bt.cfg", __func__);
		memcpy(write_pinmux_pin_mode.content, audio_pinmux_mode->content, audio_pinmux_mode->length);
	} else {
		BTMTK_INFO_DEV(bdev, "%s load default audio pinmux mode", __func__);
	}
	BTMTK_INFO_RAW(bdev, write_pinmux_pin_mode.content,
	...
	for (i = 0; i < PINMUX_REG_NUM_7902; i++) {
		...
		BTMTK_INFO_RAW(bdev, write_pinmux_cmd.content, write_pinmux_cmd.len, "%s: Send CMD:", __func__);
		...
		BTMTK_INFO_DEV(bdev, "%s, confirm pinmux num : 0x%02x, mode :0x%02x", __func__,
...
int btmtk_cif_chip_common_register(struct btmtk_dev *bdev)
{
	int retval = 0;
	...
	btmtk_reg_hif_chip_hook(bdev, &hook_chip);
...
int btmtk_cif_chip_register(struct btmtk_dev *bdev)
{
	...
	switch (chip_id) {
	case 0x7922:
		btmtk_cif_chip_7922_register(bdev);
		break;
	case 0x7902:
		btmtk_cif_chip_7902_register(bdev);
		break;
	case 0x7961:
		btmtk_cif_chip_7961_register(bdev);
		break;
	case 0x6639:
		...
		if (ret) {
			BTMTK_ERR_DEV(bdev, "%s: load raw data from %s failed", __func__, bdev->chip_data_file_name);
			goto register_end;
		}

		btmtk_cif_chip_6639_register(bdev);
		break;
	case 0x7925:
		(void)snprintf(bdev->chip_data_file_name, MAX_BIN_FILE_NAME_LEN, "bt_mt%x_data.bin",
				bdev->chip_id & 0xffff);
	...
	case 0x6635:
	case 0x6653:
		btmtk_cif_chip_66xx_register(bdev);
		break;
	case 0x7663:
		(void)snprintf(bdev->chip_data_file_name, MAX_BIN_FILE_NAME_LEN, "bt_mt%x_data.bin",
				bdev->chip_id & 0xffff);
		...
		btmtk_cif_chip_7663_register(bdev);
		break;
	default:
		BTMTK_ERR_DEV(bdev, "Unknown Mediatek device(%04X)", bdev->chip_id);
		ret = -1;
		break;
	}
