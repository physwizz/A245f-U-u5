...
OUT:
	up(&rd_mtx);
	/* release the data to host wakelock early, avoid waiting 100ms in BT_event_cb */
	if (!btmtk_rx_data_valid())
		__pm_relax(bt_wakelock);

	if (retval > 0)
		btmtk_hci_snoop_save(g_sbdev, HCI_SNOOP_TYPE_RX_DATA_STACK, i_buf, retval);
...
int _ioctl_copy_evt_to_buf(uint8_t *buf, int len)
{
	...
	BTMTK_INFO_RAW(g_sbdev, ioc_buf, (len + 1), "%s: len[%d] RX: ", __func__, len + 1);
	return 0;
}
...
static long BT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOCTL_BT_INTTRX:
		...
		if (copy_from_user(ioc_buf, (uint8_t __user *)arg, IOCTL_BT_HOST_INTTRX_SIZE))
			...
			if (ioc_buf[0] == 0x01 && ioc_buf[1] == 0x2D && ioc_buf[2] == 0xFC) {
				BTMTK_INFO_RAW(g_sbdev, ioc_buf, ioc_buf[3] + 4, "%s: len[%d] TX: ",
					__func__, ioc_buf[3] + 4);
				...
				BTMTK_INFO_RAW(g_sbdev, ioc_buf, ioc_buf[6] + 7, "%s: len[%d] TX: ",
					__func__, ioc_buf[6] + 7);
				if (ioc_buf[6] > 240) {
	...
	case COMBO_IOCTL_BT_FINDER_MODE:
		...
		break;
	case COMBO_IOCTL_BT_RESET_MODE:
		if (copy_from_user(ioc_buf, (uint8_t __user *)arg, 1))
			retval = -EFAULT;

		BTMTK_INFO("RESET mode [%d] for meta test flow", ioc_buf[0]);
		atomic_set(&g_sbdev->meta_tst_reset_mode, ioc_buf[0]);
		break;
	default:
		break;
	}
...
static int BT_open(struct inode *inode, struct file *file)
{
	...
	if (BT_open_count > 0) {
		if (btmtk_fops_get_state(g_sbdev) == BTMTK_FOPS_STATE_OPENED) {
			...
			BTMTK_INFO("Driver not openned, not ready for finder, return fail");
			return -1;
		}
	}
	...
	do {
		...
		if (g_sbdev->is_pre_cal_done) {
			...
			atomic_set(&g_sbdev->host_bt_state, 1);
#if (CFG_SUPPORT_MD_CCCI == 1)
			schedule_delayed_work(&g_sbdev->ccci_md_work, 0);
#endif
...
static int BT_close(struct inode *inode, struct file *file)
{
	...
	atomic_set(&g_sbdev->host_bt_state, 0);
#if (CFG_SUPPORT_MD_CCCI == 1)
	schedule_delayed_work(&g_sbdev->ccci_md_work, 0);
#endif
...
int BT_init(void)
{
	...
	BTMTK_INFO("%s start node name(%s)", __func__, BT_DRIVER_NODE_NAME);
