...
extern int btmtk_dongle_num;
extern int btmtk_mcu_num;
/* To allow g_bdev being sized from btmtk_dongle_num setting */
...
#define BT_MULTI_DONGLE_NUM_MAX		4

#define BT_MULTI_MCU_NUM_MAX 2
...
/* stpbtfwlog setting */
#define FWLOG_QUEUE_COUNT			(400 * BT_MULTI_DONGLE_NUM_MAX)
 ...
 */
/* Driver -> Picus
 * F0 XX XX 01 01 AA 10 BB BB CC CC CC ...
 * F0 : specific header between picus and driver
 * XX XX total command length
 * 01 : FW dongle interface setting type
 * 01 AA length of this segment is 1, AA is dongle index
 * 11 : raw data type, FWLOG_RX
 * BB BB CC CC ... : BB BB is raw data's length
 *                 : CC CC ... is raw data, include HCI type
 */
...
#define FWLOG_FLOW_CTRL_DATA_MAX_LEN 32
...
#define MAIN_MUTEX_INIT(bdev) \
	do { \
		mutex_init(&bdev->btmtk_chip_state_mutex); \
		mutex_init(&bdev->btmtk_fops_state_mutex); \
		mutex_init(&bdev->btmtk_handle_mutex); \
		mutex_init(&bdev->btmtk_filter_cmd_num_mutex); \
	} while (0)

#define MAIN_MUTEX_DESTROY(bdev) \
	do { \
		mutex_destroy(&bdev->btmtk_chip_state_mutex); \
		mutex_destroy(&bdev->btmtk_fops_state_mutex); \
		mutex_destroy(&bdev->btmtk_handle_mutex); \
		mutex_destroy(&bdev->btmtk_filter_cmd_num_mutex); \
	} while (0)
...
#if BT_SWIP_SUPPORT
extern struct mutex btmtk_vnd_event_cb_mutex;
#define VND_EVENT_MUTEX_LOCK()	mutex_lock(&btmtk_vnd_event_cb_mutex)
#define VND_EVENT_MUTEX_UNLOCK()	mutex_unlock(&btmtk_vnd_event_cb_mutex)
#define BTMTK_VND_KO_REG 1
#define BTMTK_VND_KO_NONE_REG 0
extern event_callback evt_cb;
extern atomic_t vnd_module_registered;
extern atomic_t swip_opcode_flag;
extern char swip_opcode[2];
#endif
...
enum {
	...
	HCI_SNOOP_TYPE_TX_SCO_STACK,
	HCI_SNOOP_TYPE_TX_SCO_HIF,
	HCI_SNOOP_TYPE_RX_SCO_STACK,
	HCI_SNOOP_TYPE_RX_SCO_HIF,
	HCI_SNOOP_TYPE_RX_DATA_STACK,
	HCI_SNOOP_TYPE_MAX
};
...
#if (USE_DEVICE_NODE == 1)
typedef void (*cif_chrdev_fw_log_state_ptr)(struct btmtk_dev *bdev, uint8_t state);
...
typedef int (*cif_send_and_recv_ptr)(struct btmtk_dev *bdev,
		...
		int delay, int retry, int pkt_type, bool flag);
typedef int (*cif_raw_filter_ptr)(struct btmtk_dev *bdev, const unsigned char *buf, int size);
...
typedef int (*cif_enter_standby_ptr)(struct btmtk_dev *bdev);
...
typedef int (*cif_get_cif_info)(struct btmtk_dev *bdev, char *str, u8 len);
...
struct btmtk_main_info {
	...
	struct mutex fwlog_ctrl_lock;
	struct work_struct  fwlog_flow_ctrl_work;
	char	fwlog_flow_ctrl_data[FWLOG_FLOW_CTRL_DATA_MAX_LEN];
	u8	fwlog_flow_ctrl_data_len;
...
#define DONGLE_FILE_IS_VALID(bdev, file_idx) \
		(bdev->dongle_index == DONGLE_IDX_0 || file_idx == BTMTK_DONGLE_TYPE_DONGLE_INDEX)
...
struct btmtk_dev {
	...
	/* debug sop */
	struct st_debug_sop debug_sop;

	/* woble */
	struct st_woble *bt_woble;

	/* buffer mode */
	struct st_buffer_mode *buffer_mode;
	...
	/* notify fw screen on/off state */
	struct work_struct  blank_state_work;

	/* notify fw suspend/resume state */
...
#endif
	int le_audio_clk_fw_co_gpio;
	struct mutex btmtk_chip_state_mutex;
	struct mutex btmtk_fops_state_mutex;
	struct mutex btmtk_handle_mutex;
	struct mutex btmtk_filter_cmd_num_mutex;

	char event_need_compare[EVENT_COMPARE_SIZE];
	char event_need_compare_len;
	atomic_t event_compare_status;

	struct btmtk_main_info main_info;

	bool		is_evt_intercept_disable;

	atomic_t host_bt_state;
#if (CFG_SUPPORT_MD_CCCI == 1)
	struct delayed_work	ccci_md_work;
#endif
};
...
static inline int is_mtk_chip(u32 chip_id)
{
	if (is_mt7663(chip_id) || is_mt7961(chip_id) || is_mt7922(chip_id) || is_mt66xx(chip_id) ||
		is_mt7902(chip_id) || is_mt6639(chip_id) || is_mt7925(chip_id))
		return 1;
	return 0;
}
...
int btmtk_load_code_from_bin(struct btmtk_dev *bdev, u8 **image, char *bin_name,
...
int32_t bgfsys_bt_patch_dl(struct btmtk_dev *bdev);
