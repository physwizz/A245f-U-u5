...
/* for PowerOffBLE(find my phone) reset state */
enum finder_reset_state {
	FMD_RESET_STATE_INIT = 0,
	FMD_RESET_STATE_START = 1,
	FMD_RESET_STATE_DONE = 2
};
...
extern int mtk8250_wakeup_callback_register(UART_WAKEUP_CB wakeup_cb, void *wakeup_param);
#endif // CFG_UART_WAKEUP_SUPPORT

#if defined(CFG_INBAND_SUPPORT)
/* inband irq related */
extern int mtk8250_uart_hub_inband_is_support(void);
extern int mtk8250_uart_hub_inband_enable_ctrl(int enable);
...
extern void mtk8250_set_flush_flag(struct tty_struct *tty, bool status);
...
extern void mtk8250_set_runtime_active_status(struct tty_struct *tty);
#endif
...
#define COMBO_IOCTL_BT_RESET_MODE	_IOW(COMBO_IOC_MAGIC, 7, void*)
