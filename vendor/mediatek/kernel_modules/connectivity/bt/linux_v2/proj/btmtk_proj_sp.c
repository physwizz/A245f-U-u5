...
#if (CFG_SUPPORT_MD_CCCI == 1)
struct __attribute__((__packed__)) ccci_ctrl_msg_t {
	uint8_t	source_id;
	uint8_t	msg_id;
	uint8_t	buf_len;
	uint8_t	Resv;
...
void btmtk_dev_link_uart(void)
{
	...
	struct platform_device *btmtk_uart_device = btmtk_get_uart_device();
...
int btmtk_wakeup_uarthub(void) {
	#define BTMTK_WAKEUP_UARTHUB_CNT	50
	int ready_retry = BTMTK_WAKEUP_UARTHUB_CNT, ret = 0;
	struct btmtk_uart_dev *cif_dev = NULL;
	...
	while (mtk8250_uart_hub_is_ready() <= 0 && --ready_retry) {
		if (ready_retry < BTMTK_WAKEUP_UARTHUB_CNT - 3)
...
void btmtk_release_uarthub(bool force)
{
	...
	if (cif_dev->sleep_flow_hw_mech_en && cif_dev->sleep_en) {
		...
		BTMTK_DBG("%s: Clear DEV0_HOST_AWAKE ret[%d]", __func__, ret);
	}
...
void btmtk_sp_coredump_end(void)
{
	...
	if(g_sbdev->collect_fwdump)
		ret = connv3_coredump_end(g_sbdev->main_info.hif_hook.coredump_handler, g_sbdev->assert_reason);
	...
	btmtk_assert_wake_unlock(g_sbdev);
...
int btmtk_set_gpio_default(void)
{
	...
	if (!g_sbdev->main_info.find_my_phone_mode) {
...
int btmtk_sp_close(void)
{
	...
	if (atomic_read(&g_sbdev->assert_state) == BTMTK_ASSERT_START) {
		...
		if (!wait_for_completion_timeout(&g_sbdev->dump_comp, msecs_to_jiffies(WAIT_FW_DUMP_TIMEOUT))) {
			...
			/* trigger hif dump */
			if (g_sbdev->main_info.hif_hook.dump_hif_debug_sop) {
				g_sbdev->main_info.hif_hook.dump_hif_debug_sop(g_sbdev);
	...
	/* if uart already disconnect, do not use tty related API */
	if (btmtk_get_chip_state(g_sbdev) != BTMTK_STATE_DISCONNECT && cif_dev->tty != NULL) {
...
static int btmtk_pre_chip_rst_handler(enum connv3_drv_type drv, char *reason, unsigned int reset_type)
{
	...
	if (g_sbdev->main_info.hif_hook.trigger_assert == NULL) {
	...
	g_sbdev->main_info.reset_stack_flag = HW_ERR_CODE_CHIP_RESET;
	...
	atomic_set(&g_sbdev->main_info.chip_reset, BTMTK_RESET_DOING);
	g_sbdev->main_info.hif_hook.trigger_assert(g_sbdev);

	if (reset_type == CONNV3_CHIP_RST_TYPE_DFD_DUMP) {
		...
		if (g_sbdev->dfd_value_addr_remap_base == NULL) {
			...
			ret = connv3_coredump_send(g_sbdev->main_info.hif_hook.coredump_handler, "PRED", temp_buf,
					g_sbdev->dfd_value_size);
			iounmap(g_sbdev->dfd_value_addr_remap_base);
			if (ret)
				BTMTK_ERR("%s: connv3_coredump_send failed, ret = %d", __func__, ret);
		}
		/* for the latter bt_close */
...
{
	BTMTK_INFO("%s: state[%d] is_whole_chip_reset[%d], reset_type[%d]", __func__,
				g_bt_state, g_sbdev->is_whole_chip_reset, g_sbdev->reset_type);
	...
	//atomic_set(&g_sbdev->main_info.chip_reset, BTMTK_RESET_DONE);
...
static int btmtk_post_chip_rst_on_handler(unsigned int type)
{
	...
	if (g_sbdev->reset_type == CONNV3_CHIP_RST_TYPE_DFD_DUMP) {
		...
		ret = g_sbdev->main_info.hif_hook.pre_open(g_sbdev);
		if (ret != 0) {
		...
		for (int region = 1; region <= 5; region++) {
			...
			while (total_length >= 0) {
				...
				/* dump DFD post data */
				ret = btmtk_main_send_cmd(g_sbdev, cmd, sizeof(cmd), evt, sizeof(evt), DELAY_TIMES,
		...
		uart_cfg.fc = UART_DISABLE_FC;
		uart_cfg.iBaudrate = BT_UART_DEFAULT_BAUD;
		ret = btmtk_uart_set_ap_fw_uart_cfg(g_sbdev, &uart_cfg);
		if (ret < 0)
			return ret;
	}
 ...
 *******************************************************************************
 */
int btmtk_dump_start(void *priv_data, unsigned int force_dump, enum dump_type dump_type){
	...
	BTMTK_INFO("%s start, dump_type[%d]", __func__, dump_type);
	...
	if (!force_dump && atomic_read(&cif_dev->own_state) != BTMTK_DRV_OWN) {
		BTMTK_WARN("%s: not in drv own, force_dump[%d], own_state[%d]",
			__func__, force_dump, atomic_read(&cif_dev->own_state));
		return -1;
	}
...
{
	return btmtk_dump_start(priv_data, TRUE, DUMP_TYPE_HIF);
}
...
int btmtk_power_dump_start(void *priv_data, unsigned int force_dump)
{
	return btmtk_dump_start(priv_data, force_dump, DUMP_TYPE_POWER);
}

int btmtk_power_dump_end(void *priv_data)
{
	return btmtk_dump_end(priv_data);
}
...
struct connv3_power_dump_cb btmtk_pwr_dump_cb = {
	.power_dump_start = btmtk_power_dump_start,
	.power_dump_end = btmtk_power_dump_end,
};
...
static int btmtk_send_connfem_new(struct btmtk_dev *bdev)
{
	...
	BTMTK_DBG_RAW(bdev, cmd, offset, "%s: Send: ", __func__);
...
static irqreturn_t btmtk_host_wake_isr(int irq, void *dev)
{
	...
	/* 0 value for wake up*/
	if (!host_wake)
		g_sbdev->main_info.hif_hook.wakeup_host(bdev);
...
void btmtk_uart_wakeup_cb(void *para)
{
	struct btmtk_dev *bdev = (struct btmtk_dev *)para;
	...
	if (bdev->main_info.hif_hook.wakeup_host)
		bdev->main_info.hif_hook.wakeup_host(bdev);
	else
		BTMTK_WARN_DEV(bdev, "%s: main_info.hif_hook.wakeup_host is NULL", __func__);
}

/*
 *******************************************************************************
 *			 bt notify fw system suspned/resume state feature
 *******************************************************************************
 */
int btmtk_intcmd_system_status(unsigned char system_state)
{
	 ...
	 *		0x01 vendor specific event
	 *		0x02 no event
	 */
	u8 cmd[] = { 0x01, 0x5D, 0xFC, 0x03, 0x02, 0x05, 0x00};
	// u8 evt[] = {0x04, 0x0E, 0x07, 0x01, 0x5D, 0xFC};
	int ret = 0;

	BTMTK_INFO("%s: system_state[%d]", __func__, system_state);
	...
	ret = btmtk_main_send_cmd(g_sbdev,
			...
			DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_SEND_DIRECT, CMD_NO_NEED_FILTER);

	if (ret < 0)
		BTMTK_WARN("%s fail to set system_state to fw", __func__);

	return ret;
}

void btmtk_system_state_work(struct work_struct *work)
{

	BTMTK_INFO("%s: chip_state[%d]", __func__, btmtk_get_chip_state(g_sbdev));

#if (USE_DEVICE_NODE == 1)
	if (btmtk_get_chip_state(g_sbdev) == BTMTK_STATE_WORKING) {
#if (CFG_SUPPORT_FWLOG_OFF_WHEN_SUSPEND == 1)
		/* notify fw system resume */
		btmtk_intcmd_system_status(WMT_PARA_RESUME);
...
int btmtk_disp_notify_cb(struct notifier_block *nb, unsigned long value, void *v)
{
	...
	BTMTK_INFO("%s: before_after_blank[%ld], blank_power_down[%d], fops_state[%d], fw_log_on[%d]",
				__func__, value, *data, btmtk_fops_get_state(g_sbdev), g_sbdev->main_info.fw_log_on);
	...
	if (value == MTK_DISP_EARLY_EVENT_BLANK) {
		...
		if (btmtk_fops_get_state(g_sbdev) == BTMTK_FOPS_STATE_OPENED && g_sbdev->main_info.fw_log_on)
...
#if (CFG_SUPPORT_MD_CCCI == 1)
void btmtk_ccci_notify_md_work(struct work_struct *work)
{
	...
	if (g_sbdev == NULL) {
		BTMTK_WARN("%s: g_sbdev is NULL", __func__);
		return;
	}

	port_id = mtk_ccci_request_port("ccci_idc_bt");

	if (port_id < 0) {
		BTMTK_WARN("%s: mtk_ccci_request_port 'ccci_idc_bt' not found", __func__);
		return;
	}

	/* fill msg */
	idc_ccci_ctrl_msg.source_id = 1;	/* wifi: 0, bt: 1 */
	idc_ccci_ctrl_msg.msg_id = 0;
	idc_ccci_ctrl_msg.buf_len = 1;
	idc_ccci_ctrl_msg.buf[0] = atomic_read(&g_sbdev->host_bt_state);

	ret = mtk_ccci_send_data(port_id, (char *)&idc_ccci_ctrl_msg, sizeof(idc_ccci_ctrl_msg));

	if (ret < 0) {
		bool result = schedule_delayed_work(&g_sbdev->ccci_md_work, msecs_to_jiffies(1000));
		BTMTK_WARN("%s: mtk_ccci_send_data on_off[%d] fail, retry result[%d]",
			__func__, atomic_read(&g_sbdev->host_bt_state), result);
	} else
		BTMTK_INFO("%s: mtk_ccci_send_data on_off[%d] OK", __func__, atomic_read(&g_sbdev->host_bt_state));

	ret = mtk_ccci_release_port(port_id);
	if (ret < 0)
		BTMTK_WARN("%s: mtk_ccci_release_port fail", __func__);
}
#endif // CFG_SUPPORT_MD_CCCI

void btmtk_sp_worker_init(struct btmtk_dev *bdev)
{
	if (!bdev) {
		BTMTK_ERR("%s: bdev is NULL", __func__);
		return;
	...
	INIT_WORK(&bdev->system_state_work, btmtk_system_state_work);
	INIT_WORK(&bdev->hif_dump_work, btmtk_hif_dump_work);
#if (CFG_SUPPORT_MD_CCCI == 1)
	INIT_DELAYED_WORK(&bdev->ccci_md_work, btmtk_ccci_notify_md_work);
#endif
}
...
int btmtk_connv3_sub_drv_init(struct btmtk_dev *bdev)
{
	...
	atomic_set(&bdev->meta_tst_reset_mode, 0);
	bdev->dfd_value_addr_remap_base = NULL;
	tty->dev->of_node = of_find_node_by_name(NULL, "connscp");
	if (!tty->dev->of_node)
		BTMTK_ERR("[ERR] %s: connscp of_node not found", __func__);

	ret = of_property_read_u64(tty->dev->of_node, "dfd-value-addr", &bdev->dfd_value_addr);
	if (ret < 0)
		BTMTK_ERR("[ERR] %s: connscp dfd-value-addr ret[%d]", __func__, ret);
	...
	if (ret < 0)
		BTMTK_ERR("[ERR] %s: connscp dfd-value-size ret[%d]", __func__, ret);
	BTMTK_INFO("%s: dfd-value-size =  0x%04x", __func__, bdev->dfd_value_size);

	tty->dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,bt-coredump-reserve-memory");
	if (!tty->dev->of_node) {
		BTMTK_ERR("[ERR] %s: mediatek,bt-coredump-reserve-memory of_node not found", __func__);

	} else {
		rmem = of_reserved_mem_lookup(tty->dev->of_node);
		if (!rmem) {
			BTMTK_ERR("[ERR] %s: of_reserved_mem_lookup failed.", __func__);
		} else {
			cif_dev->base = rmem->base;
			cif_dev->size = rmem->size;
			BTMTK_INFO("%s: Coredump EMI base=%llx, size=0x%zx", __func__, cif_dev->base, cif_dev->size);
		}
	}

	/*
	tty->dev->of_node = of_parse_phandle(tty->dev->of_node, "memory-region", 0);
	if (!tty->dev->of_node)
		BTMTK_ERR("[ERR] %s: memory-region not found", __func__);
	*/
...
#if defined(CFG_UART_WAKEUP_SUPPORT)
		ret = mtk8250_wakeup_callback_register(btmtk_uart_wakeup_cb, bdev);
		BTMTK_INFO("%s mtk8250_wakeup_callback_register ret[%d]", __func__, ret);
		cif_dev->uart_wakeup_cb_en = 1;
#endif // CFG_UART_WAKEUP_SUPPORT
	}
...
void btmtk_pwr_on_uds_work(struct work_struct *work)
{
	...
	if (btmtk_fops_get_state(g_sbdev) == BTMTK_FOPS_STATE_OPENED) {
		BTMTK_WARN("%s: fops is already opened", __func__);
		return;
	}
...
{
	u8 cmd[] = { 0x01, 0x64, 0xFD, 0x02, 0x01, 0x01 };
	//u8 evt[] = { 0x04, 0x0E, 0x05, 0x01, 0x64, 0xFD, 0x00 , 0x01};
	...
	g_sbdev->fmd_reset_state = FMD_RESET_STATE_START;

	/* this cmd would trigger hci reset(dynamic download) flow, so not use send_anr_recv wait event */
	ret = btmtk_main_send_cmd(g_sbdev, cmd, sizeof(cmd), NULL, 0,
		DELAY_TIMES, RETRY_TIMES, BTMTK_TX_PKT_SEND_NO_ASSERT, CMD_NO_NEED_FILTER);
	if (ret < 0) {
	...
	/* wait event back because fw need do hci reset (dynamic download) */
	while (g_sbdev->fmd_reset_state == FMD_RESET_STATE_START && retry) {
		BTMTK_INFO("%s: wait for fmd_reset_state retry[%d]", __func__, retry);
		msleep(300);
		retry--;
	}
	...
	/* timeout */
	if (retry == 0) {
		BTMTK_ERR("%s: wait for fmd_reset_state timeout", __func__);
		/* reset state */
		g_sbdev->fmd_reset_state = FMD_RESET_STATE_INIT;
	}
...
int btmtk_uart_launcher_deinit(void)
{
	...
	if (g_sbdev != NULL) {
		...
		cif_dev = (struct btmtk_uart_dev *)g_sbdev->cif_dev;
		if (cif_dev && cif_dev->tty && cif_dev->tty->port && cif_dev->tty->port->count == 0)
			cif_dev->tty->port->count = 1;
	}
	/* set disconnect to avoid android BT open/close driver */
