...
static uint8_t apucRoamingReasonToLog[ROAMING_REASON_NUM] = {
	/* map to ROAMING_REASON_POOR_RCPI(0) */
	ROAMING_CATEGORY_LOW_RSSI,
	/* map to ROAMING_REASON_TX_ERR(1) */
	ROAMING_CATEGORY_UNSPECIFIC,
	/* map to ROAMING_REASON_TX_ERR(1) */
	ROAMING_CATEGORY_UNSPECIFIC,
	/* map to ROAMING_REASON_RETRY(2) */
	ROAMING_CATEGORY_IDLE,
	/* map to ROAMING_REASON_IDLE(3) */
	ROAMING_CATEGORY_HIGH_CU,
	/* map to ROAMING_REASON_HIGH_CU(4)*/
	ROAMING_CATEGORY_BTCOEX,
	/* map to ROAMING_REASON_BT_COEX(5) */
	ROAMING_CATEGORY_BEACON_LOST,
	/* map to ROAMING_REASON_BEACON_TIMEOUT(6) */
	ROAMING_CATEGORY_UNSPECIFIC,
	/* map to ROAMING_REASON_INACTIVE(7) */
	ROAMING_CATEGORY_EMERGENCY,
	/* map to ROAMING_REASON_SAA_FAIL(8) */
	ROAMING_CATEGORY_UNSPECIFIC,
	/* map to ROAMING_REASON_UPPER_LAYER_TRIGGER(9) */
	ROAMING_CATEGORY_BTM_REQ,
	/* map to ROAMING_REASON_BTM(10) */
	ROAMING_CATEGORY_SCAN_TIMER,
	/* map to ROAMING_REASON_SCAN_TIMER(11) */
	ROAMING_CATEGORY_INACTIVE_TIMER,
	/* map to ROAMING_REASON_INACTIVE_TIMER(12) */
};
#if 0
	...
	const uint8_t *bssid)
{
	...
	if (!fgIsMldAp) {
		kalSprintf(log,
			"[BTM] RESP token=%d status=%d delay=%d target="
			RPTMACSTR,
			dialogToken, status, delay,
			RPTMAC2STR(bssid ? bssid : aucZeroMacAddr));
	} else {
		kalSprintf(log,
			"[BTM] RESP band=%s token=%d status=%d delay=%d target="
			RPTMACSTR,
			apucBandStr[prBssInfo->eBand],
			dialogToken, status, delay,
			RPTMAC2STR(bssid ? bssid : aucZeroMacAddr));
	...
	struct BSS_DESC *prBssDesc)
{
	...
	if (roamingFsmIsDiscovering(prAdapter, ucBssIndex) &&
		...
		if (prRoamInfo->eReason == ROAMING_REASON_POOR_RCPI ||
			...
			kalSprintf(aucFwTime, " [fw_time=%u]",
				prRoamInfo->u4RoamingFwTime*1000);
	...
	uint8_t ucBssIndex)
{
	...
	for (ucIdx = 0; (ucIdx < prScanInfo->ucSparseChannelArrayValidNum &&
		...
		prWifiVar->ucPos += kalSnprintf(prWifiVar->rScanDoneFreqLog +
				prWifiVar->ucPos,
	...
	struct BSS_DESC *prSelectedBssDesc)
{
	...
	if (fgIsRoam) {
		...
		kalSprintf(aucLog,
			"[ROAM] RESULT %s [reason=%d no candidate]",
			"NO_ROAM", ucReason);
	...
	uint8_t ucSn)
{
	...
	} else if (ucEapCode == 3 || ucEapCode == 4) {
		kalSprintf(log,
			"[EAP] %s",
			apucEapCode[ucEapCode]);
		kalBufferWifiLog(prAdapter,
	...
	uint16_t u2RxStatusCode)
{
	...
	if (IS_STA_IN_AIS(prAdapter, prStaRec) &&
		...
		if (!fgIsMldAp) {
			...
			kalSprintf(log + strlen(log),
				" RESP bssid=" RPTMACSTR
				" sn=%d status=%d assoc_id=%d mld_mac="
				RPTMACSTR,
				RPTMAC2STR(prStaRec->aucMacAddr),
				(int)WLAN_GET_SEQ_SEQ(
					prAssocRspFrame->u2SeqCtrl),
	...
	enum ENUM_TX_RESULT_CODE rTxDoneStatus)
{
	...
	if (IS_STA_IN_AIS(prAdapter, prStaRec) && prBssDesc) {
		...
		if (!fgIsMldAp)
			...
			set = aisGetSearchResult(prAdapter,
					prMsduInfo->ucBssIndex);
			if (set) {
				for (i = MLD_LINK_MAX; i > 0; i--) {
					if (set->aprBssDescW[i-1]) {
						eBand = set->
						aprBssDescW[i-1]->eBand;
						kalSprintf(buf +
							strlen(buf), "%s+",
							apucBandStr[eBand]);
					}
...
{
	switch (reason) {
	case MLO_LINK_STATE_CHANGE_REASON_DEFAULT:
		return "default";
	case MLO_LINK_STATE_CHANGE_REASON_TEST:
		return "test";
	case MLO_LINK_STATE_CHANGE_REASON_T2LM:
		return "T2LM";
	case MLO_LINK_STATE_CHANGE_REASON_EMLSR:
		return "EMLSR";
	case MLO_LINK_STATE_CHANGE_REASON_LINK_RECOMMEND:
		return "link recommed";
	case MLO_LINK_STATE_CHANGE_REASON_RCPI:
		return "rssi";
	case MLO_LINK_STATE_CHANGE_REASON_COEX:
		return "coex";
	case MLO_LINK_STATE_CHANGE_REASON_CONCURRENT:
		return "concurrent";
	case MLO_LINK_STATE_CHANGE_REASON_TPUT_HIGH:
		return "tput high";
	case MLO_LINK_STATE_CHANGE_REASON_TPUT_LOW:
		return "tput low";
	default: return "unknown";
	}
}
	...
	struct STA_RECORD *prStaRec)
{
	...
	if (prStaRec->ucDLTidBitmap == 0)
		...
		for (i = 0; i < MAX_NUM_T2LM_TIDS; i++) {
			...
			if (ucBitmap)
				kalSprintf(dl_tidlog +
					strlen(dl_tidlog), "%d,", i);
		}
		if (strlen(dl_tidlog) > 1)
			dl_tidlog[strlen(dl_tidlog) - 1] = '\0';
		kalSprintf(log + strlen(log),
			" tid_dl=%s", dl_tidlog);
	}
	...
	if (prStaRec->ucULTidBitmap == 0)
		...
		for (i = 0; i < MAX_NUM_T2LM_TIDS; i++) {
			...
			if (ucBitmap)
				kalSprintf(ul_tidlog +
				strlen(ul_tidlog), "%d,", i);
		}
		if (strlen(ul_tidlog) > 1)
			ul_tidlog[strlen(ul_tidlog) - 1] = '\0';
		kalSprintf(log + strlen(log),
			" tid_ul=%s", ul_tidlog);
	}
