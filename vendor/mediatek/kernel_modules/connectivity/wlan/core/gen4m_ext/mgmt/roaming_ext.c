	...
	if ((prAisBssInfo->eConnectionState == MEDIA_STATE_CONNECTED ||
		...
		case ROAMING_REASON_RETRY:
		{
			if (ad->rNchoInfo.fgNCHOEnabled &&
			    r2 - r1 <= ad->rNchoInfo.i4RoamDelta) {
				DBGLOG(APS, WARN,
					MACSTR " low rssi %d - %d <= %d\n",
					MAC2STR(bss->aucBSSID), r2, r1,
					ad->rNchoInfo.i4RoamDelta);
				return FALSE;
			}
			break;
		}
		...
		r2 = RCPI_TO_dBm(bss->ucRCPI);
		if (r2 < ad->rWifiVar.cConMinRssi) {
			DBGLOG(APS, WARN, MACSTR " low rssi %d < %d\n",
				MAC2STR(bss->aucBSSID),
				r2, ad->rWifiVar.cConMinRssi);
			return FALSE;
		}
	}
			  ...
			  uint32_t *pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(struct CFG_NCHO_SCAN_CHNL);
	...
	if ((prAdapter->rNchoInfo.rRoamScnChnl.ucChannelListNum +
			prRoamScnChnl->ucChannelListNum) >=
			MAXIMUM_OPERATION_CHANNEL_LIST) {
		DBGLOG(INIT, WARN, "NCHO rRoamScnChnl number exceeded %d\n",
			MAXIMUM_OPERATION_CHANNEL_LIST);
		return WLAN_STATUS_FAILURE;
	}

	for (uint8_t i = 0; i < prRoamScnChnl->ucChannelListNum;
		i++) {
		uint8_t fgDup = FALSE;
		for (uint8_t j = 0;
			j < prAdapter->rNchoInfo.rRoamScnChnl.
				ucChannelListNum;
				j++) {
			if ((prRoamScnChnl->arChnlInfoList[i].ucChannelNum ==
				prAdapter->rNchoInfo.rRoamScnChnl.
				arChnlInfoList[j].ucChannelNum) &&
				(prRoamScnChnl->arChnlInfoList[i].eBand ==
				prAdapter->rNchoInfo.rRoamScnChnl.
				arChnlInfoList[j].eBand)) {
				fgDup = TRUE;
				break;
			}
		}
		if (fgDup == FALSE) {
			uint8_t ucIdx = prAdapter->rNchoInfo.
				rRoamScnChnl.ucChannelListNum;
			prAdapter->rNchoInfo.rRoamScnChnl.
				arChnlInfoList[ucIdx] =
...
wlanoidSetNchoRoamScnCtrl(struct ADAPTER *prAdapter,
	...
	DBGLOG(INIT, TRACE,
		"SETROAMSCANCONTROL is deprecated from Android 13\n");
...
wlanoidQueryNchoRoamScnCtrl(struct ADAPTER *prAdapter,
			    ...
			    uint32_t *pu4QueryInfoLen) {
	DBGLOG(INIT, TRACE,
		"GETROAMSCANCONTROL is deprecated from Android 13\n");
	return WLAN_STATUS_NOT_SUPPORTED;
}
	...
	uint8_t ucBssIndex)
{
	...
	if (prBtmParam->ucIsCisco && prBtmParam->ucVsieReasonCode == 0x00) {
		...
		/*
		* TODO: the disassociation frame reason code
		* must be set to 0X0C
		*/
	...
	uint8_t ucBssIndex)
{
	...
	DBGLOG(ROAMING, INFO,
		"Start roaming scan timer, reason[%u] source[%u] timeout[%u]\n",
	...
	uint8_t ucBssIndex)
{
	...
	if (totalDiffPkts >=
		prAdapter->rWifiVar.ucRSInactiveCount *
			MSEC_TO_SEC(rPeriod)) {
