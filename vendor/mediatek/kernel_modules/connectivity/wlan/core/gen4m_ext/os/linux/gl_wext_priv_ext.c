...
struct CMD_VALIDATE_POLICY
	set_ext_policy[COMMON_CMD_SET_ARG_NUM(2)] = {
...
struct CMD_VALIDATE_POLICY
	...
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U32,
		.min = 0,
		.max = SAP_RPS_IPKT_MAX
	},
	[COMMON_CMD_ATTR_IDX(2)] = {
		.type = NLA_U8,
		.min = SAP_RPS_PHASE_MIN,
		.max = SAP_RPS_PHASE_MAX
	},
	[COMMON_CMD_ATTR_IDX(3)] = {
		.type = NLA_U32,
		.min = 0,
		.max = SAP_RPS_THREHOLD_MAX
	},
...
struct CMD_VALIDATE_POLICY
	sap_sus_policy[COMMON_CMD_SET_ARG_NUM(3)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U8,
		.min = 0,
		.max = SAP_RPS_ENABLE_TRUE
...
/* P2P Command Policy */
struct CMD_VALIDATE_POLICY
	set_ecsa_policy[COMMON_CMD_SET_ARG_NUM(3)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U8,
		.min = 0,
		.max = U8_MAX
...
struct CMD_VALIDATE_POLICY
	u32_ext_policy[COMMON_CMD_SET_ARG_NUM(7)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U32,
		.min = 0,
		.max = U32_MAX
...
struct CMD_VALIDATE_POLICY
	u8_ext_policy[COMMON_CMD_SET_ARG_NUM(7)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U8,
		.min = 0,
		.max = U8_MAX
...
struct CMD_VALIDATE_POLICY
	sta_info_policy[COMMON_CMD_SET_ARG_NUM(2)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_STRING,
		.min = 3,
		.max = 17
...
#if CFG_SUPPORT_BW_SELECT
struct CMD_VALIDATE_POLICY
	set_max_bw_policy[COMMON_CMD_SET_ARG_NUM(3)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U8,
		.min = 1,
		.max = 1
...
struct CMD_VALIDATE_POLICY
	reassoc_ext_policy[COMMON_CMD_SET_ARG_NUM(4)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_STRING,
		.len = 17
	},
...
struct CMD_VALIDATE_POLICY
	set_roam_trigger_ext_policy[COMMON_CMD_SET_ARG_NUM(2)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_S8,
		.min = -100,
		.max = -50
...
struct CMD_VALIDATE_POLICY
	set_wtc_mode_policy[COMMON_CMD_SET_ARG_NUM(7)] = {
	[COMMON_CMD_ATTR_IDX(1)] = {
		.type = NLA_U8,
		.min = 0,
		.max = U8_MAX
...
invalid_rate:
	DBGLOG(REQ, ERROR,
		"BAD:preamble=%u nss=%u bw=%u mcs=%u ofdm=%u cck=%u\n",
		...
		for (j = 0; j < STATS_LLS_RATE_NUM; j++, src_rate++) {
			...
			if (src_rate->tx_mpdu || src_rate->mpdu_lost ||
				...
				prAdapter->rQueryStaInfo.u2RxRate[
						src_rate->rate.rateMcsIdx] +=
					p2pFuncReceivedMpduCount(sta_rec,
						src_rate,
						ofdm_idx >= 0 ? ofdm_idx : 0,
						cck_idx >= 0 ? cck_idx : 0);
				DBGLOG(REQ, WARN,
					"tx_count[%u]=%u, rx_count[%u]=%u\n",
					src_rate->rate.rateMcsIdx,
					prAdapter->rQueryStaInfo.u2TxRate[
						src_rate->rate.rateMcsIdx],
					src_rate->rate.rateMcsIdx,
				...
				prAdapter->rQueryStaInfo.u2RxPkts +=
					prAdapter->rQueryStaInfo.u2RxRate[
						src_rate->rate.rateMcsIdx];
				DBGLOG(REQ, WARN,
					"tx_pkts=%u tx_fails=%u tx_retries=%u rx_pkts=%u\n",
					prAdapter->rQueryStaInfo.u2TxPkts,
					prAdapter->rQueryStaInfo.u2TxFailPkts,
					prAdapter->rQueryStaInfo.u2TxRetryPkts,
					prAdapter->rQueryStaInfo.u2RxPkts);
			...
			uint32_t *pu4SetInfoLen)
{
	...
	index += kalSprintf(pvSetBuffer + index, "%02x,%02x,%02x,%02x,%02x,%02x ",
...
static uint32_t __get_sta_trx_info_query_stats(
		...
		struct PARAM_GET_STA_TRX_INFO *prParam)
{
	...
	u4Rate = rLinkSpeed.rLq[
		prParam->ucBssidx].u2TxLinkSpeed;
...
#define ERROR_NO_SAP_CLIENTS	"No connection on SAP"
	...
	*/
	...
	if (UNEQUAL_MAC_ADDR(aucZeroMacAddr, prParam->aucTargetMac))
		/*
		* cap, oui, frequency, bandwidth, rssi,
		* data_rate, 11_mode, antenna, mu_mimo,
		* reason, supported_band, avg_rssi
		*/
		i4Written += kalSnprintf(prRespBuf + i4Written,
					 u4RespBufSz - i4Written,
...
int __priv_driver_get_sta_trx_info(
		struct net_device *prNetDev,
	...
	int i4TotalLen)
{
	...
	if (IS_BSS_APGO(bss))
		u4Ret = cnmIdcCsaReq(prGlueInfo->prAdapter,
			eBand, ch, MODE_DISALLOW_TX, bw, ucRoleIdx);
	else
	...
	int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		i4BytesWritten = kstrtos32(apcArgv[1], 0, &i4Type);

		if (i4BytesWritten)
			DBGLOG(REQ, ERROR, "parse i4Type error %d\n",
					i4BytesWritten);

#if (CFG_SUPPORT_WIFI_6G == 1)
		if (i4Type == -1 || i4Type == 1) {
			DBGLOG(REQ, INFO,
				"Enanble 6G cap, i4Type: %d , 6g disallow option: %d\n",
				i4Type, prAdapter->rWifiVar.ucDisallowBand6G);
...
#endif
		} else if (i4Type == 0 && prAdapter->fgEnRfTestMode == FALSE) {
			DBGLOG(REQ, INFO,
				"Disable 6G cap, i4Type: %d, 6g disallow option: %d\n",
				i4Type, prAdapter->rWifiVar.ucDisallowBand6G);
		...
		if (i4Type == 2 || i4Type == -1) {
			/* Disable */
			kalSetPowerLimitByIoCtrl(prGlueInfo, 0, "FCC_CHANNEL", NULL);
		} else if (i4Type == 1 || i4Type == 0) {
			/* Enable */
			kalSetPowerLimitByIoCtrl(prGlueInfo, 1, "FCC_CHANNEL", NULL);
		}
		/*
		* Update supported channel list in channel
		* table based on current country domain
		*/
	...
	int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		if (ucType == 1) {
			...
			rStatus = rsnSetCountryCodefor6g(
			...
			if (prGlueInfo->prAdapter->fgEnRfTestMode == FALSE) {
				prGlueInfo->prAdapter->fg6eOffSpecNotShow
					= TRUE;
				DBGLOG(RSN, INFO,
					"Filter out out-of-spec 6G AP\n");
			}
		}
	...
	struct wireless_dev *wdev, char *pcCommand, int i4TotalLen)
{
	...
	* wlanCfgParseArgument, we should copy
	* original pcCommand first.
	*/
	...
	int i4TotalLen)
{
	...
	if (u4Ret)
		DBGLOG(REQ, LOUD,
			"parse u4SapThreshldTime error u4Ret=%d\n", u4Ret);
	...
	uint8_t ucBssIndex)
{
	...
	for (i = 0; i < 16; i++) {
		if (linkID[i] && strlen(buf) == 0)
			LOGBUF(pucBuf, sizeof(buf),
				i4WrittenByte, "%d:%d", i, linkCU[i]);
	...
	int i4TotalLen)
{
	...
	prRoamScnChnl =
		kalMemAlloc(sizeof(struct CFG_NCHO_SCAN_CHNL),
			VIR_MEM_TYPE);
