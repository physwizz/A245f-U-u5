				...
				char *pcCommand, int i4TotalLen)
{
	...
	int8_t *apcArgv[WLAN_CFG_ARGV_MAX] = {0};
	uint8_t ch_num = 0;
	uint8_t ucMode = MODE_DISALLOW_TX;
	uint32_t u4Ret = 0;
	uint8_t ucBw = MAX_BW_NUM;
	uint8_t ucRoleIdx = 0, ucBssIdx = 0;
	enum ENUM_BAND eBand = BAND_NULL;
	int32_t i4BytesWritten = 0;
	...
	prGlueInfo = *((struct GLUE_INFO **) netdev_priv(prNetDev));
	if (mtk_Netdev_To_RoleIdx(prGlueInfo, prNetDev, &ucRoleIdx) != 0 ||
	    p2pFuncRoleToBssIdx(prGlueInfo->prAdapter, ucRoleIdx,
				&ucBssIdx) != WLAN_STATUS_SUCCESS) {
		DBGLOG(REQ, WARN, "Invalid interface\n");
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		       "Invalid interface\n");
		goto error;
	}
	...
	bss = GET_BSS_INFO_BY_INDEX(prGlueInfo->prAdapter, ucBssIdx);
	if (!IS_BSS_APGO(bss)) {
		DBGLOG(REQ, WARN, "Incorrect bss opmode\n");
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		       "Incorrect bss opmode\n");
		goto error;
	}

	if (kalkStrtou8(apcArgv[1], 0, (uint8_t *)&eBand) ||
	    kalkStrtou8(apcArgv[2], 0, &ch_num))
		goto error;

	for (u4Idx = 3; u4Idx < i4Argc; u4Idx++) {
		if (kalStrStr(apcArgv[u4Idx], "mode=")) {
			if (kalkStrtou8(apcArgv[u4Idx] + 5, 0, &ucMode) != 0 ||
			    ucMode >= MODE_NUM)
				goto error;
		} else if (kalStrStr(apcArgv[u4Idx], "bw=")) {
			if (kalkStrtou8(apcArgv[u4Idx] + 3, 0, &ucBw) != 0 ||
			    ucBw >= MAX_BW_NUM)
				goto error;
		}
	}
	...
	if (rStatus != CSA_STATUS_SUCCESS)
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		       "\nRunning CSA, but NOT RECOMMENDED\n"
		       "which may cause to disconnect or violate regulations.\n");
	...
	u4Ret = cnmIdcCsaReq(prGlueInfo->prAdapter,
		eBand, ch_num, ucMode, ucBw, ucRoleIdx);
#else
...
error:
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
	       "Command template:\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
	       "iwpriv <interface> driver 'csa_ex <band> <channel> [mode=<mode>] [bw=<bandwidth>]'\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
	       "    <band> - 1: 2.4G, 2: 5G, 3: 6G\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
	       "    <mode> - 0: non-blocking TX mode, 1: blocking TX mode\n");
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
...
uint8_t test_passthrough[5][100] = {
	/*_googlecast.tcp.local1 */
	{0x0b, 0x5f, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65,
	 0x63, 0x61, 0x73, 0x74, 0x03, 0x74, 0x63, 0x70,
	 0x06, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x31, 0x00},
	/*_googlecast.tcp.local2 */
	{0x0b, 0x5f, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65,
	 0x63, 0x61, 0x73, 0x74, 0x03, 0x74, 0x63, 0x70,
	 0x06, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x32, 0x00},

	/*_googlecast.tcp.local3 */
	{0x0b, 0x5f, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65,
	 0x63, 0x61, 0x73, 0x74, 0x03, 0x74, 0x63, 0x70,
	 0x06, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x33, 0x00},

	/*_googlezone._tcp.local4 */
	{0x0b, 0x5f, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65,
	 0x7a, 0x6f, 0x6e, 0x65, 0x04, 0x5f, 0x74, 0x63,
	 0x70, 0x06, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x34, 0x00},
	/*_googlezone._tcp.local5 */
	{0x0b, 0x5f, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65,
	 0x7a, 0x6f, 0x6e, 0x65, 0x04, 0x5f, 0x74, 0x63,
	 0x70, 0x06, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x35, 0x00}
};
		 ...
		 char *pcCommand,  int i4TotalLen)
{
	...
	/* The len is unlikely bigger than 99*/
	len = (j >= 100) ? 100 : (j+1);
	/* add record 1 */
	...
	prMdnsUplayerInfo->mdns_param.query[0].name_length = len;
...
#define MAX_BSS_DUMP_NUM 20
	...
	LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
		...
		prAdapter->rWifiVar.ucMlsrLinkWeight);
#endif
...
skip_mld_info:
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry,
		...
		for (i = *pu2BssNum; i > 0; i--) {
			if (prBssDesc->ucRCPI < aprBssDesc[i - 1]->ucRCPI)
				break;

			aprBssDesc[i] = aprBssDesc[i - 1];
		}

		aprBssDesc[i] = prBssDesc;
		(*pu2BssNum)++;
	}
	...
	GET_CURRENT_SYSTIME(&rCurrTime);

#if (CFG_SUPPORT_802_11BE_MLO == 1)
	if (fgFoundMldAp)
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			"\nBSSID / Freq / RSSI / Age / LinkId / MLD_ADDR\n");
	...
	for (i = 0; i < u2Bss2GNum; i++) {
		...
		LOGBUF(pcCommand, i4TotalLen, i4BytesWritten,
			""MACSTR"%s\t%d\t%d\t%ds\t%d\tMLD["MACSTR"]\n",
			MAC2STR(prBssDesc->aucBSSID),
			IS_AIS_CONN_BSSDESC(prAisFsmInfo, prBssDesc) ? "*" : "",
			nicChannelNum2Freq(prBssDesc->ucChannelNum,
				   prBssDesc->eBand) / 1000,
			RCPI_TO_dBm(prBssDesc->ucRCPI),
			SYSTIME_TO_SEC(rCurrTime - prBssDesc->rUpdateTime),
					...
					char *pcCommand, int i4TotalLen)
{
	...
	sStaAssocMetricsResp->iRssi =
		RCPI_TO_dBm((au4RxV[3] & RX_VT_RCPI0_MASK)
		>> RX_VT_RCPI0_OFFSET);
					...
					char *pcCommand, int i4TotalLen)
{
	...
	this_char++;
	DBGLOG(REQ, LOUD, "string = %s\n", this_char);
	u4MemSize = sizeof(struct PARAM_CUSTOM_CU_REP_REQ_STRUCT);
	prSetCuRepReqInfo = (struct PARAM_CUSTOM_CU_REP_REQ_STRUCT *)
			kalMemZAlloc(u4MemSize, VIR_MEM_TYPE);

	if (prSetCuRepReqInfo == NULL) {
		DBGLOG(INIT, ERROR, "alloc memory fail\n");
		i4BytesWritten = -1;
		goto exit;
	}

	i4BytesWritten = sscanf(this_char,
				"%u-%hhx:%hhx:%hhx:%hhx:%hhx:%hhx-%u-%u-%u-%u",
				&prSetCuRepReqInfo->ucUnitNum,
				&prSetCuRepReqInfo->aucPeerMac[0],
				&prSetCuRepReqInfo->aucPeerMac[1],
				&prSetCuRepReqInfo->aucPeerMac[2],
				&prSetCuRepReqInfo->aucPeerMac[3],
				&prSetCuRepReqInfo->aucPeerMac[4],
				&prSetCuRepReqInfo->aucPeerMac[5],
	...
	DBGLOG(REQ, TRACE,
		...
		prSetCuRepReqInfo->ucChanList[1],
		prSetCuRepReqInfo->ucChanList[2]);
	prSetCuRepReqInfo->ucBssIdx = ucBssIdx;

	/*4. Channel load request */
	rStatus = kalIoctl(prGlueInfo, wlanoidSendCuReportRequest,
			   prSetCuRepReqInfo,
			   u4MemSize,
			   &i4BytesWritten);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
exit:
	if (prSetCuRepReqInfo)
		kalMemFree(prSetCuRepReqInfo,
			   VIR_MEM_TYPE,
			   u4MemSize);
	return i4BytesWritten;
}
			...
			char *pcCommand, int i4TotalLen)
{
	...
	if (ucType == 1 /* mc */ || ucType == 2 /* az */) {
		...
		if (i4Argc > 5)
			u4Ret = kalkStrtou8(apcArgv[5], 0, &ucI2rLmrFeedback);

		/* 11az: Immeidate R2I feedback */
		if (i4Argc > 6)
			u4Ret = kalkStrtou8(apcArgv[6], 0, &ucImmeR2iFeedback);

		/* 11az: Immediate I2R feedback */
		if (i4Argc > 7)
			u4Ret = kalkStrtou8(apcArgv[7], 0, &ucImmeI2rFeedback);
...
error:
	...
	i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
				      i4TotalLen - i4BytesWritten,
				      "iwpriv <interface> driver 'p2p2_gc_csa band=<band> channel=<channel> [bw=<bandwidth>] [link=<link>]'\n");
	i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
				      i4TotalLen - i4BytesWritten,
				      "    <band> - 1: 2.4G, 2: 5G, 3: 6G\n");
	i4BytesWritten += kalSnprintf(pcCommand + i4BytesWritten,
