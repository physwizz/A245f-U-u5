...
static const char * const apucDebugReqType[AIS_REQUEST_NUM] = {
	"SCAN",
	"RECONNECT",
	"ROAMING_SEARCH",
	"ROAMING_CONNECT",
...
const uint8_t *apucMlrcActionStr[MLRC_ACTION_NUM + 1] = {
	"EMPTY",
	"NONE",
	"ADD_DEPAUSE",
	"DELETE_PAUSE",
...
const uint8_t *apucMlrcStateStr[MLRC_STATE_NUM + 1] = {
	"IDLE",
	"NEGO",
	"NEGO_FAIL",
	"RECONFIG",
...
#if (CFG_SUPPORT_ML_RECONFIG == 1)
static void aisFsmRunApRemovalTimeout(
	struct ADAPTER *prAdapter, uintptr_t ulParamPtr);
static void aisFsmRunPendingMlrcOpTimeout(
	struct ADAPTER *prAdapter, uintptr_t ulParamPtr);
...
void aisApplyLinkAddress(struct ADAPTER *prAdapter,
	struct AIS_FSM_INFO *prAisFsmInfo,
	uint8_t aucOwnMacAddr[], uint8_t ucLinkIdx)
{
	...
	if (!prAdapter->rWifiVar.ucMacAddrOverride ||
		...
		wlanHwAddrToBin(prAdapter->rWifiVar.aucMacAddrStr +
			ucLinkIdx * 18, aucOwnMacAddr);

		if (kalIsZeroEtherAddr(aucOwnMacAddr)) {
			DBGLOG(AIS, WARN,
		...
		DBGLOG(AIS, INFO, "link: %d, mac: " MACSTR "\n",
			ucLinkIdx, MAC2STR(aucOwnMacAddr));
	}
}
	...
	uint8_t ucLinkIdx)
{
	...
	aisApplyLinkAddress(prAdapter, prAisFsmInfo,
	...
	struct BSS_DESC_SET *prBssDescSet)
{
	...
	eLinkPlan = apsRfBandBmapToLinkPlan(prBssDescSet->ucRfBandBmap,
	...
	DBGLOG(AIS, INFO, "Add mld " MACSTR
	       ...
	       MAC2STR(prBssDesc->rMlInfo.aucMldAddr), eLinkPlan,
	       apsGetLinkPlanStr(eLinkPlan), prEntry->aucCount[eLinkPlan],
	       prAdapter->rWifiVar.ucAisMldTrialLimit, prEntry->u4BlockBmap);
	return prEntry;
}
...
#if (CFG_SUPPORT_ML_RECONFIG == 1)
	cnmTimerInitTimer(prAdapter,
		&prAisFsmInfo->rMlrcInfo.rApRemovalTimer,
		aisFsmRunApRemovalTimeout,
		(uintptr_t)ucBssIndex);
	cnmTimerInitTimer(prAdapter,
		&prAisFsmInfo->rMlrcInfo.rPendingMlrcOpTimer,
		aisFsmRunPendingMlrcOpTimeout,
		(uintptr_t)ucBssIndex);
	aisResetMlrcInfo(prAdapter, ucBssIndex);
	...
	uint8_t ucAisIndex)
{
	...
	if (prAisBssInfo &&
	    ...
	    !(IS_AIS_CONN_BSSDESC(prAisFsmInfo, prBss))) {
	...
	struct STA_RECORD *prStaRec, uint16_t u2ApRemovalTimer)
{
	...
	if (u4ApRemovalTime != 0 &&
	    !timerPendingTimer(&prAisFsmInfo->rMlrcInfo.rApRemovalTimer)) {
		DBGLOG(AIS, INFO,
	...
	uint8_t ucLinkIndex)
{
	...
	prBssDescSet = &prAisFsmInfo->rSearchResult;
	...
	if (mldSingleLink(prAdapter, prStaRec, ucBssIndex)) {
		struct BSS_DESC_W *w = prBssDescSet->aprBssDescW[ucLinkIndex];

		prStaRec->fgApRemoval = w->fgApRemoval;
...
fail:
	...
	aisFsmStateAbort(prAdapter, DISCONNECT_REASON_CODE_LOCALLY, FALSE,
...
{
	struct CONNECTION_SETTINGS *conn = aisGetConnSettings(
		prAdapter, ucBssIndex);

	if (conn->eConnectionPolicy == CONNECT_BY_BSSID_REUSE) {
		conn->eConnectionPolicy = CONNECT_BY_SSID_BEST_RSSI;
...
#if CFG_SUPPORT_ROAMING
		aisGetRoamingInfo(prAdapter, ucBssIndex)->eReason,
#else
		ROAMING_REASON_POOR_RCPI,
#endif
		ucBssIndex, set, FALSE);
}
...
uint8_t aisNeedTargetScan(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	discovering =
		...
		(roam->eCurrentState >= ROAMING_STATE_DISCOVERY &&
		 roam->eCurrentState < ROAMING_STATE_NUM);
	...
	if (prAdapter->rNchoInfo.u4RoamScanControl
		...
		issued = TRUE;
		DBGLOG(AIS, INFO, "Ncho mode, force to do partial scan\n");
	}
	...
	DBGLOG(AIS, INFO, "Discovering[%u] Postpone[%u] PartialScan[%u]\n",
			discovering, postponing, issued);

	return (discovering || postponing) && issued;
}
	...
	uint8_t ucBssIndex)
{
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		struct BSS_DESC *prBssDesc = prBssDescSet->aprBssDescW[i] ?
			prBssDescSet->aprBssDescW[i]->prBssDesc : NULL;
		...
		if (!prAisBssInfo) {
			prAisBssInfo = aisAllocBssInfo(prAdapter, prAisFsmInfo,
				i, !prBssDescSet->aprBssDescW[i]->fgUnSyncOm);
			if (!prAisBssInfo) {
	...
	struct BSS_DESC_SET *prBssDescSet)
{
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		for (j = 0; j < prBssDescSet->ucLinkNum; j++) {
			if (prBssDesc ==
			    prBssDescSet->aprBssDescW[j]->prBssDesc) {
				match++;
				break;
			}
		}
...
uint8_t aisJoinFailureOverLimit(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	if (ais->rJoinReqTime != 0 &&
	    CHECK_FOR_TIMEOUT(kalGetTimeTick(),
		ais->rJoinReqTime, SEC_TO_SYSTIME(AIS_JOIN_TIMEOUT))) {
		DBGLOG(AIS, WARN, "Join retry: start at %lu over %ds fail!\n",
			ais->rJoinReqTime,
			AIS_JOIN_TIMEOUT);
		return TRUE;
	}

	if (ais->ucConnTrialCount > prAdapter->rWifiVar.ucAisConnTrialLimit) {
		DBGLOG(AIS, WARN, "Join retry: %d/%d fail!\n",
...
void aisFsmClearPostponedBTO(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo;
...
enum ENUM_AIS_STATE aisSearchHandleBadBssDesc(struct ADAPTER *prAdapter,
	struct BSS_DESC_SET *prBssDescSet, uint8_t fgDisallowScan,
	uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *ais;
	...
	if (roamingFsmScheduleNextSearch(prAdapter, ucBssIndex)) {
		DBGLOG(AIS, INFO,
...
#else
	if (fgDisallowScan) {
		/* disallow to do more scan */
	} else if (ais->fgTargetChnlScanIssued ||
		   ais->ucScanTrialCount < roam->rRoamScanParam.ucScanCount) {
		...
		DBGLOG(AIS, INFO,
			"[Roaming] No target found(trial=%d/%d), try %s scan again\n",
			ais->ucScanTrialCount,
			roam->rRoamScanParam.ucScanCount,
			ais->fgTargetChnlScanIssued ? "partial" : "full");
	...
	DBGLOG(AIS, INFO,
		"[Roaming] No target found(trial=%d/%d), disallowScan=%d, issued=%d\n",
		ais->ucScanTrialCount,
		roam->rRoamScanParam.ucScanCount,
		fgDisallowScan, ais->fgTargetChnlScanIssued);
	 ...
	 */
	if (ais->ucReasonOfDisconnect == DISCONNECT_REASON_CODE_ROAMING)
	...
	struct BSS_DESC_SET *prBssDescSet, uint8_t ucBssIndex)
{
	...
	if (roam->eReason == ROAMING_REASON_BTM) {
		...
		if (u2PrefLinks == prBssDescSet->u2ValidLinks)
			return FALSE;
	}
#endif
					...
					uintptr_t ulParamPtr)
{
	...
	if (prAdapter->rWifiVar.fgApRemovalByT2LM) {
		struct MLD_STA_RECORD *prMldStaRec =
			aisGetMldStaRec(prAdapter, ucBssIndex);

		DBGLOG(AIS, INFO,
			"BSS[%d] AP link removed by t2lm!\n",
			ucBssIndex);

		if (prAdapter->rWifiVar.ucMlrcOpCap)
			aisFsmMlReconfigUpdateState(prAdapter, ucBssIndex);
		else
...
void aisFsmMlReconfigUpdateBssInfoForAllAP(
	struct ADAPTER *prAdapter, struct LR_RESP_INFO *prLrInfo,
	uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo;
	struct BSS_INFO *prAisBssInfo;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		/* use same setting */
		if (prMlrc->aeMlrcAction[i] == MLRC_ACTION_ADD_DEPAUSE) {
			struct SW_RFB *prSwRfb;

			prSwRfb = mldReconfigBuildAssocSwRfb(prAdapter,
				prMlrc->prLRResponseSwRfb, prLrInfo,
				prBssDesc, prStaRec);
...
void aisFsmMlReconfigInstallKeyForAllAP(
	struct ADAPTER *prAdapter, struct LR_RESP_INFO *prLrInfo,
	uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		if (!prStaRec ||
		     prMlrc->aeMlrcAction[i] != MLRC_ACTION_ADD_DEPAUSE)
			continue;

		if (prLrInfo->u2ValidMloGTK & BIT(prStaRec->ucLinkId))
			wlanSetAddKey(prAdapter,
				&prLrInfo->rMloGTK[prStaRec->ucLinkId],
				sizeof(struct PARAM_KEY), &len, FALSE);
		if (prLrInfo->u2ValidMloIGTK & BIT(prStaRec->ucLinkId))
			wlanSetAddKey(prAdapter,
				&prLrInfo->rMloIGTK[prStaRec->ucLinkId],
				sizeof(struct PARAM_KEY), &len, FALSE);
		if (prLrInfo->u2ValidMloBIGTK & BIT(prStaRec->ucLinkId))
			wlanSetAddKey(prAdapter,
				&prLrInfo->rMloBIGTK[prStaRec->ucLinkId],
				sizeof(struct PARAM_KEY), &len, FALSE);
	}
}
	...
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	if (prMldStaRec && prMldStaRec->fgMlrcOp)
		nicUniCmdUpdateMldRecfgState(prAdapter, prMldStaRec);
}

void aisFsmMlReconfigCompleteAction(
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo;
	struct MLD_STA_RECORD *prMldStaRec;
	struct MLRC_INFO *prMlrc;
	struct LR_RESP_INFO *prLrInfo = NULL;

	prAisFsmInfo = aisGetAisFsmInfo(prAdapter, ucBssIndex);
	prMldStaRec = aisGetMldStaRec(prAdapter, ucBssIndex);
	prMlrc = aisGetMlrcInfo(prAdapter, ucBssIndex);

	if (prAisFsmInfo->rMlrcInfo.eMlrcState != MLRC_STATE_RECONFIG) {
		DBGLOG(AIS, INFO, "wrong state %d\n",
			prAisFsmInfo->rMlrcInfo.eMlrcState);
		return;
	}

	DBGLOG(AIS, INFO, "LR: LRResponseSwRfb=%p\n",
		prMlrc->prLRResponseSwRfb);

	prLrInfo = kalMemZAlloc(sizeof(struct LR_RESP_INFO), VIR_MEM_TYPE);
	if (!prLrInfo)
		goto done;

	if (prMlrc->prLRResponseSwRfb)
		mldParseLRRespActionFrame(prAdapter, prMlrc->prLRResponseSwRfb,
			prLrInfo, ucBssIndex);

	aisFsmMlReconfigUpdateBssInfoForAllAP(prAdapter,
			prLrInfo, ucBssIndex);

	aisFsmMlReconfigInstallKeyForAllAP(prAdapter,
			prLrInfo, ucBssIndex);

	/* update pause after authrized */
	...
	if (prLrInfo)
		kalMemFree(prLrInfo, VIR_MEM_TYPE, sizeof(struct LR_RESP_INFO));

	if (prMlrc->prLRResponseSwRfb) {
		nicRxReturnRFB(prAdapter, prMlrc->prLRResponseSwRfb);
		prMlrc->prLRResponseSwRfb = NULL;
	}

	aisSetMlrcState(prAdapter, MLRC_STATE_IDLE, ucBssIndex);

	if (prMlrc->fgHasPendingAction)
		cnmTimerStartTimer(prAdapter,
			&prAisFsmInfo->rMlrcInfo.rPendingMlrcOpTimer,
			MLRC_OP_PENDING_DELAY);
}

uint8_t aisSearchHandleBssDescForMlReconfig(struct ADAPTER *prAdapter,
	struct BSS_DESC_SET *prBssDescSet, uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo =
		aisGetAisFsmInfo(prAdapter, ucBssIndex);
	...
	struct MLD_BSS_INFO *mld_bssinfo =
		aisGetMldBssInfo(prAdapter, ucBssIndex);
	struct MLRC_INFO *prMlrc =
		aisGetMlrcInfo(prAdapter, ucBssIndex);
	struct BSS_DESC *prMainBssDesc = prBssDescSet->prMainBssDesc;
	uint8_t i, j, fgHasDel = FALSE, fgHasAdd = FALSE;
	uint8_t fgMlrcOpMode = prAdapter->rWifiVar.fgMlrcOpMode;
	enum ENUM_MLRC_STATE eMlrcState = MLRC_STATE_IDLE;

	if (!mld_starec || !mld_starec->fgMlrcOp ||
	    UNEQUAL_MAC_ADDR(prMainBssDesc->rMlInfo.aucMldAddr,
	...
	/* decide action */
	for (i = 0; i < MLD_LINK_MAX; i++) {
		struct BSS_DESC *bss = aisGetLinkBssDesc(prAisFsmInfo, i);
		struct STA_RECORD *prStaRec = aisGetLinkStaRec(prAisFsmInfo, i);

		prMlrc->aprBssDesc[i] = NULL;
		prMlrc->aeMlrcAction[i] = MLRC_ACTION_EMPTY;
		...
		if (bss && prStaRec) {
			for (j = 0; j < prBssDescSet->ucLinkNum; j++) {
				...
				if (bss == w->prBssDesc && !w->fgApRemoval)
					break;
			}

			/* found in target set */
			if (j != prBssDescSet->ucLinkNum) {
				if (prStaRec->fgApRemoval) {
					prMlrc->aeMlrcAction[i] =
		...
		if (prMlrc->aeMlrcAction[i] != MLRC_ACTION_EMPTY)
			prMlrc->aprBssDesc[i] = bss;
	}

	for (i = 0; i < prBssDescSet->ucLinkNum; i++) {
		struct BSS_DESC_W *w = prBssDescSet->aprBssDescW[i];
		struct BSS_DESC *bss = w->prBssDesc;

		if (!bss || w->fgApRemoval)
			continue;

		for (j = 0; j < MLD_LINK_MAX; j++) {
			if (bss == prMlrc->aprBssDesc[j])
				break;
		}

		/* new link but not found, only support by reassoc */
		if (j == MLD_LINK_MAX)
			goto done;
	}

	/* allow only one action at a time, delete first */
	if (fgHasDel &&
	   (fgMlrcOpMode == MLRC_MODE_DEL_THEN_ADD ||
	    fgMlrcOpMode == MLRC_MODE_DEL_ONLY)) {
		for (i = 0; i < MLD_LINK_MAX; i++) {
			if (prMlrc->aeMlrcAction[i] ==
					MLRC_ACTION_ADD_DEPAUSE) {
				DBGLOG(AIS, INFO,
					"LR: #%d %s_ACTION=%s BssDesc=%p\n",
					i,
					fgMlrcOpMode == MLRC_MODE_DEL_THEN_ADD ?
					"PENDING" : "REMOVE", apucMlrcActionStr[
					prMlrc->aeMlrcAction[i]],
					prMlrc->aprBssDesc[i]);

				if (fgMlrcOpMode == MLRC_MODE_DEL_THEN_ADD)
					prMlrc->fgHasPendingAction = TRUE;
				prMlrc->aeMlrcAction[i] = MLRC_ACTION_NONE;
				/* prMlrc->aprBssDesc[i] not changed */
			}
		}
	}

	/* allow only one action at a time, add first */
	if (fgHasAdd &&
	   (fgMlrcOpMode == MLRC_MODE_ADD_THEN_DEL ||
	    fgMlrcOpMode == MLRC_MODE_ADD_ONLY)) {
		for (i = 0; i < MLD_LINK_MAX; i++) {
			if (prMlrc->aeMlrcAction[i] ==
				MLRC_ACTION_DELETE_PAUSE) {
				DBGLOG(AIS, INFO,
				...
				if (fgMlrcOpMode == MLRC_MODE_ADD_THEN_DEL)
					prMlrc->fgHasPendingAction = TRUE;
				prMlrc->aeMlrcAction[i] = MLRC_ACTION_NONE;
				prMlrc->aprBssDesc[i] =
					aisGetLinkBssDesc(prAisFsmInfo, i);
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		DBGLOG(AIS, INFO,
			"LR: #%d ACTION=%s BssDesc=%p Bss%d LinkId%d Widx%d\n",
			i, apucMlrcActionStr[prMlrc->aeMlrcAction[i]],
			prMlrc->aprBssDesc[i], prStaRec->ucBssIndex,
			prStaRec->ucLinkId, prStaRec->ucWlanIndex);

		if (prMlrc->aeMlrcAction[i] == MLRC_ACTION_DELETE_PAUSE)
			prStaRec->fgApRemoval = TRUE;
		else if (prMlrc->aeMlrcAction[i] == MLRC_ACTION_ADD_DEPAUSE)
			prStaRec->fgApRemoval = FALSE;
	}

	eMlrcState = MLRC_STATE_NEGO;

done:
	aisSetMlrcState(prAdapter, eMlrcState, ucBssIndex);

	/* update correct mld cap for LR req */
	mldBssUpdateCap(prAdapter, mld_bssinfo, prBssDescSet);

	return eMlrcState == MLRC_STATE_NEGO;
}

enum ENUM_AIS_STATE aisSearchHandleReconnectForMlReconfig(
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo =
		aisGetAisFsmInfo(prAdapter, ucBssIndex);

	if (aisGetMlrcState(prAdapter, ucBssIndex) == MLRC_STATE_RECONFIG) {
		DBGLOG(AIS, INFO, "LR: nego done\n");

		mldBssSyncCap(prAdapter,
			aisGetMldBssInfo(prAdapter, ucBssIndex));

		return AIS_STATE_LINK_RECONFIG;
	}

	DBGLOG(AIS, INFO, "LR: nego fail\n");
	...
	struct BSS_DESC_SET *prBssDescSet, uint8_t ucBssIndex)
{
	...
	if (prAisBssInfo->eConnectionState == MEDIA_STATE_DISCONNECTED) {
		if (aisJoinFailureOverLimit(prAdapter, ucBssIndex))
		...
		if (aisJoinFailureOverLimit(prAdapter, ucBssIndex))
			return aisSearchHandleBadBssDesc(prAdapter,
				prBssDescSet, TRUE, ucBssIndex);

		if (!aisBssDescAllowed(prAdapter, prAisFsmInfo, prBssDescSet))
			return aisSearchHandleBadBssDesc(prAdapter,
				prBssDescSet, FALSE, ucBssIndex);
...
#if (CFG_SUPPORT_ML_RECONFIG == 1)
		if (!aisSearchHandleBssDescForMlReconfig(prAdapter,
			prBssDescSet, ucBssIndex))
#endif /* CFG_SUPPORT_ML_RECONFIG */
			aisFillBssInfoFromBssDesc(prAdapter,
				prAisFsmInfo, prBssDescSet, ucBssIndex);
	...
	uint8_t *pucPrimaryChannel, uint8_t ucBssIndex)
{
	...
	if (ais->eCurrentState == AIS_STATE_LOOKING_FOR &&
	    ais->fgTargetChnlScanIssued &&
	    (setting->eConnectionPolicy == CONNECT_BY_BSSID ||
	     setting->eConnectionPolicy == CONNECT_BY_BSSID_HINT) &&
	...
	struct AIS_FSM_INFO *prAisFsmInfo, uint8_t ucBssIndex)
{
	...
	if (aisFsmIsRequestPending(prAdapter,
		...
		aisHandleBeaconTimeout(prAdapter, ucBssIndex);
		...
		AIS_REQUEST_LINK_RECONFIG, TRUE, &prAisReq, ucBssIndex)) {
		aisGetConnSettings(prAdapter, ucBssIndex)->eConnectionPolicy =
			CONNECT_BY_BSSID_REUSE;
		eNextState = AIS_STATE_SEARCH;
		cnmMemFree(prAdapter, prAisReq);
#endif /* CFG_SUPPORT_ML_RECONFIG */
	...
	do {
		...
		case AIS_STATE_IDLE:
			...
			if (prAisReq)
				DBGLOG(AIS, INFO,
					"eReqType=%s\n",
					aisGetFsmReqType(prAisReq->eReqType));
			else
			...
			if (prAisReq == NULL) {
				...
				if (prAisReq->eReqType == AIS_REQUEST_RECONNECT)
					eNextState = AIS_STATE_SEARCH;
				else
					eNextState = AIS_STATE_LOOKING_FOR;
				fgIsTransition = TRUE;
		...
		case AIS_STATE_SEARCH: {
			 ...
			 */
			aisSearchBssDescByScore(prAdapter, ucBssIndex, set);
...
send_msg:
		...
		case AIS_STATE_JOIN: {
			...
			struct MSG_SAA_FSM_START *prJoinReqMsg;
			uint32_t u4State = WLAN_STATUS_SUCCESS;

			for (i = 0; i < MLD_LINK_MAX; i++) {
				struct BSS_INFO *bss = aisGetLinkBssInfo(
					prAisFsmInfo, i);
				struct BSS_DESC *bssdesc = aisGetLinkBssDesc(
					prAisFsmInfo, i);
				...
				cnmOpModeGetTRxNss(prAdapter,
						   ...
						   &bss->ucOpRxNss,
						   &bss->ucOpTxNss);
				u4State = aisFsmStateInit_JOIN(prAdapter,
						prAisFsmInfo,
						&prMainStaRec,
						i);
				if (u4State != WLAN_STATUS_SUCCESS)
					break;
			}

			/* only setup link needs to do SAA */
			if (!prMainStaRec || !prMainBssDesc ||
			    u4State != WLAN_STATUS_SUCCESS)
				break;

			/* update fgMgmtProtection from main link */
			aisFsmUpdateRsnSetting(prAdapter,
				prMainBssDesc, ucBssIndex);

			if (prMainBssDesc->ucSSIDLen)
				COPY_SSID(prConnSettings->aucSSID,
					  prConnSettings->ucSSIDLen,
					  prMainBssDesc->aucSSID,
					  prMainBssDesc->ucSSIDLen);

			/* Send a Msg to trigger SAA to start JOIN */
			prJoinReqMsg = (struct MSG_SAA_FSM_START *)
				cnmMemAlloc(prAdapter, RAM_TYPE_MSG,
				sizeof(struct MSG_SAA_FSM_START));
			if (!prJoinReqMsg) {
				DBGLOG(AIS, ERROR,
					"Can't trigger SAA FSM\n");
				break;
			}

			prJoinReqMsg->rMsgHdr.eMsgId =
...
#if (CFG_SUPPORT_ML_RECONFIG == 1)
		case AIS_STATE_LINK_RECONFIG:
			aisFsmMlReconfigCompleteAction(prAdapter, ucBssIndex);
			eNextState = AIS_STATE_NORMAL_TR;
			fgIsTransition = TRUE;
			break;
#endif /* CFG_SUPPORT_ML_RECONFIG */
	...
	struct BSS_DESC_SET *prBssDescSet)
{
	...
	if (ais->ucReasonOfDisconnect ==
		...
		if (conn->eConnectionPolicy == CONNECT_BY_BSSID ||
			...
			/* not scan yet, scan freq hint first */
			if (ais->ucScanTrialCount == 0) {
	...
	if (ais->ucReasonOfDisconnect ==
		...
		DISCONNECT_REASON_CODE_DISASSOCIATED) {
		/* only allow scan once when receiving deauth or deassoc */
		if (ais->ucScanTrialCount > 0)
			return AIS_STATE_JOIN_FAILURE;
		else
			return AIS_STATE_LOOKING_FOR;
...
void aisFsmGetCurrentEssChnlList(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	if (!prAdapter->rWifiVar.fgDisClearECL) {
		kalMemZero(&prAisSpecBssInfo->arCurEssChnlInfo[0],
		       CFG_MAX_NUM_OF_CHNL_INFO * sizeof(struct ESS_CHNL_INFO));
		kalMemZero(prAisSpecBssInfo->aucCurEssChnlBitMap, 64);
		prAisSpecBssInfo->ucCurEssChnlInfoNum = 0;
	}

	prEssChnlInfo = &prAisSpecBssInfo->arCurEssChnlInfo[0];
	pucChnlInfoNum = &prAisSpecBssInfo->ucCurEssChnlInfoNum;
	pauChnlBitMap = prAisSpecBssInfo->aucCurEssChnlBitMap;
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry,
		...
		if (!aisFsmUpdateChannelList(prBssDesc->ucChannelNum,
			prBssDesc->eBand, pauChnlBitMap, pucChnlInfoNum,
			prEssChnlInfo))
			goto updated;
	}
	...
	if (prAdapter->rNchoInfo.fgNCHOEnabled) {
		...
		for (i = 0; *pucChnlInfoNum < CFG_MAX_NUM_OF_CHNL_INFO &&
			...
			if (!aisFsmUpdateChannelList(chnl, eBand,
			    pauChnlBitMap, pucChnlInfoNum, prEssChnlInfo))
				goto updated;
		}
 ...
 */
/*----------------------------------------------------------------------------*/
enum ENUM_AIS_STATE aisFsmScanResultsUpdate(struct ADAPTER *prAdapter,
	uint8_t ucBssIndex)
{
	struct AIS_FSM_INFO *prAisFsmInfo;
...
#if (CFG_SUPPORT_ROAMING == 1)
		struct APS_INFO *prApsInfo =
			aisGetApsInfo(prAdapter, ucBssIndex);

		DBGLOG(AIS, INFO,
			"<CONN> SCAN_DONE bidx=%d ap_count=%d coex=%d\n",
			ucBssIndex, prApsInfo->u4EssApNum,
			prApsInfo->fgIsGBandCoex);
#endif
	...
	return eNextState;
} /* end of aisFsmScanResultsUpdate() */
			    ...
			    struct MSG_HDR *prMsgHdr)
{
	...
	if (prBcnRmParam->eState == RM_ON_GOING) {
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry,
				    struct BSS_DESC)
		{
			if (IS_AIS_CONN_BSSDESC(prAisFsmInfo, prBssDesc) ||
			    TIME_BEFORE(prRmReq->rScanStartTime,
...
void aisAddDeauthBlockList(struct ADAPTER *prAdapter,
	struct BSS_DESC *prBssDesc, uint16_t u2DeauthReason)
{
	if (prBssDesc) {
			 ...
			 struct MSG_HDR *prMsgHdr)
{
	...
	aisFsmClearRequest(prAdapter, AIS_REQUEST_LOOKING_FOR, ucBssIndex);
	aisFsmClearRequest(prAdapter, AIS_REQUEST_BTO, ucBssIndex);
	...
	struct SW_RFB *prAssocRspSwRfb, uint8_t ucBssIndex)
{
	...
	DBGLOG(AIS, INFO,
	       ...
	       ucBssIndex, prStaRec->ucJoinFailureCount,
	       prBssDesc->ucJoinFailureCount,
	       prBssDesc->ucTempRejectCount,
...
#if CFG_SUPPORT_MBO
	if (pucIE && prStaRec->u2StatusCode ==
	...
	fgTempReject = aisHandleTemporaryReject(prAdapter, prStaRec, prBssDesc);
	aisTargetBssResetConnecting(prAdapter, prAisFsmInfo);
	aisRestoreAllLink(prAdapter, prAisFsmInfo);
					     ...
					     struct MSG_HDR *prMsgHdr)
{
	...
	struct SW_RFB *prAssocRspSwRfb;
	struct BSS_INFO *prAisBssInfo;
	struct LINK_SPEED_EX_ *prLq = NULL;
	struct CONNECTION_SETTINGS *prConnSettings;
	uint8_t ucBssIndex = 0;
	...
	do {
		...
		if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
			DBGLOG(AIS, INFO,
				"<CONN> CONN_SUCCESS bidx=%d ssid=%s bssid="
				MACSTR"\n", ucBssIndex,
				     ...
				     uint8_t ucBssIndex)
{
	...
	DBGLOG(AIS, INFO,
	       "<CONN> DISCONN_DONE bidx=%d ssid=%s bssid="MACSTR"\n",
	       ucBssIndex, HIDE(prAisBssInfo->aucSSID),
	       MAC2STR(prAisBssInfo->aucBSSID));
	...
	kalMemZero(&prAisSpecificBssInfo->arCurEssChnlInfo[0],
		CFG_MAX_NUM_OF_CHNL_INFO * sizeof(struct ESS_CHNL_INFO));
	kalMemZero(prAisSpecificBssInfo->aucCurEssChnlBitMap, 64);
	prAisSpecificBssInfo->ucCurEssChnlInfoNum = 0;

	kalMemZero(prAisSpecificBssInfo->aucQueriedBssid,
		sizeof(prAisSpecificBssInfo->aucQueriedBssid));
	prAisSpecificBssInfo->ucQueriedBssidIdx = 0;

	/* remove deferred bto event when disconnect */
	aisFsmClearPostponedBTO(prAdapter, ucBssIndex);
		...
		uint8_t ucBssIndex)
{
	...
	DBGLOG(AIS, EVENT, "policy %d, timeout %d, trial %d\n",
			     ...
			     struct SW_RFB *prAssocRspSwRfb)
{
	...
	if (prAssocRspSwRfb) {
		...
		u2IELength = (uint16_t) ((prAssocRspSwRfb->u2PacketLen -
			prAssocRspSwRfb->u2HeaderLen) -
		       (OFFSET_OF(struct WLAN_ASSOC_RSP_FRAME, aucInfoElem[0]) -
			WLAN_MAC_MGMT_HEADER_LEN));
		pucIE = prAssocRspFrame->aucInfoElem;
	...
	/* 4 <1.2> Setup SSID */
	kalMemZero(prAisBssInfo->aucSSID, ELEM_MAX_LEN_SSID);
	...
	if (prAssocRspSwRfb)
		mqmProcessAssocRsp(prAdapter, prAssocRspSwRfb,
			pucIE, u2IELength);
	 ...
	 */
	if (prAssocRspSwRfb)
		rlmProcessAssocRsp(prAdapter, prAssocRspSwRfb,
			pucIE, u2IELength);
		...
		u_int8_t fgDelayIndication, uint8_t ucBssIndex)
{
	...
	if (prAisBssInfo->eConnectionState == MEDIA_STATE_CONNECTED) {
		...
		if (fgDelayIndication) {
			...
			u_int32_t u4ScnResultsTimeout =
				prAdapter->rWifiVar.u4DiscoverTimeout;
			...
			GET_CURRENT_SYSTIME(&(prAisFsmInfo->rJoinReqTime));
			if (CHECK_FOR_TIMEOUT(prAisFsmInfo->rJoinReqTime,
				prAisFsmInfo->rScanDoneTime,
				SEC_TO_SYSTIME(u4ScnResultsTimeout)))
				eAisRequest = AIS_REQUEST_LOOKING_FOR;
			else
				eAisRequest = AIS_REQUEST_RECONNECT;

			aisFsmClearRequest(prAdapter,
				AIS_REQUEST_RECONNECT, ucBssIndex);
			aisFsmClearRequest(prAdapter,
				AIS_REQUEST_LOOKING_FOR, ucBssIndex);

			switch (prAisFsmInfo->ucReasonOfDisconnect) {
			case DISCONNECT_REASON_CODE_RADIO_LOST:
				roam->eReason = ROAMING_REASON_BEACON_TIMEOUT;
				aisFsmInsertRequest(prAdapter,
					eAisRequest, ucBssIndex);
				break;
			...
			case DISCONNECT_REASON_CODE_REASSOCIATION:
				...
				/* no need to scan if upper layer trigger */
				aisFsmInsertRequest(prAdapter,
					AIS_REQUEST_RECONNECT, ucBssIndex);
				break;
			default:
...
#if CFG_SUPPORT_802_11W
	struct AIS_SPECIFIC_BSS_INFO *prAisSpecificBssInfo;
	struct CONNECTION_SETTINGS *prcConnSetting;
#endif
#if CFG_SUPPORT_ROAMING
	struct ROAMING_INFO *prRoamingFsmInfo =
			aisGetRoamingInfo(prAdapter, ucBssIndex);
...
#if CFG_SUPPORT_802_11W
	prAisSpecificBssInfo = aisGetAisSpecBssInfo(prAdapter, ucBssIndex);
	prcConnSetting = aisGetConnSettings(prAdapter, ucBssIndex);
#endif
			       ...
			       uintptr_t ulParamPtr)
{
	...
	if (ucBssIndex >= MAX_BSSID_NUM) {
		DBGLOG(AIS, ERROR, "ucBssIndex [%d] out of range!\n",
			ucBssIndex);
		return;
	}
	...
	uint8_t ucBcnTimeoutReason, uint8_t fgTryRecover, uint8_t ucBssIndex)
{
	...
	if (prAisBtoInfo->prBtoBssDesc) {
		DBGLOG(AIS, EVENT,
			"Drop BTO event because there's ongoing BTO\n");
		return;
	}
	...
	if (fgDoAbortIndication) {
		...
		if (roam || join) {
			DBGLOG(AIS, EVENT,
				"Postpone aisBssBeaconTimeout, roam=%d, join=%d, recover=%d\n",
				roam, join, fgTryRecover);
			aisFsmInsertRequest(prAdapter,
				AIS_REQUEST_BTO,
				ucBssIndex);
		} else {
			DBGLOG(AIS, EVENT,
				"aisBssBeaconTimeout, roam=%d, join=%d, recover=%d\n",
				roam, join, fgTryRecover);
			aisHandleBeaconTimeout(prAdapter, ucBssIndex);
		}
	} else {
...
void aisHandleBeaconTimeout(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	struct BSS_INFO *prAisBssInfo;
	struct AIS_FSM_INFO *prAisFsmInfo;
	struct AIS_BTO_INFO *prAisBtoInfo;
	...
	prAisBssInfo->u2DeauthReason =
		...
		prAisBtoInfo->ucBcnTimeoutReason;
	prAisBtoInfo->prBtoBssDesc->fgIsInBTO = TRUE;

	for (i = 0; i < MLD_LINK_MAX; i++) {
		struct BSS_DESC *prBss = aisGetLinkBssDesc(prAisFsmInfo, i);

		if (prBss)
			prBss->fgIsInBTO = TRUE;
	}

#if CFG_SUPPORT_ROAMING
	if (prAisBtoInfo->fgTryRecover) {
		struct CMD_ROAMING_TRANSIT rRoamingData = {0};
		struct ROAMING_INFO *prRoamingFsmInfo =
			aisGetRoamingInfo(prAdapter, ucBssIndex);
	...
	uint32_t u4ReqScan, uint8_t ucBssIndex)
{
	...
	/* abort ongoing scan to handle roaming event immediately */
...
#if (CFG_EXT_ROAMING == 1)
#if (CFG_EXT_ROAMING_WTC == 1)
	if (prRoamingInfo->eReason == ROAMING_REASON_BTM &&
	    prBtmReq->ucIsCisco &&
	    prWtc->ucScanMode > 1) {
		prAisFsmInfo->fgTargetChnlScanIssued = FALSE;
	}
#endif /* CFG_EXT_ROAMING_WTC == 1 */
	if (prRoamingInfo->eReason == ROAMING_REASON_IDLE) {
		prAisFsmInfo->fgTargetChnlScanIssued = FALSE;
	} else if (prRoamingInfo->eReason == ROAMING_REASON_INACTIVE_TIMER) {
		prAisFsmInfo->fgTargetChnlScanIssued = FALSE;
	}
#endif /* CFG_EXT_ROAMING == 1 */
...
#if (CFG_SUPPORT_ML_RECONFIG == 1)
	if (aisGetMlrcState(ad, ucBssIndex) != MLRC_STATE_IDLE)
		return aisSearchHandleReconnectForMlReconfig(ad, ucBssIndex);
#endif
	...
	struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	if (roamingFsmInDecision(prAdapter, FALSE, ucBssIndex) &&
	    ...
	    CHECK_FOR_TIMEOUT(ais->rScanDoneTime,
		...
		if (prAdapter->rWifiVar.u4AisTestMode.fgIgnoreBandCheck) {
			DBGLOG(AIS, WARN, "[TEST] Trigger inactive roaming\n");
			return TRUE;
		}
#endif
	...
	LINK_FOR_EACH_ENTRY_SAFE(prPendingReqHdr,
		...
		if (prPendingReqHdr->eReqType == eReqType) {
			...
			if (bRemove == TRUE) {
				...
				DBGLOG(AIS, INFO, "Remove req=%s\n",
					aisGetFsmReqType(eReqType));
			}
			     ...
			     uint8_t ucBssIndex)
{
	...
	DBGLOG(AIS, INFO, "eCurrentState=%s, eReqType=%s, u4NumElem=%d\n",
	       aisGetFsmState(prAisFsmInfo->eCurrentState),
	       aisGetFsmReqType(eReqType),
			     ...
			     uint8_t ucBssIndex)
{
	...
	DBGLOG(AIS, INFO,
	       "To %s, eCurrentState=%s, eReqType=%s, u4NumElem=%d\n",
	       fgToHead ? "head" : "tail",
	       aisGetFsmState(prAisFsmInfo->eCurrentState),
	       aisGetFsmReqType(prAisReq->eReqType),
	       prAisFsmInfo->rPendingReqList.u4NumElem);
				 ...
				 struct MSG_HDR *prMsgHdr)
{
	...
	if (prAdapter->rWifiVar.ucT2LMNegotiationSupport &&
		...
		struct BSS_INFO *prBssInfo;
		uint16_t u2MapValue, u2ValidLinks;
		struct MLD_STA_RECORD *prMldStarec;
		int32_t i;
		 ...
		 */
		if ((u2PrefLinks & u2ValidLinks) == 0 ||
		    (u2PrefLinks & ~u2ValidLinks) != 0) {
			DBGLOG(AIS, WARN,
			     "Skip t2lm, ValidLinks=0x%x PrefLinks=0x%x\n",
			     u2ValidLinks, u2PrefLinks);
			goto skip_t2lm;
		}
		...
		if (prAdapter->rWifiVar.u4T2LMMapValue != 0) {
			...
			u2MapValue = u2PrefLinks & u2ValidLinks;
		}
...
void aisFsmQueryCandidates(struct ADAPTER *prAdapter, uint8_t ucBssIndex)
{
	...
	for (i = 0; i < AIS_MAX_QUERIED_BSSID_NUM; i++) {
		if (EQUAL_MAC_ADDR(prBssDesc->aucBSSID,
			prAisSpecificBssInfo->aucQueriedBssid[i])) {
			DBGLOG(WNM, WARN, "BTM: don't query old ap\n");
			return;
		}
	}
	...
	if (!(prBssDesc->fgQueriedCandidates & BIT(ucBssIndex))) {
		...
		kalMemCopy(prAisSpecificBssInfo->aucQueriedBssid[
			prAisSpecificBssInfo->ucQueriedBssidIdx],
			prBssDesc->aucBSSID, MAC_ADDR_LEN);
		prAisSpecificBssInfo->ucQueriedBssidIdx	=
			(prAisSpecificBssInfo->ucQueriedBssidIdx + 1) %
			AIS_MAX_QUERIED_BSSID_NUM;
...
void aisResetMlrcInfo(
	...
	uint8_t ucBssIndex)
{
	struct MLRC_INFO *prMlrc = aisGetMlrcInfo(prAdapter, ucBssIndex);

	aisSetMlrcState(prAdapter, MLRC_STATE_IDLE, ucBssIndex);
	prMlrc->fgHasPendingAction = FALSE;
	cnmTimerStopTimer(prAdapter, &prMlrc->rApRemovalTimer);
	cnmTimerStopTimer(prAdapter, &prMlrc->rPendingMlrcOpTimer);
	kalMemZero(prMlrc->aprBssDesc, sizeof(prMlrc->aprBssDesc));
	kalMemZero(prMlrc->aeMlrcAction, sizeof(prMlrc->aeMlrcAction));
	if (prMlrc->prLRResponseSwRfb) {
		nicRxReturnRFB(prAdapter, prMlrc->prLRResponseSwRfb);
		prMlrc->prLRResponseSwRfb = NULL;
	}
...
struct MLRC_INFO *aisGetMlrcInfo(
	...
	uint8_t ucBssIndex)
{
	return &aisGetAisFsmInfo(prAdapter, ucBssIndex)->rMlrcInfo;
}
	...
	uint8_t ucBssIndex)
{
	...
	if (!prAisFsmInfo)
		return;
	prMlrc = &prAisFsmInfo->rMlrcInfo;

	if (prMlrc->eMlrcState != eMlrcState) {
		DBGLOG(AIS, STATE, "LR:[%d] TRANSITION: [%s] -> [%s]\n",
			prAisFsmInfo->ucAisIndex,
			apucMlrcStateStr[prMlrc->eMlrcState],
			apucMlrcStateStr[eMlrcState]);

		prMlrc->eMlrcState = eMlrcState;
	}
}
...
enum ENUM_MLRC_STATE aisGetMlrcState(
	...
	uint8_t ucBssIndex)
{
	return aisGetMlrcInfo(prAdapter, ucBssIndex)->eMlrcState;
}
#endif /* CFG_SUPPORT_ML_RECONFIG */
	...
	struct AIS_FSM_INFO *prAisFsmInfo)
{
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		bss = aisGetLinkBssDesc(prAisFsmInfo, i);
		sta = aisGetLinkStaRec(prAisFsmInfo, i);

		if (!bss || !sta)
			continue;

		/* highest band: 6G > 5G > 2.4G */
		if (cnmStaRecIsActive(prAdapter, sta) &&
		    (currBss == NULL || bss->eBand > currBss->eBand ||
		    (bss->eBand == currBss->eBand &&
		     bss->ucRCPI > currBss->ucRCPI)))
			currBss = bss;
	}
	...
	uint8_t ucBssIndex)
{
	...
	if (prBssDesc)
		return RCPI_TO_dBm(prBssDesc->ucRCPI);
	...
	uint8_t *ucChTokenId)
{
	...
	uint8_t ucReqChNum = 0, ucReqChCount = 0;
	uint32_t u4MsgSz;
	uint8_t i = 0;
	...
	*ucChTokenId = cnmIncreaseTokenId(prAdapter);
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		prSubReq = (struct MSG_CH_REQ *)&prMsgChReq[ucReqChCount];
	...
	uint8_t ucBssIndex, struct MSG_SCN_SCAN_REQ_V2 *prScanReqMsg)
{
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_LOOKING_FOR)
		DBGLOG(AIS, INFO,
			"<CONN> SCAN_START ml_scan_conut=%d scan_type=%s ssid_type=%s chnl_type=%s chnl_num=%d dwell=%d,%d func=0x%x,0x%x",
			prAisFsmInfo->ucMlProbeSendCount,
			apucScanTypeStr[prScanReqMsg->eScanType],
			scanLogSSIDType(prScanReqMsg->ucSSIDType),
			apucScanChnlStr[prScanReqMsg->eScanChannel],
	...
	struct PARAM_SCAN_REQUEST_ADV *prScanRequest)
{
	...
	if (prAisFsmInfo->eCurrentState == AIS_STATE_LOOKING_FOR)
		DBGLOG(AIS, INFO,
			"<CONN> SCAN_START type=%s ssid_type=%s chnl_type=%s num=%d dwell=%d,%d func=0x%x,0x%x",
			apucScanTypeStr[prScanReqMsg->eScanType],
			scanLogSSIDType(prScanReqMsg->ucSSIDType),
			apucScanChnlStr[prScanReqMsg->eScanChannel],
			prScanReqMsg->ucChannelListNum,
