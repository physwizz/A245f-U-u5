...
static signed int __fm_ops_register(struct fm_lowlevel_ops *ops)
{
	signed int ret = 0;
...
static signed short fm_cust_config_setting(void)
{
	...
	if (chipid == 0) {
		WCN_DBG(FM_ERR | MAIN, "Invalid chipid:0x%x\n", chipid);
		return -1;
	}
...
signed int fm_ops_register(void)
{
	signed int ret = 0;
	...
	ret = __fm_ops_register(&fm_low_ops);
	if (ret != 0)
		goto end;

	ret = fm_cust_config_setting();
	if (ret != 0)
		goto end;
