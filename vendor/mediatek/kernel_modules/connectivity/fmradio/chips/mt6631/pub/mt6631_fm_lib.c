...
static signed int mt6631_PowerUp(unsigned short *chip_id, unsigned short *device_id)
{
	...
	*device_id = tmp_reg;
	...
	ret = mt6631_pwrup_DSP_download(mt6631_get_patch_tbl());
	if (ret) {
	...
	fm_reg_read(0x62, &tmp_reg);
	if (tmp_reg != mt6631_hw_info.chip_id) {
		mt6631_show_reg();
		WCN_DBG(FM_ALT | CHIP,
...
static signed int mt6631_full_cqi_get(signed int min_freq, signed int max_freq, signed int space, signed int cnt)
{
	...
	for (k = 0; (orig_freq == 10000) && (g_dbg_level == 0xffffffff) && (k < cnt); k++) {
		...
		if (snprintf(&cqi_log_path[strlen(FM_CQI_LOG_PATH)],
			     sizeof(cqi_log_path) - strlen(FM_CQI_LOG_PATH),
			     "%d.txt", k + 1) < 0)
		...
		for (j = 0; j < num; j++) {
			...
			if (!ret && fm_res) {
				...
				for (i = 0; i < fm_res->cqi[1]; i++) {
					...
					/* format to buffer */
					if (snprintf(cqi_log_buf, sizeof(cqi_log_buf),
