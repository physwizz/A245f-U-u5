...
static INT32 consys_sleep_info_read_ctrl(WMT_SLEEP_COUNT_TYPE type, PUINT64 sleep_counter,
					PUINT64 sleep_timer);
...
*/
/* CCF part */
struct clk *clk_infracfg_ao_ccif4_ap_cg_mt6789;       /* For direct path */
...
extern int g_mapped_reg_table_sz_mt6789;
extern REG_MAP_ADDR g_mapped_reg_table_mt6789[];
...
WMT_CONSYS_IC_OPS consys_ic_ops_mt6789 = {
	...
	.consys_ic_get_debug_reg_ary_size = &g_mapped_reg_table_sz_mt6789,
	.consys_ic_get_debug_reg_ary = g_mapped_reg_table_mt6789,
};
...
UINT32 g_vcn33_1_voltage_mt6789 = 3300000;
...
static INT32 consys_clk_get_from_dts(struct platform_device *pdev)
{
	...
	clk_infracfg_ao_ccif4_ap_cg_mt6789 = devm_clk_get(&pdev->dev, "ccif");
	if (IS_ERR(clk_infracfg_ao_ccif4_ap_cg_mt6789)) {
		WMT_PLAT_PR_INFO("[CCF]cannot get clk_infracfg_ao_ccif4_ap_cg_mt6789 clock.\n");
		return PTR_ERR(clk_infracfg_ao_ccif4_ap_cg_mt6789);
	}
	WMT_PLAT_PR_DBG("[CCF]clk_infracfg_ao_ccif4_ap_cg_mt6789=%p\n",
			clk_infracfg_ao_ccif4_ap_cg_mt6789);
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	consys_set_if_pinmux_mt6789_gen(enable);
}

static VOID consys_hw_reset_bit_set(MTK_WCN_BOOL enable)
{
	consys_hw_reset_bit_set_mt6789_gen(enable);
}
...
static INT32 consys_polling_goto_idle(VOID)
{
	 ...
	 */
	iRet = consys_polling_goto_idle_mt6789_gen(&consys_ver_id);
...
static VOID consys_hw_spm_clk_gating_enable(VOID)
{
	consys_hw_spm_clk_gating_enable_mt6789_gen();
}

int consys_platform_spm_conn_ctrl_mt6789(unsigned int enable)
{
	int ret = 0;
	struct platform_device *pdev = get_consys_device();

	if (!pdev) {
		WMT_PLAT_PR_INFO("get_consys_device fail.\n");
		return -1;
	}
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
...
#if CONSYS_PWR_ON_OFF_API_AVAILABLE
		iRet = clk_prepare_enable(clk_infracfg_ao_ccif4_ap_cg_mt6789);
		if (iRet) {
			WMT_PLAT_PR_INFO("clk_prepare_enable(infracfg_ao_ccif4) fail(%d)\n", iRet);
			return iRet;
		}
		WMT_PLAT_PR_DBG("clk_prepare_enable(clk_infracfg_ao_ccif4_ap_cg_mt6789) ok\n");

		iRet = consys_platform_spm_conn_ctrl_mt6789(1);
		if (iRet) {
			WMT_PLAT_PR_INFO("consys_platform_spm_conn_ctrl_mt6789 fail(%d)\n", iRet);
			return iRet;
		}
...
#else
		consys_hw_power_ctrl_mt6789_gen(true);
		...
		iRet = consys_platform_spm_conn_ctrl_mt6789(0);
		if (iRet) {
			WMT_PLAT_PR_INFO("consys_platform_spm_conn_ctrl_mt6789 fail(%d)\n", iRet);
			return iRet;
		}
		...
		clk_disable_unprepare(clk_infracfg_ao_ccif4_ap_cg_mt6789);
		WMT_PLAT_PR_DBG("clk_disable_unprepare(infracfg_ao_ccif4_ap_cg_mt6789) calling\n");

#else
		consys_hw_power_ctrl_mt6789_gen(false);
#endif /* CONSYS_PWR_ON_OFF_API_AVAILABLE */
	}
...
static INT32 polling_consys_chipid(VOID)
{
	...
	check = polling_consys_chipid_mt6789_gen(&consys_ver_id, &consys_fw_ver);
	if (check == 0) {
...
static VOID consys_set_access_emi_hw_mode(VOID)
{
	consys_set_access_emi_hw_mode_mt6789_gen();
}
...
static VOID consys_afe_reg_setting(VOID)
{
	...
	consys_afe_reg_setting_mt6789_gen();
#endif
}

static VOID consys_emi_entry_address(VOID)
{
	consys_emi_entry_address_mt6789_gen();
}

static VOID consys_set_xo_osc_ctrl(VOID)
{
	consys_set_xo_osc_ctrl_mt6789_gen();
}

static VOID consys_identify_adie(VOID)
{
	consys_identify_adie_mt6789_gen();
}

static VOID consys_wifi_ctrl_setting(VOID)
{
	consys_wifi_ctrl_setting_mt6789_gen();
}

static VOID consys_wifi_ctrl_switch_conn_mode(VOID)
{
	consys_wifi_ctrl_switch_conn_mode_mt6789_gen();
}

static VOID consys_bus_timeout_config(VOID)
{
	consys_bus_timeout_config_mt6789_gen();
}

static VOID consys_set_mcu_mem_pdn_delay(VOID)
{
	consys_set_mcu_mem_pdn_delay_mt6789_gen();
}
...
static INT32 consys_hw_vcn33_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (reg_VCN33_BT) {
			regulator_set_voltage(reg_VCN33_BT, g_vcn33_1_voltage_mt6789,
					g_vcn33_1_voltage_mt6789);
			if (regulator_enable(reg_VCN33_BT))
				WMT_PLAT_PR_INFO("WMT do WIFI PMIC on fail!\n");
		}
		if (g_regmap) {
			regmap_write(g_regmap, PMIC_RG_LDO_VCN33_EN_0_ADDR, 1);/* b0=1, b1= 0 */
			regmap_write(g_regmap, PMIC_RG_LDO_VCN33_SW_OP_EN_ADDR, 3); /* b0=1, b1=1 */
			regmap_write(g_regmap, PMIC_RG_LDO_VCN33_HW0_OP_CFG_ADDR, 2); /* bit1=1 */
		}
	} else {
		if (reg_VCN33_BT)
...
{
#if CONSYS_PMIC_CTRL_ENABLE
	/* vcn13 ext ctrl GPIO 68
	 * mode : 0 GPIO_BASE(0x10005000)+0x380[19:16]
	 * DIR  : 1 GPIO_BASE(0x10005000)+0x20[4]
	 * DOUT : 1/0 (high/low) GPIO_BASE(0x10005000)+0x120[4]
	 */
	...
	if (enable) {
		if (g_regmap) {
			...
			if (vs2_voter_vosel == 0) {
				regmap_read(g_regmap, PMIC_RG_BUCK_VS2_VOTER_VOSEL,
						&vs2_voter_vosel);
				WMT_PLAT_PR_INFO("Backup default value of vs2_voter_vosel = 0x%x\n",
						vs2_voter_vosel);
			}
			if (vs2_vosel == 0) {
				regmap_read(g_regmap, PMIC_RG_BUCK_VS2_VOSEL, &vs2_vosel);
				WMT_PLAT_PR_INFO("Backup default value of vs2_vosel = 0x%x\n",
						vs2_vosel);
			}
			if (vs2_vosel_sleep == 0) {
				regmap_read(g_regmap, PMIC_RG_BUCK_VS2_VOSEL_SLEEP,
						&vs2_vosel_sleep);
			...
			regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOTER_VOSEL, 0x46);
			/* Set VS2 to 1.4625V */
			regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOSEL, 0x4D);
			/* request VS2 to 1.4625V by VS2 VOTER (use bit 4) */
			regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOTER_EN_SET, 0x10);
			/* Set VS2 sleep to 1.375V */
			regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOSEL_SLEEP, 0x46);
		}
		...
		if (g_regmap) {
			...
			regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOTER_EN_CLR, 0x10);
			/* Set VS2 to VS2_VOTER_VOSEL */
			if (vs2_vosel > 0) {
				regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOSEL, vs2_vosel);
				WMT_PLAT_PR_INFO("Set vs2_vosel = 0x%x\n", vs2_vosel);
			}
			/* Set VS2 sleep to vs2_vosel_sleep */
			if (vs2_vosel_sleep > 0) {
				regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOSEL_SLEEP,
						vs2_vosel_sleep);
				WMT_PLAT_PR_INFO("Set vs2_vosel_sleep = 0x%x\n", vs2_vosel_sleep);
			}
			/* Set VS2 base to backup voltage */
			if (vs2_voter_vosel > 0) {
				regmap_write(g_regmap, PMIC_RG_BUCK_VS2_VOTER_VOSEL,
						vs2_voter_vosel);
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	if (enable) {
		...
		if (g_regmap) {
			...
			regmap_write(g_regmap, PMIC_RG_LDO_VCN18_EN_ADDR,  1); /* b0=1, b1=0 */
			regmap_write(g_regmap, PMIC_RG_LDO_VCN18_SW_OP_EN_ADDR, 3); /* b0=1, b1=1 */
			regmap_write(g_regmap, PMIC_RG_LDO_VCN18_HW0_OP_CFG_ADDR, 2); /* bit1=1 */
		}
...
static INT32 consys_hw_bt_vcn33_ctrl(UINT32 enable)
{
	...
	if (enable) {
		...
		if (g_regmap)
			regmap_write(g_regmap, PMIC_RG_LDO_VCN33_HW0_OP_CFG_ADDR, 0); /* bit1=0 */
...
static INT32 consys_hw_wifi_vcn33_ctrl(UINT32 enable)
{
	...
	if (enable) {
		...
		if (reg_VCN33_WIFI) {
			regulator_set_voltage(reg_VCN33_WIFI, g_vcn33_1_voltage_mt6789,
					g_vcn33_1_voltage_mt6789);
...
static UINT32 consys_read_cpupcr(VOID)
{
	...
	if (mtk_consys_check_reg_readable_by_addr(conn_reg.mcu_conn_hif_on_base +
			CONSYS_CPUPCR_OFFSET) == 0)
		return 0;
...
#define AP2CONN_OSC_EN_BIT	5
	...
	if (conn_reg.mcu_cfg_on_base != 0 &&
		...
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_MON_CTL_ADDR_OFFSET,
				0x80000001);
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_MON_SEL0_ADDR_OFFSET,
				0x03020100);
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_MON_SEL1_ADDR_OFFSET,
				0x07060504);
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_MON_SEL2_ADDR_OFFSET,
				0x0b0a0908);
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_MON_SEL3_ADDR_OFFSET,
				0x0f0e0d0c);
		CONSYS_REG_WRITE(conn_reg.mcu_top_misc_on_base + CONN_CFG_ON_DBGSEL_ADDR_OFFSET,
				0x3);
		value = CONSYS_REG_READ(conn_reg.mcu_top_misc_on_base +
				CONN_CFG_ON_MON_FLAG_RECORD_ADDR_OFFSET);
		...
		if ((mcu_osc_en + bt_osc_en + gps_osc_en + wifi_osc_en + fm_osc_en + ap2conn_osc_en)
				>= 1) {
			len += osal_sprintf(strbuf + len, " (%s%s%s%s%s%s",
					((mcu_osc_en == 1) ? "MCU," : ""), ((bt_osc_en == 1) ?
					"BT," : ""),
					((gps_osc_en == 1) ? "GPS," : ""), ((wifi_osc_en == 1) ?
					"WIFI," : ""),
					((fm_osc_en == 1) ? "FM," : ""), ((ap2conn_osc_en == 1) ?
					"AP2CONN," : ""));
	...
	if (magic_number == 0x7) {
		if (clock_hif_ctrl == 0x0 && clock_umac_ctrl == 0x0 && sleep_disable_mode == 0x0 &&
			...
			len += osal_sprintf(strbuf + len,
				" (gating,hif:0x%01x,umac:0x%01x,slp:0x%02x,sub:0x%02x,clk:0x%02x)",
				clock_hif_ctrl, clock_umac_ctrl, sleep_disable_mode,
				subsys_clk_req_stutus, clk_rate);
			if (state) {
...
static VOID consys_devapc_dump_debug_info(VOID)
{
	...
	if (atomic_read(&g_power_on) == 1) {
		...
		dump_conn_debug_dump_mt6789("DEVAPC_Violation");
	} else
...
INT32 consys_sleep_info_read_ctrl(WMT_SLEEP_COUNT_TYPE type, PUINT64 sleep_counter,
		PUINT64 sleep_timer)
{
	/*
	...
	if (sleep_counter) {
		...
		if (value_sleep_counter_rd == 0) {
			...
			if ((type == WMT_SLEEP_COUNT_BT || type == WMT_SLEEP_COUNT_WF)
					&& in_sleep_state == 1)
...
{
	return dump_conn_mcu_pc_log_mt6789("");
}
...
static INT32 consys_common_dump(const char *trg_str)
{
	...
	ret += dump_conn_mcu_pc_log_mt6789(trg_str);

	ret += dump_conn_debug_dump_mt6789(trg_str);
	ret += dump_conn_mcu_debug_flag_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer1_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer2_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer3_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer4_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_timeout_info_mt6789(trg_str);
	ret += dump_conn_bus_hang_debug_mt6789(trg_str);
	ret += dump_conn_mcu_apb_timeout_info_mt6789(trg_str);
	ret += dump_conn_apb_bus0_hang_mt6789(trg_str);
	ret += dump_conn_apb_bus1_hang_mt6789(trg_str);
	ret += dump_conn_apb_bus2_hang_mt6789(trg_str);
	ret += dump_conn_emi_ctrl_host_csr_mt6789(trg_str);
	ret += dump_conn_mcu_confg_emi_ctrl_mt6789(trg_str);
	ret += dump_conn_mcu_cpu_probe_mt6789(trg_str);
	ret += dump_conn_mcu_ahb_probe_mt6789(trg_str);
	ret += dump_conn_mcu_idlm_prot_prob_mt6789(trg_str);
	ret += dump_conn_mcu_wf_cmdbt_ram_prob_mt6789(trg_str);
	ret += dump_conn_mcu_pda_dbg_flag_mt6789(trg_str);
	ret += dump_conn_mcu_sysram_prb_mt6789(trg_str);
	ret += dump_conn_mcu_confg_mt6789(trg_str);
	ret += dump_conn_mcu_i_eidlm_mt6789(trg_str);
	ret += dump_conn_mcu_dma_mt6789(trg_str);
	ret += dump_conn_mcu_tcm_prob_mt6789(trg_str);
	ret += dump_conn_mcu_met_prob_mt6789(trg_str);
	ret += dump_conn_mcusys_n9_mt6789(trg_str);
	...
	/* Not dump_conn_cmdbt_register_mt6789 to prevent hang issue */
	ret += dump_conn_emi_detect_mt6789(trg_str);
	ret += dump_conn_cmdbt_debug_mt6789(trg_str);
	/* Not dump_conn_hif_reg_debug to prevent hang issue */
	/* Not dump_conn_mcu_confg_bus_hang_reg to prevent hang issue */
	ret += dump_wf_pdma_reg_debug_mt6789(trg_str);
	ret += dump_conn_to_EMI_bus_path_mt6789(trg_str);
...
static VOID consys_set_vcn33_1_voltage(UINT32 voltage)
{
	if (voltage == 0 || voltage == g_vcn33_1_voltage_mt6789)
		return;

	g_vcn33_1_voltage_mt6789 = voltage;
}
