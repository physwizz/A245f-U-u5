...
int mtk_cam_ctx_alloc_workers(struct mtk_cam_ctx *ctx)
{
	...
	if (CAM_DEBUG_ENABLED(CTRL))
		dev_info(ctx->cam->dev, "%s:%d: %p/%p/%p/%p\n",
			__func__, ctx->stream_id,
			ctx->kthread_packs[MTK_CAM_KTHREAD_SENSOR].worker_task,
			ctx->kthread_packs[MTK_CAM_KTHREAD_FLOW].worker_task,
			ctx->kthread_packs[MTK_CAM_KTHREAD_DONE].worker_task,
			ctx->kthread_packs[MTK_CAM_KTHREAD_TUNING].worker_task);

	for (i = MTK_CAM_KTHREAD_START; i < MTK_CAM_KTHREAD_NUM; i++) {
		if (!ctx->kthread_packs[i].worker_task) {
			kthread_init_worker(&ctx->kthread_packs[i].kworker);
			ctx->kthread_packs[i].worker_task =
				mtk_cam_ctx_create_task(ctx,
							name_table[i],
							&ctx->kthread_packs[i].kworker,
							prio_table[i]);
			if (!ctx->kthread_packs[i].worker_task)
				goto fail_uninit_worker_task;
		}
	}
...
static void mtk_cam_ctx_reset(struct mtk_cam_ctx *ctx)
{
	...
	struct mtk_cam_kthread_pack cached[MTK_CAM_KTHREAD_NUM];
	/* cached the pre alloc kthread */
	memcpy(cached, ctx->kthread_packs, sizeof(cached));
#endif
	...
	memcpy(ctx->kthread_packs, cached, sizeof(cached));
#endif
}
...
static inline bool _apply_mux_setting(char *prefix, struct mtk_cam_job *job,
				      struct mtk_cam_seninf_mux_setting *settings,
				      unsigned int num, bool cq_en)
{
	...
	param.rdy_mask_en.rdy_sw_en = 1;
	param.rdy_mask_en.rdy_cq_en = cq_en;
	param.rdy_mask_en.rdy_grp_en = 1;

	return mtk_cam_seninf_mux_setup(job->src_ctx->seninf, &param);
}

static inline int
_add_disable_mux_setting(struct mtk_cam_ctx *ctx,
			 struct mtk_cam_seninf_mux_setting *settings,
			 unsigned int *cnt)
{
	int i;

	for (i = 0; i < ctx->steaming_mux_num; i++) {
		...
		settings[*cnt].source = ctx->steaming_mux_pack[i * 2];
		settings[*cnt].camtg  = ctx->steaming_mux_pack[i * 2 + 1];
		settings[*cnt].enable = 0;
		(*cnt)++;
	}
...
static inline int
_add_sensor_meta_dump_mux_setting(struct mtk_cam_job *job,
				  struct mtk_cam_seninf_mux_setting *settings,
				  unsigned int *cnt,
				  ...
				  unsigned int sv_max_pixel_mode)
{
	...
	if (*cnt >= MUX_SETTING_NUM) {
		dev_err(job->src_ctx->cam->dev, "%s lack of mux settings", __func__);
		return -1;
	}

	settings[*cnt].seninf = job->src_ctx->seninf;
	settings[*cnt].source = PAD_SRC_GENERAL0;
	settings[*cnt].camtg  = sv_dev_cammux_id;
	settings[*cnt].tag_id = SVTAG_SENSOR_META;
...
static inline int
_add_mraw_mux_setting(struct mtk_cam_job *job,
		      struct mtk_cam_seninf_mux_setting *settings,
		      unsigned int *cnt,
		      int sv_dev_cammux_id,
		      unsigned int sv_max_pixel_mode)
...
static inline int
_add_camsv_mux_setting(struct mtk_cam_job *job,
		       struct mtk_cam_seninf_mux_setting *settings,
		       unsigned int *cnt,
		       int sv_dev_cammux_id,
		       unsigned int sv_max_pixel_mode)
...
static void
_store_mux_settings_to_ctx(struct mtk_cam_ctx *ctx,
			   struct mtk_cam_seninf_mux_setting *settings,
			   unsigned int idx_start, unsigned int idx_end)
{
	int idx, pack_idx;

	for (idx = idx_start; idx < idx_end; idx++) {
		pack_idx = (idx - idx_start) * 2;
		ctx->steaming_mux_pack[pack_idx] = settings[idx].source;
		ctx->steaming_mux_pack[pack_idx + 1] = settings[idx].camtg;
	}
...
static int _config_1exp_cam_mux(struct mtk_cam_job *job, bool disable_prev_mux)
{
	...
	struct mtk_camsv_device *sv_dev = NULL;
	struct mtk_cam_seninf_mux_setting settings[MUX_SETTING_NUM];
	int raw_id, raw_tg_idx, raw_set, first_tag_idx, sv_dev_cammux_id;
	unsigned int cnt, setting_start, sv_max_pixel_mode = 0;
	...
	if (raw_id < 0) {
		dev_info(cam->dev, "%s: no raw_id %d, used_engine:%#x",
			__func__, raw_id, job->used_engine);
		return -1;
	}
	...
	raw_set = MTK_SENINF_RAW_SET1;

	sv_dev = dev_get_drvdata(cam->engines.sv_devs[raw_id]);
	first_tag_idx = get_sv_tag_idx(1, MTKCAM_IPI_ORDER_FIRST_TAG, false,
		is_dcg_with_vs(job), is_fusion);
	sv_dev_cammux_id = sv_dev->cammux_id;
	CALL_PLAT_V4L2(get_sv_max_pixel_mode, sv_dev->id, &sv_max_pixel_mode);

	is_dc = is_dc_mode(job) ? true : false;
	is_offline_ts = is_offline_timeshare(job) ? true : false;

	if (disable_prev_mux) {
		if (_add_disable_mux_setting(ctx, settings, &cnt))
			goto WARN_EXIT;
	}
	...
	if (is_dcg_with_vs(job)) {
		/* 1st exp, PAD_SRC_RAW0 */
		/* camsv for pure raw and dc */
		if (cnt >= MUX_SETTING_NUM)
			goto WARN_EXIT;
	...
	if (_add_sensor_meta_dump_mux_setting(job, settings, &cnt,
					      sv_dev_cammux_id, sv_max_pixel_mode))
		goto WARN_EXIT;

	if (_add_mraw_mux_setting(job, settings, &cnt, sv_dev_cammux_id,
				  sv_max_pixel_mode))
		goto WARN_EXIT;

	if (_add_camsv_mux_setting(job, settings, &cnt, sv_dev_cammux_id,
				   sv_max_pixel_mode))
		goto WARN_EXIT;
	...
	_store_mux_settings_to_ctx(ctx, settings, setting_start, cnt);

	return 0;

WARN_EXIT:
	WARN(1, "%s lack of mux settings %u", __func__, cnt);
	return -1;
}

static int _config_2exp_cam_mux(struct mtk_cam_job *job, bool disable_prev_mux)
{
	struct mtk_cam_ctx *ctx = job->src_ctx;
	struct mtk_cam_device *cam = ctx->cam;
	struct v4l2_subdev *seninf = ctx->seninf;
	struct mtk_camsv_device *sv_dev = NULL;
	struct mtk_cam_seninf_mux_setting settings[MUX_SETTING_NUM];
	int raw_id, raw_tg_idx, raw_set, first_tag_idx, last_tag_idx;
	int sv_dev_cammux_id, hw_scen;
	unsigned int cnt, setting_start , sv_max_pixel_mode = 0;
	bool is_dc, is_offline_ts;
	bool is_fusion = job_sensor_exp_num(job) == job_exp_num(job) ? true : false;
	...
	sv_dev = dev_get_drvdata(cam->engines.sv_devs[raw_id]);
	first_tag_idx = get_sv_tag_idx(2, MTKCAM_IPI_ORDER_FIRST_TAG, false,
		is_dcg_with_vs(job), is_fusion);
	last_tag_idx = get_sv_tag_idx(2, MTKCAM_IPI_ORDER_LAST_TAG, false,
		is_dcg_with_vs(job), is_fusion);
	sv_dev_cammux_id = sv_dev->cammux_id;
	CALL_PLAT_V4L2(get_sv_max_pixel_mode, sv_dev->id, &sv_max_pixel_mode);
...
int apply_cam_mux_switch(struct mtk_cam_job *job, bool disable_prev_mux)
{
	...
	case 1:
		ret = _config_1exp_cam_mux(job, disable_prev_mux);
		break;
	case 2:
		ret = _config_2exp_cam_mux(job, disable_prev_mux);
		break;
	default:  /* TODO: 3exp */
		dev_info(job->src_ctx->cam->dev, "%s unknown exp %d",
			__func__, config_exposure_num);
		ret = -1;
		break;
	};
...
int ctx_stream_on_seninf_sensor(struct mtk_cam_job *job, int seninf_pad_bitmask)
{
	...
	/* camtg */
	apply_cam_mux_switch(job, false);

	/* TODO: merge pixelmode API */
	/* raw */
	if (ctx->has_raw_subdev) {
		for (seninf_pad = PAD_SRC_RAW0;
		     seninf_pad <= PAD_SRC_RAW2;
		     seninf_pad++) {
			if (seninf_pad_bitmask & 1 << seninf_pad)
				mtk_cam_seninf_set_pixelmode(seninf, seninf_pad,
							     GET_PLAT_V4L2(raw_tg_pixelmode));
		}
	}
...
void mtk_cam_ctx_raw_qof_disable(struct mtk_cam_ctx *ctx)
{
	int i;
...
int mtk_cam_ctx_queue_sensor_worker(struct mtk_cam_ctx *ctx,
				    struct kthread_work *work)
{
	return ctx_kthread_queue_work(ctx, &ctx->kthread_packs[MTK_CAM_KTHREAD_SENSOR].kworker,
				      work, __func__);
}
...
int mtk_cam_ctx_queue_done_worker(struct mtk_cam_ctx *ctx,
				  struct kthread_work *work)
{
	return ctx_kthread_queue_work(ctx, &ctx->kthread_packs[MTK_CAM_KTHREAD_DONE].kworker,
				      work, __func__);
}
...
int mtk_cam_ctx_queue_flow_worker(struct mtk_cam_ctx *ctx,
				  struct kthread_work *work)
{
	return ctx_kthread_queue_work(ctx, &ctx->kthread_packs[MTK_CAM_KTHREAD_FLOW].kworker,
				      work, __func__);
}
...
int mtk_cam_ctx_queue_tuning_worker(struct mtk_cam_ctx *ctx,
				    struct kthread_work *work)
{
	return ctx_kthread_queue_work(ctx, &ctx->kthread_packs[MTK_CAM_KTHREAD_TUNING].kworker,
				      work, __func__);
}
...
static struct component_match *mtk_cam_match_add(struct device *dev)
{
	...
	int yuv_num, rms_num, camsv_real_hw_num = 0;

	(void) camsv_real_hw_num;
...
static int rms_power_on(struct device *dev)
{
	...
	raw = dev_get_drvdata(cam->engines.raw_devs[rms->id]);

	if (qof_is_enabled(raw))
		ret = qof_rms_mtcmos_ctrl_get(raw);
	else
...
static int rms_power_off(struct device *dev)
{
	struct mtk_rms_device *rms = dev_get_drvdata(dev);
	struct mtk_cam_device *cam = rms->cam;
	struct mtk_raw_device *raw;
	...
	if (qof_is_enabled(raw))
		ret = qof_rms_mtcmos_ctrl_put(raw);
	else
	...
	unsigned long engine_mask, int enable)
{
	if (enable) {
		if (rms_freerun) {
			loop_each_engine_rms(eng, engine_mask, rms_power_on, enable);
		...
		if (rms_freerun) {
			loop_each_engine_rms(eng, engine_mask, rms_power_off, enable);
...
static int mtk_cam_probe(struct platform_device *pdev)
{
	...
	cam_dev->hwccf_link_set = ioremap(REG_HWCCF_BASE + REG_HWCCF_LINK_SET_ADDR, 0x4);
	if (IS_ERR(cam_dev->hwccf_link_set)) {
		dev_err(dev, "%s: failed to map hwccf_link_set\n", __func__);
		cam_dev->hwccf_link_set = NULL;
	}

	cam_dev->hwccf_link_clr = ioremap(REG_HWCCF_BASE + REG_HWCCF_LINK_CLR_ADDR, 0x4);
	if (IS_ERR(cam_dev->hwccf_link_clr)) {
		dev_err(dev, "%s: failed to map hwccf_link_clr\n", __func__);
		cam_dev->hwccf_link_clr = NULL;
	}

	cam_dev->hwccf_link_status = ioremap(REG_HWCCF_BASE + REG_HWCCF_LINK_STA_ADDR, 0x4);
	if (IS_ERR(cam_dev->hwccf_link_status)) {
		dev_err(dev, "%s: failed to map hwccf_link_status\n", __func__);
		cam_dev->hwccf_link_status = NULL;
	}
...
static int mtk_cam_runtime_suspend(struct device *dev)
{
	...
	if (cam_dev->dvfs.mmdvfs_clk && clk_num > 0)
		clk_num--;
