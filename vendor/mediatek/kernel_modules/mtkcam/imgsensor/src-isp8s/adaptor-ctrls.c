...
static void dump_perframe_info(struct adaptor_ctx *ctx, struct mtk_hdr_ae *ae_ctrl)
{
	...
	if (ctx->latest_ebd.recv_ts) {
		...
		if (ebd_msg) {
			...
			if (sret <= 0) {
				adaptor_loge(ctx, "show ebd log failed\n");
				kfree(ebd_msg);
				ebd_msg = NULL;
			}
		}
...
static void get_dispatch_gain(struct adaptor_ctx *ctx, u32 tgain, u32 *again, u32 *dgain)
{
	...
	adaptor_logm(ctx, "again tlb cnt = %u sz(%u), gain(t/a/d) = %u / %u / %u\n",
	 ...
	 const u64 delay_ms)
{
	...
	if (_adaptor_work == NULL) {
		adaptor_loge(ctx, "_adaptor_work kmalloc failed\n");
		return 0;
	}

	if (!ctx->is_streaming) {
		adaptor_logi(ctx, "ctx->is_streaming if %d, skip the func\n",
				ctx->is_streaming);
		kfree(_adaptor_work);
		return 0;
	}
	...
	adaptor_logi(ctx, "queue worker_done %llu\n", time_boot);
...
static int _sensor_reset_s_stream(struct v4l2_ctrl *ctrl)
{
	...
	adaptor_logm(ctx, "val: %d, stream_off_state: %d\n",
...
static int _aov_switch_i2c_bus_scl_aux(struct v4l2_ctrl *ctrl)
{
	...
	case SCL13:
		if (!ctx->pinctrl || !ctx->state[STATE_SCL_AP]) {
			adaptor_loge(ctx, "ctx->pinctrl = 0x%p ctx->state[STATE_SCL_AP] = 0x%p\n",
				ctx->pinctrl,
				ctx->state[STATE_SCL_AP]);
			return -EINVAL;
		}
		ret = pinctrl_select_state(ctx->pinctrl, ctx->state[STATE_SCL_AP]);
		if (ret < 0) {
			adaptor_logi(ctx, "select(%s)(fail) %d\n",
				state_names[STATE_SCL_AP], ret);
			return ret;
		}
		adaptor_logi(ctx, "select(%s)(correct)\n",
	...
	case SCL3:
		if (!ctx->pinctrl || !ctx->state[STATE_SCL_SCP]) {
			adaptor_loge(ctx, "ctx->pinctrl = 0x%p ctx->state[STATE_SCL_SCP] = 0x%p\n",
			ctx->pinctrl,
			ctx->state[STATE_SCL_SCP]);
			return -EINVAL;
		}
	...
	default:
		adaptor_logi(ctx,
			"i2c bus aux function not support(%d)\n",
			ctrl->val);
		return -EINVAL;
	}
...
static int _aov_switch_i2c_bus_sda_aux(struct v4l2_ctrl *ctrl)
{
	...
	case SDA13:
		if (!ctx->pinctrl || !ctx->state[STATE_SDA_AP]) {
			adaptor_loge(ctx, "ctx->pinctrl = 0x%p ctx->state[STATE_SDA_AP] = 0x%p\n",
			ctx->pinctrl,
			ctx->state[STATE_SDA_AP]);
			return -EINVAL;
		}
	...
	case SDA3:
		if (!ctx->pinctrl || !ctx->state[STATE_SDA_SCP]) {
			adaptor_loge(ctx, "ctx->pinctrl = 0x%p ctx->state[STATE_SDA_SCP] = 0x%p\n",
			ctx->pinctrl,
			ctx->state[STATE_SDA_SCP]);
			return -EINVAL;
		}
		...
		break;
	default:
		adaptor_logi(ctx, "i2c bus aux function not support(%d)\n",
			ctrl->val);
		return -EINVAL;
	}
...
static int _aov_switch_rx_param(struct v4l2_ctrl *ctrl)
{
	...
	case CSI_CLK_312:
		...
		break;
	default:
		adaptor_loge(ctx, "csi clk not support(%d)\n",
			ctrl->val);
		return -EINVAL;
	}
...
static int _aov_switch_pm_ops(struct v4l2_ctrl *ctrl)
{
	...
	case AOV_PM_RELAX:
		if (ctx->aov_pm_ops_flag == 1) {
			...
			adaptor_logi(ctx,
				"switch to __pm_relax(correct),pm_ops_flag(%d)\n",
				ctx->aov_pm_ops_flag);
		} else
			adaptor_logi(ctx,
				"switch to __pm_relax(no need),pm_ops_flag(%d)\n",
				ctx->aov_pm_ops_flag);
		break;
	case AOV_PM_STAY_AWAKE:
		if (ctx->aov_pm_ops_flag == 0) {
			...
			adaptor_logi(ctx,
				"switch to __pm_stay_awake(correct),pm_ops_flag(%d)\n",
				ctx->aov_pm_ops_flag);
		} else
			adaptor_logi(ctx,
				"switch to __pm_stay_awake(no need),pm_ops_flag(%d)\n",
				ctx->aov_pm_ops_flag);
		break;
	case AOV_ABNORMAL_FORCE_SENSOR_PWR_OFF:
		adaptor_hw_power_off(ctx);
		adaptor_logi(ctx, "adaptor_hw_power_off(done)");
		break;
...
static int imgsensor_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (is_lock) {
		adaptor_logi(ctx,
		"skip imgsensor set ctrl with ctrl_id_offset(%u), sof_cnt(%u) p_set_ctrl_unlock_flag(%u)\n",
	...
	case V4L2_CID_START_SEAMLESS_SWITCH:
		{
			...
			if (info->target_scenario_id == 0 &&
				...
				adaptor_logi(ctx, "V4L2_CID_START_SEAMLESS_SWITCH %u invalid value\n",
					info->target_scenario_id);
				break;
			}
			...
			if (info->target_scenario_id < MODE_MAXCNT)
				...
				adaptor_loge(ctx, "info->target_scenario_id %d >= MODE_MAXCNT\n",
					info->target_scenario_id);
			}

			ctx->exposure->val = FINE_INTEG_CONVERT(info->ae_ctrl[0].exposure.arr[0],
				ctx->cur_mode->fine_intg_line);
	...
	case V4L2_CID_MTK_SENSOR_INIT:
		adaptor_logm(ctx, "V4L2_CID_MTK_SENSOR_INIT val = %d\n", ctrl->val);
		if (ctrl->val)
	...
	case V4L2_CID_MTK_AOV_SWITCH_I2C_BUS_SCL_AUX:
		...
		if (ret < 0)
			adaptor_loge(ctx,
				"_aov_switch_i2c_bus_scl_aux(fail),ret(%d)\n",
				ret);
		else
			adaptor_logi(ctx,
				"_aov_switch_i2c_bus_scl_aux(correct),ret(%d)\n",
				ret);
		break;
	case V4L2_CID_MTK_AOV_SWITCH_I2C_BUS_SDA_AUX:
		...
		if (ret < 0)
			adaptor_loge(ctx,
				"_aov_switch_i2c_bus_sda_aux(fail),ret(%d)\n",
				ret);
		else
			adaptor_logi(ctx,
				"_aov_switch_i2c_bus_sda_aux(correct),ret(%d)\n",
				ret);
		break;
	...
	case V4L2_CID_MTK_AOV_SWITCH_PM_OPS:
		...
		if (ret < 0)
			adaptor_loge(ctx,
				"_aov_switch_pm_ops(fail),ret(%d)\n",
				ret);
		else
			adaptor_logi(ctx,
				"_aov_switch_pm_ops(correct),ret(%d)\n",
				ret);
		break;
	case V4L2_CID_MTK_AOV_SWITCH_MCLK_ULPOSC:
		...
		break;
	case V4L2_CID_MTK_WAKEUP_CAMERA_PMIC:
		dev_info(dev, "V4L2_CID_MTK_WAKEUP_CAMERA_PMIC val = %d\n", ctrl->val);
		if (ctrl->val)
...
static const struct v4l2_ctrl_config cfg_wake_up_camera_pmic = {
	.ops = &ctrl_ops,
	.id = V4L2_CID_MTK_WAKEUP_CAMERA_PMIC,
	.name = "wake_up_cam_pmic",
	.type = V4L2_CTRL_TYPE_BOOLEAN,
	.flags = V4L2_CTRL_FLAG_EXECUTE_ON_WRITE,
	.max = 1,
	.step = 1,
