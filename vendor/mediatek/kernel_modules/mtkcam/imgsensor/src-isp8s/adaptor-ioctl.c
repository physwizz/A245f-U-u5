...
static int g_vcinfo_by_scenario(struct adaptor_ctx *ctx, void *arg)
{
	...
	if (copy_to_user((void *)info->p_vcinfo, vcinfo2, sizeof(struct SENSOR_VC_INFO2_STRUCT))) {
		kfree(vcinfo2);
		adaptor_loge(ctx, "copy_to_user fail\n");
		return -EFAULT;
	}
...
static int g_sensor_info(struct adaptor_ctx *ctx, void *arg)
{
	...
	if (ret < 0)
		adaptor_loge(ctx, "get sensor info fail, ret:%d\n", ret);
...
static int g_output_format_by_scenario(struct adaptor_ctx *ctx, void *arg)
{
	...
	target->value = para.u64[1];
	adaptor_logd(ctx, "scenario %u outputformat = %u\n",
...
static int g_seamless_switch_scenario(struct adaptor_ctx *ctx, void *arg)
{
	...
	if (ret_for_copy_to_user)
		adaptor_logi(ctx, "ret_for_copy_to_user: %d\n", ret_for_copy_to_user);
...
static int g_fsync_frame_length_info(struct adaptor_ctx *ctx, void *arg)
{
	...
	if (unlikely(arg == NULL)) {
		ret = -ENOIOCTLCMD;
		adaptor_loge(ctx,
			"VIDIOC_MTK_G_FS_FRAME_LENGTH_INFO, get input pointer arg:%p, return\n",
			arg);
		return ret;
	}
	...
	adaptor_logi(ctx,
		"VIDIOC_MTK_G_FS_FRAME_LENGTH_INFO, idx:%u, target_min_fl_us:%u, out_fl_us:%u, ret:%d\n",
		ctx->idx,
...
static int s_tg(struct adaptor_ctx *ctx, void *arg)
{
	...
	adaptor_logi(ctx, "notify frame-sync module of TG %d\n", *p_tg);
