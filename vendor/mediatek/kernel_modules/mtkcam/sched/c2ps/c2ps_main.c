...
{
	struct cpu_info *g_cpu_info = NULL;
	cpumask_t mask;
	...
	cpumask_clear(&mask);
	if (likely(g_cpu_info->cluster_first_cpu[1] > 0)) {
		int cpu;

		for (cpu = 1; cpu < g_cpu_info->cluster_first_cpu[1]; cpu++)
			cpumask_set_cpu(cpu, &mask);
		set_cpus_allowed_ptr(c2ps_tsk, &mask);
	}
